...
import cromwell.backend.google.pipelines.batch.actors.GcpBatchInitializationActorSpec._
import cromwell.backend.google.pipelines.batch.models.GcpBatchTestConfig.{BatchGlobalConfig, googleConfiguration, batchAttributes}
...
class GcpBatchInitializationActorSpec extends TestKitSuite with AnyFlatSpecLike with Matchers
  with ImplicitSender {
  ...
  private def getJesBackendProps(workflowDescriptor: BackendWorkflowDescriptor,
                                 ...
                                 jesConfiguration: GcpBatchConfiguration): Props = {
    val ioActor = mockIoActor
    val params = GcpBatchInitializationActorParams(workflowDescriptor, ioActor, calls, jesConfiguration, emptyActor, restarting = false)
  ...
  private def getJesBackend(workflowDescriptor: BackendWorkflowDescriptor, calls: Set[CommandCallNode], conf: BackendConfigurationDescriptor) = {
    val props = getJesBackendProps(workflowDescriptor, calls, new GcpBatchConfiguration(conf, googleConfiguration, batchAttributes))
