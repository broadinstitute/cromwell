...
private[executable] object ExecutableValidation {
  ...
  private[executable] def validateExecutable(entryPoint: ExecutableCallable,
                                             ...
                                             parseGraphInputs: (Graph,
                                                                Map[String, DelayedCoercionFunction],
                                                                IoFunctionSet
                                             ) => IOChecked[ResolvedExecutableInputs],
                                             inputFile: Option[String],
                                             ioFunctions: IoFunctionSet
  ): IOChecked[Executable] = for {
    parsedInputs <- inputFile
      .map(inputParsingFunction)
      .map(_.toIOChecked)
      .getOrElse(IOChecked.pure(Map.empty[String, DelayedCoercionFunction]))
