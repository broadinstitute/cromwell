...
trait CustomLabelEntryComponent {
  ...
  class CustomLabelEntries(tag: Tag) extends Table[CustomLabelEntry](tag, "CUSTOM_LABEL_ENTRY") {
    ...
    def fkCustomLabelEntryWorkflowExecutionUuid = foreignKey("FK_CUSTOM_LABEL_ENTRY_WORKFLOW_EXECUTION_UUID",
                                                             ...
                                                             workflowMetadataSummaryEntries
    )(_.workflowExecutionUuid, onDelete = Cascade)

    def ucCustomLabelEntryClkWeu =
      index("UC_CUSTOM_LABEL_ENTRY_CLK_WEU", (customLabelKey, workflowExecutionUuid), unique = true)

    def ixCustomLabelEntryClkClv =
      index("IX_CUSTOM_LABEL_ENTRY_CLK_CLV", (customLabelKey, customLabelValue), unique = false)
  }
  ...
  val customLabelEntriesForWorkflowExecutionUuidAndLabelKey =
    Compiled((workflowExecutionUuid: Rep[String], labelKey: Rep[String]) =>
      for {
  ...
  def existsWorkflowIdLabelKeyAndValue(workflowId: Rep[String],
                                       labelKey: Rep[String],
                                       labelValue: Rep[String]
  ): Rep[Boolean] =
    customLabelEntries
      .filter(customLabelEntry =>
  ...
  val labelsForWorkflowExecutionUuid = Compiled((workflowExecutionUuid: Rep[String]) =>
    for {
      customLabelEntry <- customLabelEntries
      if customLabelEntry.workflowExecutionUuid === workflowExecutionUuid
  ...
  val labelsForWorkflowAndSubworkflows = Compiled((workflowExecutionUuid: Rep[String]) =>
    for {
