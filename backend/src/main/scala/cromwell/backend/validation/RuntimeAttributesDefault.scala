...
object RuntimeAttributesDefault {
  ...
  def workflowOptionsDefault(options: WorkflowOptions,
                             mapping: Map[String, Iterable[WomType]]
    ...
    } recover { case _: OptionNotFoundException =>
   ...
   */
  def withDefaults(attrs: EvaluatedRuntimeAttributes,
                   defaultsList: List[EvaluatedRuntimeAttributes]
  ): EvaluatedRuntimeAttributes =
    defaultsList.foldLeft(attrs)((acc, default) => acc ++ default.view.filterKeys(!acc.keySet.contains(_)))

  def noValueFoundFor[A](attribute: String): ValidatedNel[String, A] =
    s"Can't find an attribute value for key $attribute".invalidNel
}
