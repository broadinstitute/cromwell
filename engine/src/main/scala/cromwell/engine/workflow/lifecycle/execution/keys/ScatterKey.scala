...
private[execution] case class ScatterKey(node: ScatterNode) extends JobKey {
  ...
  def makeCollectors(count: Int, scatterCollectionFunction: ScatterCollectionFunction): Set[ScatterCollectorKey] =
    (node.outputMapping.groupBy(_.outputToGather.source.graphNode) flatMap {
  ...
  def processRunnable(data: WorkflowExecutionActorData,
    ...
    def buildExecutionDiff(scatterSize: Int, arrays: List[ScatterVariableAndValue]): WorkflowExecutionDiff =
      if (scatterSize > maxScatterWidth) {
        workflowExecutionActor ! JobFailedNonRetryableResponse(
          this,
          new Exception(s"Workflow scatter width of $scatterSize exceeds configured limit of $maxScatterWidth."),
          None
        )
