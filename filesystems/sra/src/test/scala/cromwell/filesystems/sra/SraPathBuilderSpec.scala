...
class SraPathBuilderSpec extends TestKitSuite with AnyFlatSpecLike with Matchers with PathBuilderSpecUtils {
  ...
  goodPaths foreach { good =>
    it should behave like buildGood(good)
  }

  badPaths foreach { bad =>
    it should behave like buildBad(bad)
  }
  ...
  private case class Good(description: String, in: String, accession: String, path: String)
