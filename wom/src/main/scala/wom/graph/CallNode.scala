...
final case class ExpressionCallNode private (
  override val identifier: WomIdentifier,
  ...
  override val sourceLocation: Option[SourceFileLocation]
    ...
    with ExpressionNodeLike {
  val callType: String = "expression task"
  lazy val expressionBasedOutputPorts: List[ExpressionBasedOutputPort] =
    callable.outputs.map(o =>
      ExpressionBasedOutputPort(outputIdentifierCompoundingFunction(identifier, o.localName.value),
                                o.womType,
                                this,
                                o.expression
      )
...
final case class CommandCallNode private (override val identifier: WomIdentifier,
  ...
  /**
    ...
    */
  def customOutputEvaluation(inputs: Map[String, WomValue],
                             ioFunctionSet: IoFunctionSet,
                             executionContext: ExecutionContext
  ): OutputFunctionResponse =
    callable.customizedOutputEvaluation(outputPorts, inputs, ioFunctionSet, executionContext)
}

final case class WorkflowCallNode private (
  override val identifier: WomIdentifier,
  ...
  val subworkflowCallOutputPorts: Set[SubworkflowCallOutputPort] =
    callable.innerGraph.nodes.collect { case gon: GraphOutputNode =>
      SubworkflowCallOutputPort(outputIdentifierCompoundingFunction(identifier, gon.localName), gon, this)
    }
  ...
  def graphFromDefinition(taskDefinition: TaskDefinition): ErrorOr[Graph] = {
    ...
    val inputDefinitionFold = taskDefinition.inputs.foldMap { inputDef =>
      val newNode: Option[ExternalGraphInputNode] = inputDef match {
        case RequiredInputDefinition(name, womType, valueMapper, _) =>
          Some(RequiredGraphInputNode(identifier(name), womType, name.value, valueMapper))
        case OverridableInputDefinitionWithDefault(name, womType, default, valueMapper, _) =>
    ...
    val callWithInputs = callNodeBuilder.build(uniqueIdentifier,
                                               taskDefinition,
                                               inputDefinitionFold,
                                               Set.empty,
                                               taskDefinition.sourceLocation
    )

    val outputNodes: ErrorOr[Seq[GraphOutputNode]] = callWithInputs.node.outputPorts
      .map(output =>
        PortBasedGraphOutputNode(
          identifier(LocalName(output.internalName)),
          output.womType,
          output
        )
      )
...
object CallNode {
  def resolveAndEvaluateInputs(callNode: CallNode,
                               ...
                               outputStoreLookup: OutputPort => ErrorOr[WomValue]
  ): ErrorOr[WomEvaluatedCallInputs] =
    callNode.inputDefinitionMappings
      .foldLeft(Map.empty[InputDefinition, ErrorOr[WomValue]]) {
        case (accumulatedInputsSoFar, (inputDefinition, pointer)) =>
          ...
          val coercedValue = pointer
            .fold(InputPointerToWomValue)
            .apply(
              validInputsAccumulated,
              expressionLanguageFunctions,
              outputStoreLookup,
              inputDefinition
            ) flatMap (inputDefinition.womType.coerceRawValue(_).toIOChecked)

          val contextualizedValue =
            coercedValue.contextualizeErrors(s"evaluate input '${inputDefinition.localName.value}'")
  ...
  private[graph] def apply(nodeIdentifier: WomIdentifier,
    ...
    case t: CommandTaskDefinition =>
      CommandCallNode(nodeIdentifier,
                      t,
                      inputPorts,
                      inputDefinitionMappings,
                      mustFollow,
                      outputIdentifierCompoundingFunction,
                      sourceLocation
      )
    case w: WorkflowDefinition =>
      WorkflowCallNode(nodeIdentifier,
                       w,
                       inputPorts,
                       inputDefinitionMappings,
                       mustFollow,
    ...
    case w: ExpressionTaskDefinition =>
      ExpressionCallNode(nodeIdentifier,
                         w,
                         inputPorts,
                         inputDefinitionMappings,
                         outputIdentifierCompoundingFunction,
  ...
  class CallNodeBuilder {
    ...
    def build(nodeIdentifier: WomIdentifier,
              ...
              sourceLocation: Option[SourceFileLocation],
              outputIdentifierCompoundingFunction: (WomIdentifier, String) => WomIdentifier =
                defaultOutputIdentifierCompounder
    ): CallNodeAndNewNodes = {
      val callNode = CallNode(nodeIdentifier,
                              callable,
                              inputDefinitionFold.callInputPorts,
                              inputDefinitionFold.mappings,
                              mustFollow,
                              outputIdentifierCompoundingFunction,
                              sourceLocation
      )
      graphNodeSetter._graphNode = callNode
      CallNodeAndNewNodes(callNode,
                          inputDefinitionFold.newGraphInputNodes,
                          inputDefinitionFold.newExpressionNodes,
                          inputDefinitionFold.usedOuterGraphInputNodes
      )
    }
