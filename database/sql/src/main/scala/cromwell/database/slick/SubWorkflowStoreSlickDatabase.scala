...
trait SubWorkflowStoreSlickDatabase extends SubWorkflowStoreSqlDatabase {
  ...
  def addSubWorkflowStoreEntry(rootWorkflowExecutionUuid: String,
    ...
    val action = for {
      workflowStoreEntry <- dataAccess
        .workflowStoreEntriesForWorkflowExecutionUuid(rootWorkflowExecutionUuid)
        .result
        .headOption
      _ <- workflowStoreEntry match {
        ...
        case None =>
          DBIO.failed(
            new IllegalArgumentException(s"Could not find root workflow with UUID $rootWorkflowExecutionUuid")
          )
      }
    } yield ()
  ...
  override def querySubWorkflowStore(parentWorkflowExecutionUuid: String,
    ...
    val action = for {
      subWorkflowStoreEntryOption <- dataAccess
        .subWorkflowStoreEntriesForJobKey(
