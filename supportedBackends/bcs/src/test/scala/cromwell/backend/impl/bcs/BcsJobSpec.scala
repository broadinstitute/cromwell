...
class BcsJobSpec extends BcsTestUtilSpec {
  ...
  it should "have correct timeout" in {
    val timeout = 3000
    val runtime = Map("timeout" -> WdlInteger(timeout))
  ...
  it should "have correct mounts" in {
    ...
    val runtime = Map("mounts" -> WdlString(s"$src $dest $writeSupport"))
  ...
  it should "have correct cluster id" in {
    ...
    val runtime = Map("cluster" -> WdlString(clusterId))
  ...
  it should "have correct docker option" in {
    ...
    val runtime = Map("docker" -> WdlString(s"$dockerImage $dockerPath"))
  ...
  it should "have correct auto cluster configuration" in {
    ...
    val runtime = Map(
      "cluster" -> WdlString(cluster),
      "reserveOnFail" -> WdlBoolean(reserveOnFail),
      "vpc" -> WdlString(s"$cidr $vpcId"),
      "systemDisk" -> WdlString(s"$systemDiskType $systemDiskSize"),
      "dataDisk" -> WdlString(s"$dataDiskType $dataDiskSize $dataDiskMountPoint"),
      "userData" -> WdlString(s"$userDataKey $userDataValue")
    )
  ...
  private def withRuntime(runtime: Map[String, WdlValue] = Map.empty[String, WdlValue]): BcsJob = {
  ...
  private def taskWithRuntime(runtime: Map[String, WdlValue] = Map.empty[String, WdlValue]): TaskDescription = {
