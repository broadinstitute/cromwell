...
object LanguageFactoryUtil {
   ...
   */
  private def validateExecutableInputs(inputs: ResolvedExecutableInputs,
                                       ioFunctions: IoFunctionSet
  ): ErrorOr[Map[OutputPort, WomValue]] = {
    import common.validation.ErrorOr.MapTraversal
    inputs.traverse { case (key, value) =>
      value.fold(ResolvedExecutableInputsPoly).apply(ioFunctions) map { key -> _ }
    }
  }
  ...
  def simpleLooksParseable(startsWithOptions: List[String], commentIndicators: List[String])(
    content: String
  ): Boolean = {
  ...
  def chooseFactory(workflowSource: WorkflowSource, wsfc: WorkflowSourceFilesCollection): ErrorOr[LanguageFactory] =
    wsfc.workflowType match {
      ...
      case None =>
        ...
        allFactories
          .find(_.looksParsable(workflowSource))
          .getOrElse(CromwellLanguages.instance.default.default)
          .validNel
    }

  def findWorkflowSource(workflowSource: Option[WorkflowSource],
                         workflowUrl: Option[WorkflowUrl],
                         resolvers: List[ImportResolver]
  ): Checked[(WorkflowSource, List[ImportResolver])] =
    (workflowSource, workflowUrl) match {
      ...
      case (None, Some(url)) =>
        val compoundImportResolver: CheckedAtoB[ImportResolutionRequest, ResolvedImportBundle] =
          CheckedAtoB.firstSuccess(resolvers.map(_.resolver), s"resolve workflowUrl '$url'")
        val wfSourceAndResolvers: Checked[ResolvedImportBundle] =
          compoundImportResolver.run(ImportResolutionRequest(url, resolvers))
