...
object LocalJobExpressionFunctions {
  private val LocalFSScheme = "file"
  def isLocalPath(path: Path) = path.toUri.getScheme == LocalJobExpressionFunctions.LocalFSScheme
...
class LocalJobExpressionFunctions(override val fileSystems: List[FileSystem],
                                  context: CallContext
                                 ) extends WdlStandardLibraryFunctions with PureFunctions with ReadLikeFunctions with WriteFunctions {
  import LocalJobExpressionFunctions._
