...
class DrsResolverSpec extends FlatSpec with Matchers with StrictLogging {
  ...
  it should "find GCS path when its the only one in url array" in {
    val drsPath = drsPathBuilder
      .build(logger.underlying, MockDrsPaths.drsPathResolvingToOneGcsPath, Nil)
      .get
      .asInstanceOf[DrsPath]
  ...
  it should "find GCS path when DRS path resolves to multiple urls" in {
    val drsPath = drsPathBuilder
      .build(logger.underlying, MockDrsPaths.drsPathResolvingToMultiplePaths, Nil)
      .get
      .asInstanceOf[DrsPath]
  ...
  it should "throw GcsUrlNotFoundException when DRS path doesn't resolve to at least one GCS url" in {
    val drsPath = drsPathBuilder
      .build(logger.underlying, MockDrsPaths.drsPathResolvingToNoGcsPath, Nil)
      .get
      .asInstanceOf[DrsPath]
  ...
  it should "throw Runtime Exception when Martha can't find the given DRS path " in {
    val drsPath = drsPathBuilder
      .build(logger.underlying, MockDrsPaths.drsPathNotExistingInMartha, Nil)
      .get
      .asInstanceOf[DrsPath]
