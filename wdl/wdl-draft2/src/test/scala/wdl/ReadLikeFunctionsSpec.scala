...
import wdl.ReadLikeFunctionsSpec.draft2Functions
...
class ReadLikeFunctionsSpec extends FlatSpec with Matchers {
  ...
  it should "correctly report a 2048 byte file, in bytes by default" in {
    val readLike = draft2Functions(Success(2048d))
...
object ReadLikeFunctionsSpec {
  implicit val draft2VersionSpecifics = Draft2VersionSpecifics
  def draft2Functions(sizeResult: Try[Double]) = WdlStandardLibraryFunctions.fromIoFunctionSet(TestReadLikeFunctions(sizeResult))
}

case class TestReadLikeFunctions(sizeResult: Try[Double]) extends IoFunctionSet {
  ...
  override def size(file: WomValue): Future[Double] = Future.fromTry(sizeResult)
