...
case class WomCoproductType(types: NonEmptyList[WomType]) extends WomType {
  ...
  def typeExists(tpe: WomType): Try[WomBooleanType.type] =
    types.exists(_.equals(tpe)) match {
      ...
      case _ => Failure(new WomExpressionException(s"Type equality assertion failed because $tpe was not found in the coproduct of ${callCachingName}"))
    }

  override def callCachingName: String =
    types.map(_.callCachingName).toList.mkString("Coproduct[",", ", "]")
  ...
  override def equalsType(rhs: WomType): Try[WomType] =
    rhs match {
      ...
      case _ => Failure(new WomExpressionException(s"Type equality could not be asserted because $rhs was found in the coproduct of $callCachingName"))
    }
