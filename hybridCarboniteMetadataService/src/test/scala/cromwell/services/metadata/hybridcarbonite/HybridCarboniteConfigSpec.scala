  ...
  it should "reject more subtly invalid values in metadata-freezing config" in {
    ...
    carboniteConfig match {
      case Left(e) =>
        e.size shouldBe 2
        e.toList.toSet shouldEqual Set(
          "Failed to parse Carboniter 'metadata-freezing' stanza (reason 1 of 2): 'max-interval' 1 second should be greater than or equal to finite 'initial-interval' 10 seconds.",
          "Failed to parse Carboniter 'metadata-freezing' stanza (reason 2 of 2): `multiplier` must be greater than 1."
        )
      case Right(_) => fail(s"Expected to fail but the config was parsed correctly!")
    }
  }
