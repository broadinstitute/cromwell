...
import cromwell.backend.google.pipelines.batch.PipelinesConversions._
import cromwell.backend.google.pipelines.batch.api._
...
case class BatchFactory(applicationName: String, authMode: GoogleAuthMode, endpointUrl: URL, location: String)(implicit gcsTransferConfiguration: GcsTransferConfiguration) extends PipelinesApiFactoryInterface
  with ContainerSetup
  ...
  override def build(initializer: HttpRequestInitializer): PipelinesApiRequestFactory = new PipelinesApiRequestFactory {
    val gcpBatchRequest: CloudLifeSciences = new CloudLifeSciences.Builder(
