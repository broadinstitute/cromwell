...
object SampleWdl {
  ...
  object ArrayIO extends SampleWdl {
    ...
    override val rawInputs = Map(
      "wf.find.root" -> tempDir.fullPath.toString,
      "wf.find.pattern" -> "*.out", // createCannedFile makes files that have .out extension
      "wf.files" -> Seq(firstFile.fullPath, secondFile.fullPath)
    )
  }
  ...
  case class PrepareScatterGatherWdl(salt: String = UUID.randomUUID().toString) extends SampleWdl {
    ...
    override lazy val rawInputs = Map("sc_test.do_prepare.input_file" -> createCannedFile("scatter",contents).fullPath,
  ...
  object FileClobber extends SampleWdl {
    ...
    override val rawInputs = Map(
      "two.x.in" -> firstFile.fullPath,
      "two.y.in" -> secondFile.fullPath
    )
  }
  ...
  object FilePassingWorkflow extends SampleWdl {
    ...
    override val rawInputs: WorkflowRawInputs = Map(
      "file_passing.f" -> createCannedFile("canned", fileContents).fullPath
    )
  }
  ...
  object WdlFunctionsAtWorkflowLevel extends SampleWdl {
    ...
    override val rawInputs = {
      Map(
        "w.array_file" -> createCannedFile("array.txt", CannedArray).fullPath,
        "w.map_file" -> createCannedFile("map.txt", CannedMap).fullPath
      )
    }
  }
  ...
  object ArrayOfArrays extends SampleWdl {
    ...
    override val rawInputs = Map(
      "wf.nested_file" ->
        ...
        Seq(
          WdlArray(WdlArrayType(WdlStringType), Seq(firstFile.fullPath, secondFile.fullPath).map(WdlString)),
          WdlArray(WdlArrayType(WdlStringType), Seq(thirdFile.fullPath, fourthFile.fullPath).map(WdlString))
        )
      )
    )
  }
  ...
  object CallCachingHashingWdl extends SampleWdl {
    ...
    override val rawInputs = Map(
      ...
      "w.t.d" -> WdlFile(cannedFile.fullPath)
    )
  }
