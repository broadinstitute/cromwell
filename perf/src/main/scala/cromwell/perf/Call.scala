...
case class ExecutionEvents(startTime: OffsetDateTime, description: String, endTime: OffsetDateTime)
...
case class Call(shardIndex: Int,
  ...
  val callCachingEventStates =
    List("CheckingCallCache", "FetchingCachedOutputsFromDatabase", "BackendIsCopyingCachedOutputs")
  val jobPreparationEventStates =
    List("Pending", "RequestingExecutionToken", "WaitingForValueStore", "PreparingJob", "CheckingJobStore")
  ...
  val timeInCheckingCallCacheState: Duration = {
    val eventsRelatedToCC = executionEvents.collect {
      case event if event.description.equalsIgnoreCase("CheckingCallCache") =>
  ...
  val timeInCallCachingState: Duration = {
    val eventsRelatedToCC =
      executionEvents.filter(event => callCachingEventStates.exists(state => state.equalsIgnoreCase(event.description)))
  ...
  val timeInJobPreparation: Duration = {
    val durationOfEventsInPreparationState = executionEvents.collect {
      case event if jobPreparationEventStates.exists(_.equalsIgnoreCase(event.description)) =>
        Duration.between(event.startTime, event.endTime)
    }

    if (durationOfEventsInPreparationState.nonEmpty) durationOfEventsInPreparationState.reduce(_ plus _)
    else Duration.ZERO
  }
  ...
  val timeForFetchingAndCopyingCacheHit: Duration = {
    val durationOfEventsInCacheCopyingState = executionEvents.collect {
      case event if cacheCopyingEventStates.exists(_.equalsIgnoreCase(event.description)) =>
        Duration.between(event.startTime, event.endTime)
    }

    if (durationOfEventsInCacheCopyingState.nonEmpty) durationOfEventsInCacheCopyingState.reduce(_ plus _)
    else Duration.ZERO
  }
}
