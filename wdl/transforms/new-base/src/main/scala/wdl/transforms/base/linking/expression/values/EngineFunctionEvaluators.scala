...
object EngineFunctionEvaluators {
  ...
  implicit val readLinesFunctionEvaluator: ValueEvaluator[ReadLines] = new ValueEvaluator[ReadLines] {
    override def evaluateValue(a: ReadLines,
      ...
      processValidatedSingleValue[WomSingleFile, WomArray](
  ...
  implicit val readMapFunctionEvaluator: ValueEvaluator[ReadMap] = new ValueEvaluator[ReadMap] {
    override def evaluateValue(a: ReadMap,
      ...
      processValidatedSingleValue[WomSingleFile, WomMap](
        a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { fileToRead =>
        val tryResult = for {
  ...
  implicit val readObjectFunctionEvaluator: ValueEvaluator[ReadObject] = new ValueEvaluator[ReadObject] {
    override def evaluateValue(a: ReadObject,
      ...
      processValidatedSingleValue[WomSingleFile, WomObject](
        a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { fileToRead =>
        val tryResult = for {
        ...
        val rightSize: ErrorOr[WomObject] = tryResult.toErrorOr flatMap {
          ...
          case other: Array[WomObject] =>
            s"Exactly 1 TSV object expected in input file (ie 2 lines: headers and data), but instead got an array of ${other.length} entries.".invalidNel
        }
  ...
  implicit val readObjectsFunctionEvaluator: ValueEvaluator[ReadObjects] = new ValueEvaluator[ReadObjects] {
    override def evaluateValue(a: ReadObjects,
      ...
      processValidatedSingleValue[WomSingleFile, WomArray](
        ...
        tryResult
          .map(EvaluatedValue(_, Seq.empty))
          .toErrorOr
          .contextualizeErrors(s"""read_objects("${fileToRead.value}")""")
      }
  }
  ...
  implicit val readJsonFunctionEvaluator: ValueEvaluator[ReadJson] = new ValueEvaluator[ReadJson] {
    override def evaluateValue(a: ReadJson,
      ...
      def convertJsonToWom(jsValue: JsValue): Try[WomValue] =
        jsValue match {
          case _: JsNumber =>
            WomIntegerType.coerceRawValue(jsValue).recoverWith { case _ => WomFloatType.coerceRawValue(jsValue) }
      ...
      def readJson(fileToRead: WomSingleFile): ErrorOr[EvaluatedValue[WomValue]] = {
        ...
        tryResult
          .map(EvaluatedValue(_, Seq.empty))
          .toErrorOr
          .contextualizeErrors(s"""read_json("${fileToRead.value}")""")
      }
  ...
  implicit val readIntFunctionEvaluator: ValueEvaluator[ReadInt] = new ValueEvaluator[ReadInt] {
    override def evaluateValue(a: ReadInt,
      ...
      processValidatedSingleValue[WomSingleFile, WomInteger](
        a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { fileToRead =>
        val tryResult = for {
        ...
        tryResult
          .map(EvaluatedValue(_, Seq.empty))
          .toErrorOr
          .contextualizeErrors(s"""read_int("${fileToRead.value}")""")
      }
  }
  ...
  implicit val readStringFunctionEvaluator: ValueEvaluator[ReadString] = new ValueEvaluator[ReadString] {
    override def evaluateValue(a: ReadString,
      ...
      processValidatedSingleValue[WomSingleFile, WomString](
        a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { fileToRead =>
        val tryResult = for {
        ...
        tryResult
          .map(EvaluatedValue(_, Seq.empty))
          .toErrorOr
          .contextualizeErrors(s"""read_string("${fileToRead.value}")""")
      }
  }
  ...
  implicit val readFloatFunctionEvaluator: ValueEvaluator[ReadFloat] = new ValueEvaluator[ReadFloat] {
    override def evaluateValue(a: ReadFloat,
      ...
      processValidatedSingleValue[WomSingleFile, WomFloat](
        a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { fileToRead =>
        val tryResult = for {
        ...
        tryResult
          .map(EvaluatedValue(_, Seq.empty))
          .toErrorOr
          .contextualizeErrors(s"""read_float("${fileToRead.value}")""")
      }
  }
  ...
  implicit val readBooleanFunctionEvaluator: ValueEvaluator[ReadBoolean] = new ValueEvaluator[ReadBoolean] {
    override def evaluateValue(a: ReadBoolean,
      ...
      processValidatedSingleValue[WomSingleFile, WomBoolean](
        a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { fileToRead =>
        val tryResult = for {
        ...
        tryResult
          .map(EvaluatedValue(_, Seq.empty))
          .toErrorOr
          .contextualizeErrors(s"""read_boolean("${fileToRead.value}")""")
      }
  }
  ...
  implicit val writeLinesFunctionEvaluator: ValueEvaluator[WriteLines] = new ValueEvaluator[WriteLines] {
    override def evaluateValue(a: WriteLines,
      ...
      processValidatedSingleValue[WomArray, WomSingleFile](
        a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { stringsToWrite =>
        val tryResult = for {
          serialized <- ValueEvaluation.serializeWomValue(functionName,
                                                          stringsToWrite,
                                                          defaultIfOptionalEmpty =
                                                            WomArray(WomArrayType(WomStringType), Seq.empty)
          )
          written <- writeContent(functionName, ioFunctionSet, serialized)
        } yield written

        tryResult
          .map(v => EvaluatedValue(v, Seq(CommandSetupSideEffectFile(v))))
          .toErrorOr
          .contextualizeErrors(s"""$functionName(...)""")
      ...
      processValidatedSingleValue[WomArray, WomSingleFile](
        ...
        val tryResult = for {
          serialized <- ValueEvaluation.serializeWomValue(functionName,
                                                          objectToWrite,
                                                          defaultIfOptionalEmpty =
                                                            WomArray(WomArrayType(WomStringType), List.empty[WomValue])
          )
          written <- writeContent(functionName, ioFunctionSet, serialized)
        } yield written
  ...
  implicit val writeMapFunctionEvaluator: ValueEvaluator[WriteMap] = new ValueEvaluator[WriteMap] {
    override def evaluateValue(a: WriteMap,
      ...
      processValidatedSingleValue[WomMap, WomSingleFile](
        a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { mapToWrite: WomMap =>
        val tryResult = for {
          serialized <- ValueEvaluation.serializeWomValue(functionName,
                                                          mapToWrite,
                                                          defaultIfOptionalEmpty = WomMap(Map.empty)
          )
          written <- writeContent(functionName, ioFunctionSet, serialized)
        } yield written
  ...
  implicit val writeObjectFunctionEvaluator: ValueEvaluator[WriteObject] = new ValueEvaluator[WriteObject] {
    override def evaluateValue(a: WriteObject,
      ...
      processValidatedSingleValue[WomObject, WomSingleFile](
        a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { objectToWrite: WomObject =>
        val tryResult = for {
          serialized <- ValueEvaluation.serializeWomValue(functionName,
                                                          objectToWrite,
                                                          defaultIfOptionalEmpty = WomObject(Map.empty)
          )
          written <- writeContent(functionName, ioFunctionSet, serialized)
        } yield written
    ...
    override def evaluateValue(a: WriteObjects,
      ...
      processValidatedSingleValue[WomArray, WomSingleFile](
        ...
        val tryResult = for {
          serialized <- ValueEvaluation.serializeWomValue(functionName,
                                                          objectToWrite,
                                                          defaultIfOptionalEmpty = WomArray(List(WomObject(Map.empty)))
          )
          written <- writeContent(functionName, ioFunctionSet, serialized)
        } yield written
  ...
  implicit val writeJsonFunctionEvaluator: ValueEvaluator[WriteJson] = new ValueEvaluator[WriteJson] {
    override def evaluateValue(a: WriteJson,
      ...
      def evaluateParam(womValue: WomValue): ErrorOr[EvaluatedValue[WomSingleFile]] =
        womValue match {
          case WomBoolean(_) | WomString(_) | WomInteger(_) | WomFloat(_) | WomPair(_, _) =>
            convertToSingleFile(womValue)
          ...
          case _ =>
            (s"The '$functionName' method expects one of 'Boolean', 'String', 'Integer', 'Float', 'Object', 'Pair[_, _]', " +
      ...
      evaluatedSingleFile map { case (result, previousSideEffectFiles) =>
        result.copy(sideEffectFiles = result.sideEffectFiles ++ previousSideEffectFiles)
      }
    }
  }
  ...
  implicit val rangeFunctionEvaluator: ValueEvaluator[Range] = new ValueEvaluator[Range] {
    override def evaluateValue(a: Range,
      ...
      processValidatedSingleValue[WomInteger, WomArray](
        a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { integer =>
        val array = WomArray(
  ...
  implicit val lengthFunctionEvaluator: ValueEvaluator[Length] = new ValueEvaluator[Length] {
    override def evaluateValue(a: Length,
      ...
      processValidatedSingleValue[WomArray, WomInteger](
        a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { a =>
  ...
  implicit val selectFirstFunctionEvaluator: ValueEvaluator[SelectFirst] = new ValueEvaluator[SelectFirst] {
    override def evaluateValue(a: SelectFirst,
      ...
      processValidatedSingleValue[WomArray, WomValue](
        a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { array =>
        val firstValue = array.value collectFirst { case WomOptionalValue(_, Some(yay)) =>
          yay
        }
        ...
        firstValue match {
          ...
          case None =>
            s"select_first was called with ${array.size} empty values. We needed at least one to be filled.".invalidNel
        }
  ...
  implicit val selectAllFunctionEvaluator: ValueEvaluator[SelectAll] = new ValueEvaluator[SelectAll] {
    override def evaluateValue(a: SelectAll,
      ...
      processValidatedSingleValue[WomArray, WomArray](
        ...
        val goodValues = array.value.collect { case WomOptionalValue.Flattened(Some(value)) =>
          value
        }
  ...
  implicit val definedFunctionEvaluator: ValueEvaluator[Defined] = new ValueEvaluator[Defined] {
    override def evaluateValue(a: Defined,
      ...
      processValidatedSingleValue[WomOptionalValue, WomBoolean](
        a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { optionalValue =>
  ...
  implicit val floorFunctionEvaluator: ValueEvaluator[Floor] = new ValueEvaluator[Floor] {
    override def evaluateValue(a: Floor,
      ...
      processValidatedSingleValue[WomFloat, WomInteger](
        a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { float =>
  ...
  implicit val sizeFunctionEvaluator: ValueEvaluator[Size] = new ValueEvaluator[Size] {
    override def evaluateValue(a: Size,
      ...
      def fileSize(womValue: ErrorOr[EvaluatedValue[_ <: WomValue]],
                   convertToOption: Option[ErrorOr[EvaluatedValue[_ <: WomValue]]]
      ): ErrorOr[EvaluatedValue[WomFloat]] = {
        val convertTo: ErrorOr[EvaluatedValue[_ <: WomValue]] =
          convertToOption.getOrElse(EvaluatedValue(WomString("B"), Seq.empty).validNel)
        for {
          value <- womValue
          evaluatedUnitValue <- convertTo
          convertToUnit <- toUnit(evaluatedUnitValue.value)
          fileSize <- optionalSafeFileSize(value.value)
        } yield EvaluatedValue(WomFloat(MemorySize(fileSize.toDouble, MemoryUnit.Bytes).to(convertToUnit).amount),
                               value.sideEffectFiles ++ evaluatedUnitValue.sideEffectFiles
        )
      }

      val evaluatedFileValidation: ErrorOr[EvaluatedValue[_ <: WomValue]] =
        a.file.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      fileSize(evaluatedFileValidation,
               a.unit map (_.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions))
      )
    }
  }
  ...
  implicit val basenameFunctionEvaluator: ValueEvaluator[Basename] = new ValueEvaluator[Basename] {
    override def evaluateValue(a: Basename,
      ...
      a.suffixToRemove match {
        case None =>
          processValidatedSingleValue[WomString, WomString](
            a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
          ) { str =>
            EvaluatedValue(WomString(simpleBasename(str)), Seq.empty).validNel
          }
        case Some(suffixToRemove) =>
          processTwoValidatedValues[WomString, WomString, WomString](
            a.param.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions),
            suffixToRemove.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
          ) { (name, suffix) =>
  ...
  implicit val zipFunctionEvaluator: ValueEvaluator[Zip] = new ValueEvaluator[Zip] {
    override def evaluateValue(a: Zip,
      ...
      processTwoValidatedValues[WomArray, WomArray, WomArray](
        a.arg1.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions),
        ...
        if (arr1.size == arr2.size) {
          ...
          EvaluatedValue(
            WomArray(WomArrayType(WomPairType(arr1.arrayType.memberType, arr2.arrayType.memberType)), pairs),
            Seq.empty
          ).validNel
        } else {
  ...
  implicit val prefixFunctionEvaluator: ValueEvaluator[Prefix] = new ValueEvaluator[Prefix] {
    override def evaluateValue(a: Prefix,
      ...
      processTwoValidatedValues[WomString, WomArray, WomArray](
        a.prefix.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions),
        a.array.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { (prefix, array) =>
        EvaluatedValue(WomArray(array.value.map(value => WomString(prefix.value + value.valueString))),
                       Seq.empty
        ).validNel
      }
  }
  ...
  implicit val subFunctionEvaluator: ValueEvaluator[Sub] = new ValueEvaluator[Sub] {
    override def evaluateValue(a: Sub,
      ...
      processThreeValidatedValues[WomString, WomString, WomString, WomString](
        ...
        a.replace.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) { (input, pattern, replace) =>
        ErrorOr(
          EvaluatedValue(WomString(pattern.valueString.r.replaceAllIn(input.valueString, replace.valueString)),
                         Seq.empty
          )
        )
      }
  ...
  def processValidatedSingleValue[A <: WomValue, B <: WomValue](
    arg: ErrorOr[EvaluatedValue[_]]
  )(f: A => ErrorOr[EvaluatedValue[B]])(implicit coercer: WomTypeCoercer[A]): ErrorOr[EvaluatedValue[B]] =
    arg flatMap {
      case EvaluatedValue(a: WomValue, previousSideEffectFiles) if a.coercionDefined[A] =>
        a.coerceToType[A] flatMap f.apply map { result =>
          result.copy(sideEffectFiles = result.sideEffectFiles ++ previousSideEffectFiles)
        }
      case other => s"Expected ${coercer.toDisplayString} argument but got ${other.value.womType.stableName}".invalidNel
    }

  def processTwoValidatedValues[A <: WomValue, B <: WomValue, R <: WomValue](
    arg1: ErrorOr[EvaluatedValue[_ <: WomValue]],
    arg2: ErrorOr[EvaluatedValue[_ <: WomValue]]
  )(
    f: (A, B) => ErrorOr[EvaluatedValue[R]]
  )(implicit coercerA: WomTypeCoercer[A], coercerB: WomTypeCoercer[B]): ErrorOr[EvaluatedValue[R]] =
    (arg1, arg2) flatMapN {
      case (EvaluatedValue(a: WomValue, previousSideEffectFilesA),
            EvaluatedValue(b: WomValue, previousSideEffectFilesB)
          ) if a.coercionDefined[A] && b.coercionDefined[B] =>
        (a.coerceToType[A], b.coerceToType[B]) flatMapN f.apply map { result =>
          result.copy(sideEffectFiles = result.sideEffectFiles ++ previousSideEffectFilesA ++ previousSideEffectFilesB)
        }
      case (otherA, otherB) =>
  ...
  )(f: (A, B, C) => ErrorOr[EvaluatedValue[R]])(implicit
    coercerA: WomTypeCoercer[A],
    coercerB: WomTypeCoercer[B],
    coercerC: WomTypeCoercer[C]
  ): ErrorOr[EvaluatedValue[R]] =
    (arg1, arg2, arg3) flatMapN {
      case (EvaluatedValue(a, previousSideEffectFilesA),
            EvaluatedValue(b, previousSideEffectFilesB),
            EvaluatedValue(c, previousSideEffectFilesC)
          ) if a.coercionDefined[A] && b.coercionDefined[B] && c.coercionDefined[C] =>
        (a.coerceToType[A], b.coerceToType[B], c.coerceToType[C]) flatMapN f.apply map { result =>
          result.copy(sideEffectFiles =
            result.sideEffectFiles ++ previousSideEffectFilesA ++ previousSideEffectFilesB ++ previousSideEffectFilesC
          )
        }
      case (otherA, otherB, otherC) =>
        s"Expected (${coercerA.toDisplayString}, ${coercerB.toDisplayString}, ${coercerB.toDisplayString}) argument but got (${otherA.value.womType.stableName}, ${otherB.value.womType.stableName}, ${otherC.value.womType.stableName})".invalidNel
    }
}
