...
class PathBuilderFactorySpec extends TestKitSuite with AnyFlatSpecLike with ScalaFutures with Matchers {
  ...
  it should "sort factories when instantiating path builders" in {
    ...
    PathBuilderFactory
      .instantiatePathBuilders(List(DefaultPathBuilderFactory, factory1, factory2), WorkflowOptions.empty)
      .map { pathBuilders =>
...
class MockPathBuilderFactory(globalConfig: Config, val instanceConfig: Config)
