...
case class WomCompositeType(typeMap: Map[String, WomType], structName: Option[String] = None)
    extends WomObjectTypeLike {
  ...
  override protected def coercion = {
    case composite: WomObjectLike if isCoerceableFrom(composite.womType) =>
      WomObject.withTypeUnsafe(composite.values, this)
    case map: WomMap if WomStringType.isCoerceableFrom(map.womType.keyType) =>
      WomObject.withTypeUnsafe(map.value.map { case (k, v) => k.valueString -> v }, this)
  ...
  override def typeSpecificIsCoerceableFrom(otherType: WomType): Boolean =
    otherType match {
      ...
      case compositeType: WomCompositeType
          if (compositeType.typeMap.size == typeMap.size) &&
            compositeType.typeMap.forall { case (key, typeValue) =>
              typeMap.get(key).exists(_.isCoerceableFrom(typeValue))
            } =>
        true
