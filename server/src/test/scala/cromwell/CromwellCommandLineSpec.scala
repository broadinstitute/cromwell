...
class CromwellCommandLineSpec extends AnyFlatSpec with CromwellTimeoutSpec with Matchers with BeforeAndAfter {
  ...
  it should "run single when supplying wdl and inputs" in {
    ...
    val optionsLast =
      parser.parse(Array("run", threeStep.wdl, "--inputs", threeStep.inputs), CommandLineArguments()).get
    ...
    val optionsFirst =
      parser.parse(Array("run", "--inputs", threeStep.inputs, threeStep.wdl), CommandLineArguments()).get
  ...
  it should "run single when supplying wdl and inputs and options" in {
    val optionsLast = parser
      .parse(Array("run", "3step.wdl", "--inputs", "3step.inputs", "--options", "3step.options"),
    ...
    val optionsFirst = parser
      .parse(Array("run", "--inputs", "3step.inputs", "--options", "3step.options", "3step.wdl"),
             CommandLineArguments()
      )
      .get
    optionsFirst.command shouldBe Some(Run)
  ...
  it should "fail if input files do not exist" in {
    val parsedArgs = parser
      .parse(Array("run",
                   "xyzshouldnotexist.wdl",
                   "--inputs",
                   "xyzshouldnotexist.inputs",
                   "--options",
                   "xyzshouldnotexist.options"
             ),
             CommandLineArguments()
      )
      .get
  ...
  it should "fail if metadata output path is not writeable" in {
    val threeStep = WdlAndInputs(ThreeStep)
    val parsedArgs = parser
      .parse(Array("run", threeStep.wdl, "--inputs", threeStep.inputs, "--metadata-output", threeStep.metadata),
             CommandLineArguments()
      )
      .get
  ...
  it should "run if the imports path is a .zip file" in {
    ...
    val parsedArgs =
      parser.parse(Array("run", filePassing.pathAsString, "--imports", zippedPath), CommandLineArguments()).get
    val ccl = Try(CromwellEntryPoint.validateRunArguments(parsedArgs))
