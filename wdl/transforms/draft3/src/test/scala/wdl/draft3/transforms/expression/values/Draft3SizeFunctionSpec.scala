...
import wom.expression.{EmptyIoFunctionSet, ExpressionEvaluationOptions, IoFunctionSet}
import wom.types._
import wom.values._
...
class Draft3SizeFunctionSpec extends FlatSpec with Matchers {
  ...
  it should "correctly report a 2048 byte file, in bytes by default" in {
    validate(Size(PrimitiveLiteralExpressionElement(WomSingleFile("blah")), None).evaluateValue(Map.empty, testFunctions(Success(2048l)), ExpressionEvaluationOptions.default)) {
      res => assert(res.value == WomFloat(2048d))
    }
  }

  it should "correctly report a 2048 byte file, in bytes" in {
    validate(Size(PrimitiveLiteralExpressionElement(WomSingleFile("blah")), Some(PrimitiveLiteralExpressionElement(WomString("B")))).evaluateValue(Map.empty, testFunctions(Success(2048l)), ExpressionEvaluationOptions.default)) {
      res => assert(res.value == WomFloat(2048d))
    }
  }

  it should "correctly report a 2048 byte file, in KB" in {
    validate(Size(PrimitiveLiteralExpressionElement(WomSingleFile("blah")), Some(PrimitiveLiteralExpressionElement(WomString("KB")))).evaluateValue(Map.empty, testFunctions(Success(2048l)), ExpressionEvaluationOptions.default)) {
      res => assert(res.value == WomFloat(2.0d))
    }
  }

  it should "correctly report a 2048 byte file, in KiB" in {
    validate(Size(PrimitiveLiteralExpressionElement(WomSingleFile("blah")), Some(PrimitiveLiteralExpressionElement(WomString("KiB")))).evaluateValue(Map.empty, testFunctions(Success(2048l)), ExpressionEvaluationOptions.default)) {
  ...
  it should "correctly report the size of a supplied, optional, 2048 byte file" in {
    ...
    validate(Size(IdentifierLookup("x"), None).evaluateValue(Map("x" -> value), testFunctions(Success(2048l)), ExpressionEvaluationOptions.default)) {
      res => assert(res.value == WomFloat(2048d))
    }
  }
  ...
  it should "correctly report the size of a supplied, optional, 2048 byte file, in MB" in {
    ...
    validate(Size(IdentifierLookup("x"), Some(PrimitiveLiteralExpressionElement(WomString("MB")))).evaluateValue(Map("x" -> value), testFunctions(Success(2048l)), ExpressionEvaluationOptions.default)) {
  ...
  it should "correctly report that an unsupplied optional file is empty" in {
    ...
    validate(Size(IdentifierLookup("x"), None).evaluateValue(Map("x" -> value), testFunctions(Failure(new Exception("Bad call to size on an empty optional"))), ExpressionEvaluationOptions.default)) {
  ...
  it should "correctly report that an unsupplied optional file is empty, even in MB" in {
    ...
    validate(Size(IdentifierLookup("x"), Some(PrimitiveLiteralExpressionElement(WomString("MB")))).evaluateValue(Map("x" -> value), testFunctions(Failure(new Exception("Bad call to size on an empty optional"))), ExpressionEvaluationOptions.default)) {
      res => assert(res.value == WomFloat(0d))
    }
  }
  ...
  it should "correctly report the size of an array of files, in GiB" in {
    ...
    validate(Size(IdentifierLookup("x"), Some(PrimitiveLiteralExpressionElement(WomString("GiB")))).evaluateValue(Map("x" -> value), testFunctions(Success(2048l)), ExpressionEvaluationOptions.default)) {
  ...
  it should "refuse to report file sizes for Ints" in {
    ...
    val oops = Size(IdentifierLookup("x"), Some(PrimitiveLiteralExpressionElement(WomString("MB")))).evaluateValue(Map("x" -> value), testFunctions(Failure(new Exception("Bad call to size on an Int"))), ExpressionEvaluationOptions.default)
    oops match {
  ...
  it should "pass on underlying size reading errors" in {
    ...
    val oops = Size(IdentifierLookup("x"), Some(PrimitiveLiteralExpressionElement(WomString("MB")))).evaluateValue(Map("x" -> value), testFunctions(Failure(new Exception("'size' inner exception, expect me to be passed on"))), ExpressionEvaluationOptions.default)
    oops match {
      case Valid(x) => fail(s"Expected an Int? to not have a file length but instead got ${x.value.toWomString}")
