...
sealed trait WomArrayType extends WomType {
  ...
  override protected def coercion: PartialFunction[Any, WomValue] = {
    ...
    case womArray: WomArray if (allowEmpty || womArray.nonEmpty) && womArray.womType.memberType == memberType =>
      WomArray(this, womArray.value)
    case womArray: WomArray if (allowEmpty || womArray.nonEmpty) && womArray.womType.memberType == WomAnyType =>
      coerceIterable(womArray.value)
    case womArray: WomArray
        if (allowEmpty || womArray.nonEmpty) && womArray.womType.memberType.isInstanceOf[WomArrayType] && memberType
          .isInstanceOf[WomArrayType] =>
      TryUtil.sequence(womArray.value.map(memberType.coerceRawValue)) match {
        case Success(values) => WomArray(this, values)
        case Failure(ex) => throw ex
      }
    case womArray: WomArray
        if (allowEmpty || womArray.nonEmpty) && memberType.isCoerceableFrom(womArray.womType.memberType) =>
