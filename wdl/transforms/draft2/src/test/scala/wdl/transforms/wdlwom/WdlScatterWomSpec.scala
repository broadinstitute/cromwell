...
class WdlScatterWomSpec extends AnyFlatSpec with CromwellTimeoutSpec with Matchers {
  ...
  it should "convert gathered scatter outputs to workflow outputs" in {
    ...
    scatterTestGraph match {
      ...
      case Invalid(errors) =>
        fail(s"Unable to build wom version of scatter foo from WDL: ${errors.toList.mkString("\n", "\n", "\n")}")
    }
    ...
    def validateGraph(workflowGraph: Graph) = {
      ...
      def validateOuterGraph: OuterGraphValidations = {
        val scatterNode =
          workflowGraph.nodes.firstByType[ScatterNode].getOrElse(fail("Resulting graph did not contain a ScatterNode"))

        val xs_inputNode = workflowGraph.nodes
          .collectFirst {
            case gin: GraphInputNode if gin.localName == "xs" => gin
          }
          .getOrElse(fail("Resulting graph did not contain the 'xs' GraphInputNode"))

        val scatterExpressionNode = workflowGraph.nodes
          .collectFirst {
        ...
        val foo_out_output = workflowGraph.nodes
          .collectFirst {
      ...
      def validateInnerGraph(validatedOuterGraph: OuterGraphValidations): InnerGraphValidations = {
        val x_scatterCollectionInput = validatedOuterGraph.scatterNode.innerGraph.nodes
          .collectFirst {
            case gin: GraphInputNode if gin.localName == "x" => gin
          }
          .getOrElse(fail("Scatter inner graph did not contain a GraphInputNode 'x'"))

        val foo_callNode = validatedOuterGraph.scatterNode.innerGraph.nodes
          .collectFirst {
        ...
        val foo_out_innerOutput = validatedOuterGraph.scatterNode.innerGraph.nodes
          .collectFirst {
            case gon: GraphOutputNode if gon.localName == "foo.out" => gon
          }
          .getOrElse(fail("Scatter inner graph did not contain a GraphOutputNode 'foo.out'"))

        val foo_out_i_expressionNode = validatedOuterGraph.scatterNode.innerGraph.nodes
          .collectFirst {
        ...
        validatedOuterGraph.scatterNode.innerGraph.nodes should be(
          Set(x_scatterCollectionInput, foo_callNode, foo_out_innerOutput, foo_out_i_expressionNode)
        )
        InnerGraphValidations(x_scatterCollectionInput, foo_out_innerOutput)
      }

      def validateConnections(validatedOuterGraph: OuterGraphValidations,
                              validatedInnerGraph: InnerGraphValidations
      ) = {
        // The scatter collection links to its predecessor
        validatedOuterGraph.scatterNode.scatterCollectionExpressionNodes.head.inputPorts
          .map(_.upstream.graphNode) should be(Set(validatedOuterGraph.xs_inputNode))

        // The ScatterNode's "scatter variable mapping" links to the innerGraph's scatter variable input Node:
        validatedOuterGraph.scatterNode.scatterVariableInnerGraphInputNodes.head eq validatedInnerGraph.x_scatterCollectionInput should be(
          true
        )
        ...
        validatedOuterGraph.scatterNode.outputMapping.toList match {
          ...
          case other =>
            fail(
              "Expected exactly one output to be gathered in this scatter but got:" + other.mkString("\n", "\n", "\n")
            )
        }
      }
  ...
  it should "convert inputs into inner graph inputs" in {
    ...
    def validateGraph(workflowGraph: Graph) = {
      ...
      // Find that scatter:
      workflowGraph.nodes
        .collectFirst { case s: ScatterNode =>
          s
        }
  ...
  it should "convert unsatisfied call inputs in scatters into outer graph inputs" in {
    ...
    def validateGraph(workflowGraph: Graph) = {
      ...
      // Find that scatter:
      val scatterNode = workflowGraph.nodes
        .collectFirst { case s: ScatterNode =>
          s
        }
        .getOrElse(fail("Resulting graph did not contain a ScatterNode"))

      val scatterInnerInputs: Set[ExternalGraphInputNode] =
        scatterNode.innerGraph.nodes.filterByType[ExternalGraphInputNode]
      scatterInnerInputs map { _.identifier.fullyQualifiedName.value } should be(Set("scatter_test.foo.j"))
      val scatterInnerItemInput: Set[OuterGraphInputNode] =
        scatterNode.innerGraph.nodes.filterByType[OuterGraphInputNode]
      scatterInnerItemInput map { _.localName } should be(Set("s"))

      // Find the outputs:
      val outputNodes = workflowGraph.nodes.collect { case output: GraphOutputNode =>
        output
      }
      outputNodes map { on => (on.localName, on.womType) } should be(
        Set(("foo.int_out", WomArrayType(WomIntegerType)), ("foo.str_out", WomArrayType(WomStringType)))
      )
