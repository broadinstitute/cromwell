...
import wom.callable.Callable.{InputDefinition, OptionalInputDefinition, OverridableInputDefinitionWithDefault}
...
object InputPointerToWomValue extends Poly1 {
  ...
  def withInitializedValue(ioFunctionSet: IoFunctionSet, womValue: WomValue)(
    block: WomValue => IOChecked[WomValue]
  ): IOChecked[WomValue] =
    for {
  ...
  implicit def fromWomValue: Case.Aux[WomValue, ToWomValueFn] = at[WomValue] {
    womValue =>
      (_: Map[String, WomValue], ioFunctions: IoFunctionSet, _: OutputPortLookup, inputDefinition: InputDefinition) =>
  ...
  implicit def fromOutputPort: Case.Aux[OutputPort, ToWomValueFn] = at[OutputPort] {
    port => (knownValues: Map[String, WomValue],
             ioFunctions: IoFunctionSet,
             outputPortLookup: OutputPortLookup,
  ...
  implicit def fromWomExpression: Case.Aux[WomExpression, ToWomValueFn] = at[WomExpression] {
    womExpression => (knownValues: Map[String, WomValue],
                      ioFunctions: IoFunctionSet,
                      _: OutputPortLookup,
                      inputDefinition: InputDefinition
    ) =>
  ...
  def evaluate(womExpression: WomExpression,
