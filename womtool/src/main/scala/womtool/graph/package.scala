...
package object graph {
  ...
  private[graph] implicit class GraphNodeGraphics(val graphNode: GraphNode) extends AnyVal {
    ...
    def graphName: String = dotSafe(graphNode match {
      ...
      case gin: OptionalGraphInputNodeWithDefault => s"${gin.womType.callCachingName} ${gin.localName} = ..."
      case gin: GraphInputNode => s"${gin.womType.callCachingName} ${gin.localName}"
      case gon: GraphOutputNode => s"${gon.womType.callCachingName} ${gon.localName}"
      case expr: ExpressionNode =>
        val inputNames = expr.womExpression.inputs.mkString(", ")
        s"${expr.womType.callCachingName} ${expr.localName} = f($inputNames)"
      case other =>
  ...
  private[graph] implicit class GraphNodePortGraphics(val graphNodePort: GraphNodePort) extends AnyVal {
    ...
    def graphName: String = dotSafe(graphNodePort.womType.callCachingName + " " + graphNodePort.name)
