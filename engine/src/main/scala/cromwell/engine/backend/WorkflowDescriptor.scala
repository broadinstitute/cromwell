...
case class WorkflowDescriptor(id: WorkflowId,
  ...
  def toFileSystemPath(str: String): Path = str.toPath(fileSystems)

  lazy val fileHasher: FileHasher = { wdlFile: WdlFile =>
    try {
      SymbolHash(toFileSystemPath(wdlFile.value).hash)
    } catch {
      case e: Exception =>
  ...
  lazy val workflowRootPath = toFileSystemPath(wfContext.root)
  def workflowRootPathWithBaseRoot(rootPath: String): Path = {
    toFileSystemPath(backend.buildWorkflowRootPath(rootPath, name, id))
  }
