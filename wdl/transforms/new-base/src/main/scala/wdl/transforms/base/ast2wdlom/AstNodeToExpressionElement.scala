...
object AstNodeToExpressionElement {
  ...
  protected def convert(
    ...
    ast match {
      ...
      case a: GenericAst if a.getName == "StructLiteral" => //TODO, StructLiterals are a biscayne feature, but this ast parsing is 'core'. Probably fine, but should test what happens when a 1.0 wdl uses a struct literal. Theoretically, the 1.0 parser can't get us to this point.
        (for {
          objectKvs <- a.getAttributeAsVector[KvPair]("map")
          asMap = objectKvs.map(kv => kv.key -> kv.value).toMap
