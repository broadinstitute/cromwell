...
object TestFormulas extends StrictLogging {
  ...
  private def runSuccessfulWorkflow(workflow: Workflow): Test[SubmittedWorkflow] =
    runWorkflowUntilTerminalStatus(workflow, Succeeded)
  private def runFailingWorkflow(workflow: Workflow): Test[SubmittedWorkflow] =
    runWorkflowUntilTerminalStatus(workflow, Failed)
  ...
  def runSuccessfulWorkflowAndVerifyTimeAndOutputs(workflowDefinition: Workflow): Test[SubmitResponse] = for {
    ...
    _ <- fetchAndValidateOutputs(submittedWorkflow,
                                 workflowDefinition,
                                 "ROOT NOT SUPPORTED IN TIMING/OUTPUT ONLY TESTS"
    )
  ...
  def runSuccessfulWorkflowAndVerifyMetadata(
    ...
    _ <- fetchAndValidateJobManagerStyleMetadata(submittedWorkflow,
                                                 workflowDefinition,
                                                 prefetchedOriginalNonSubWorkflowMetadata = Option(metadata.value)
    )
    notArchivedFlatMetadata = metadata.asFlat
    workflowRoot = notArchivedFlatMetadata.value.get("workflowRoot").collectFirst { case JsString(r) =>
      r
    } getOrElse "No Workflow Root"
  ...
  def runWorkflowTwiceExpectingNoCaching(
    workflowDefinition: Workflow
  )(implicit cromwellTracker: Option[CromwellTracker]): Test[SubmitResponse] =
    for {
      _ <- checkDescription(workflowDefinition, validityExpectation = Option(true))
      ...
      _ <- fetchAndValidateJobManagerStyleMetadata(testWf,
                                                   workflowDefinition,
                                                   prefetchedOriginalNonSubWorkflowMetadata = None
      )
      _ = cromwellTracker.track(metadata)
  ...
  def runFailingWorkflowTwiceExpectingNoCaching(
    workflowDefinition: Workflow
  )(implicit cromwellTracker: Option[CromwellTracker]): Test[SubmitResponse] =
    for {
      _ <- checkDescription(workflowDefinition, validityExpectation = None)
  ...
  def scheduledAbort(workflowDefinition: Workflow, callMarker: CallMarker, restart: Boolean)(implicit
    cromwellTracker: Option[CromwellTracker]
  ): Test[SubmitResponse] = {
  ...
  def papiUpgrade(workflowDefinition: Workflow, callMarker: CallMarker)(implicit
    cromwellTracker: Option[CromwellTracker]
  ): Test[SubmitResponse] =
    CentaurConfig.runMode match {
      case ManagedCromwellServer(_, postRestart, withRestart) if withRestart =>
        for {
          ...
          second <- runSuccessfulWorkflow(
            workflowDefinition.secondRun
          ) // Same WDL and config but a "backend" runtime option targeting PAPI v2.
          ...
          _ <- fetchAndValidateJobManagerStyleMetadata(second,
                                                       workflowDefinition,
                                                       prefetchedOriginalNonSubWorkflowMetadata = None
          )
          _ = cromwellTracker.track(metadata)
