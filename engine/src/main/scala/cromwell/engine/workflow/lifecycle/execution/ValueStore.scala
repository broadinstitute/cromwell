...
case class ValueStore(store: Table[OutputPort, ExecutionIndex, WdlValue]) {
  ...
  final def add(values: Map[ValueKey, WdlValue]): ValueStore = {
    this.copy(store = store.addAll(values.map({ case (key, value) => (key.port, key.index, value) })))
  }

  final def get(outputKey: ValueKey): Option[WdlValue] = store.getValue(outputKey.port, outputKey.index)

  final def get(outputPort: OutputPort, index: ExecutionIndex): Option[WdlValue] = store.getValue(outputPort, index)

  /**
    * Collect all shards values for a node
    */
  final def collectShards(collector: ScatterCollectorKey): ErrorOr[Map[ValueKey, WdlArray]] = {
    lazy val outputValues: ErrorOr[Map[ValueKey, WdlArray]] =
    /*
    ...
    */
      collector.scatterGatherPorts.toList
        .traverse[ErrorOr, (ValueKey, WdlArray)]({ scatterGatherPort =>
        ...
        store.rowOptional(sourcePort) match {
          case Some(shards) =>
            ...
            if (collectedValue.size == collector.scatterWidth) {
              ...
              (ValueKey(scatterGatherPort, None) -> WdlArray(scatterGatherPort.womType, collectedValue)).validNel
            } else {
  ...
  final def collectConditional(collector: ConditionalCollectorKey, isInBypassed: Boolean): ErrorOr[Map[ValueKey, WdlOptionalValue]] = {
    ...
    store.getValue(sourcePort, collector.index) match {
      case Some(wdlValue: WdlOptionalValue) if isInBypassed && wdlValue.value.isEmpty => Map(ValueKey(conditionalPort, collector.index) -> wdlValue).validNel
      case Some(wdlValue) if !isInBypassed  => Map(ValueKey(conditionalPort, collector.index) -> WdlOptionalValue(wdlValue)).validNel
