...
class WomValueBuilderSpec extends AnyFlatSpec with CromwellTimeoutSpec with Matchers {
  ...
  val simpletonConversions = List(
    ...
    SimpletonConversion(
      ...
      List(WomValueSimpleton("bar[0]", WomInteger(1)), WomValueSimpleton("bar[1]", WomInteger(2)))
    ),
    ...
    SimpletonConversion(
      ...
      WomArray(
        ...
        List(WomArray(WomArrayType(WomIntegerType), List(WomInteger(0), WomInteger(1))),
             WomArray(WomArrayType(WomIntegerType), List(WomInteger(2), WomInteger(3)))
        )
      ),
      List(
        WomValueSimpleton("baz[0][0]", WomInteger(0)),
        WomValueSimpleton("baz[0][1]", WomInteger(1)),
        WomValueSimpleton("baz[1][0]", WomInteger(2)),
        WomValueSimpleton("baz[1][1]", WomInteger(3))
      )
    ),
    SimpletonConversion(
      "map",
      WomMap(WomMapType(WomStringType, WomStringType),
             Map(WomString("foo") -> WomString("foo"), WomString("bar") -> WomString("bar"))
      ),
    ...
    SimpletonConversion(
      "mapOfMaps",
      WomMap(
        WomMapType(WomStringType, WomMapType(WomStringType, WomStringType)),
        Map(
          WomString("foo") -> WomMap(WomMapType(WomStringType, WomStringType),
                                     Map(WomString("foo2") -> WomString("foo"))
          ),
          WomString("bar") -> WomMap(WomMapType(WomStringType, WomStringType),
                                     Map(WomString("bar2") -> WomString("bar"))
          )
        )
      ),
      List(WomValueSimpleton("mapOfMaps:foo:foo2", WomString("foo")),
           WomValueSimpleton("mapOfMaps:bar:bar2", WomString("bar"))
      )
    ),
    SimpletonConversion(
      "simplePair1",
      WomPair(WomInteger(1), WomString("hello")),
      List(WomValueSimpleton("simplePair1:left", WomInteger(1)),
           WomValueSimpleton("simplePair1:right", WomString("hello"))
      )
    ),
    SimpletonConversion(
      "simplePair2",
      WomPair(WomString("left"), WomInteger(5)),
      List(WomValueSimpleton("simplePair2:left", WomString("left")),
           WomValueSimpleton("simplePair2:right", WomInteger(5))
      )
    ),
    SimpletonConversion(
      "pairOfPairs",
      WomPair(WomPair(WomInteger(1), WomString("one")), WomPair(WomString("two"), WomInteger(2))),
      List(
    ...
    SimpletonConversion(
      ...
      WomPair(
        ...
        WomMap(WomMapType(WomStringType, WomIntegerType),
               Map(WomString("left") -> WomInteger(100), WomString("right") -> WomInteger(200))
        )
      ),
      List(
    ...
    SimpletonConversion(
      "mapOfArrays",
      WomMap(
        WomMapType(WomStringType, WomArrayType(WomIntegerType)),
        Map(
      ...
      List(
        WomValueSimpleton("mapOfArrays:foo[0]", WomInteger(0)),
        WomValueSimpleton("mapOfArrays:foo[1]", WomInteger(1)),
        WomValueSimpleton("mapOfArrays:bar[0]", WomInteger(2)),
        WomValueSimpleton("mapOfArrays:bar[1]", WomInteger(3))
      )
    ),
    SimpletonConversion(
      "escapology",
      WomMap(
        ...
        Map(WomString("foo[1]") -> WomString("foo"),
            WomString("bar[[") -> WomString("bar"),
            WomString("baz:qux") -> WomString("baz:qux")
        )
      ),
      List(
        WomValueSimpleton("escapology:foo\\[1\\]", WomString("foo")),
    ...
    SimpletonConversion(
      ...
      List(
        WomValueSimpleton("flat_object:a", WomString("aardvark")),
    ...
    SimpletonConversion(
          ...
          List(
            ...
            WomMaybePopulatedFile(
              ...
              List(
                ...
                WomMaybeListedDirectory(Option("innerDirectoryValueName"),
                                        Option(List(WomSingleFile("innerDirectorySingleFile")))
                ),
      ...
      List(
        ...
        WomValueSimpleton("directory:listing[2]:secondaryFiles[1]:listing[0]",
                          WomSingleFile("innerDirectorySingleFile")
        ),
  ...
  it should "round trip everything together with no losses" in {
    ...
    val wdlValues = (simpletonConversions map { case SimpletonConversion(name, womValue, _) =>
      WomMocks.mockOutputPort(name, womValue.womType) -> womValue
    }).toMap
  ...
  it should "decompose then reconstruct a map in an object into a coerceable value" in {
    ...
    val aMap = WomMap(
      WomMapType(WomStringType, WomArrayType(WomStringType)),
      Map(
  ...
  private def assertSimpletonsEqual(expectedSimpletons: Iterable[WomValueSimpleton],
                                    actualSimpletons: Iterable[WomValueSimpleton]
  ): Unit = {
