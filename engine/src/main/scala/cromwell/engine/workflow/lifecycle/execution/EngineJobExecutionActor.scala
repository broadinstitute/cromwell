...
class EngineJobExecutionActor(jobKey: BackendJobDescriptorKey,
  ...
  def initializeJobHashing(jobDescriptor: BackendJobDescriptor, activity: CallCachingActivity) = {
    context.actorOf(EngineJobHashingActor.props(jobDescriptor, initializationData, factory.backendHashingMethods, backendName, activity), s"ejha_for_$jobDescriptor")
  }
