...
class CromwellFileSystems(globalConfig: Config) {
  ...
  private[filesystem] val factoryBuilders: Map[String, (Constructor[_], Option[AnyRef])] =
    if (globalConfig.hasPath("filesystems")) {
      ...
      val constructorMap = configMap
        .traverse[ErrorOr, (String, (Constructor[_], Option[AnyRef]))] {
  ...
  private def processFileSystem(key: String,
                                fsConfig: ConfigObject
  ): ErrorOr[(String, (Constructor[_], Option[AnyRef]))] = {
    // This is the (optional) singleton instance shared by all factory instances
    val singletonInstance: Checked[Option[AnyRef]] = fsConfig.toConfig
      .getAs[Config]("global")
  ...
  private def instantiateSingletonConfig(filesystem: String, config: Config): Checked[AnyRef] =
    for {
      ...
      cast <- instance
        .cast[AnyRef]
        .toChecked(s"The filesystem global configuration class for $filesystem is not a Java Object")
    } yield cast

  // Create a constructor from a configuration object
  private def createConstructor(key: String, config: Config, parameterTypes: List[Class[_]]): Checked[Constructor[_]] =
    for {
  ...
  private def createConstructor(filesystem: String,
                                className: String,
                                parameterTypes: List[Class[_]]
  ): Checked[Constructor[_]] = Try(
    ...
    case e: NoSuchMethodException =>
      Failure(
        new RuntimeException(
          s"Class $className for filesystem $filesystem does not have the required constructor signature: (${parameterTypes.map(_.getCanonicalName).mkString(", ")})",
          e
        )
      )
  }.toChecked
  ...
  private def instantiate(name: String,
                          constructor: Constructor[_],
                          instanceConfig: Config,
                          global: Option[AnyRef]
  ): Checked[PathBuilderFactory] =
    for {
      ...
      cast <- instance
        .cast[PathBuilderFactory]
        .toChecked(s"The filesystem class for $name is not an instance of PathBuilderFactory")
    } yield cast
  ...
  private def getConstructor(fileSystemName: String): Checked[(Constructor[_], Option[AnyRef])] = factoryBuilders
    .get(fileSystemName)
    .toChecked(
      s"Cannot find a filesystem with name $fileSystemName in the configuration. Available filesystems: ${factoryBuilders.keySet
          .mkString(", ")}"
    )
