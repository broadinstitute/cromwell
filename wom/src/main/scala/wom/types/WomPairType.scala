...
case class WomPairType(leftType: WomType, rightType: WomType) extends WomType {
  ...
  def coercePair(m: Map[String, JsValue], womPairType: WomPairType): WomPair = {
    ...
    if (failures.isEmpty) {
      ...
      throw new UnsupportedOperationException(s"Failed to coerce one or more values for creating a ${womPairType.callCachingName}:\n${failures.toList.mkString("\n")}")
    }
  }

  override def callCachingName: String = s"Pair[${leftType.callCachingName}, ${rightType.callCachingName}]"
}
