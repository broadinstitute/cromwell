...
//      runLobAction(action)
        ...
        val action = DBIO.sequence(metadataEntries.grouped(insertBatchSize).map(dataAccess.metadataEntryIdsAutoInc ++= _))
...
//        insertMetadata
//      }
//      runLobAction(DBIO.sequence(insertActions)).void
    }
  }
