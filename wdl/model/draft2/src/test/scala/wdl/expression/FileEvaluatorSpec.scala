...
class FileEvaluatorSpec extends AnyFlatSpec with CromwellTimeoutSpec with Matchers {
  val expressions = Table(
    ...
    ("""read_int("/etc/file1") + read_string("/bin/file2")""",
     Seq(WomSingleFile("/etc/file1"), WomSingleFile("/bin/file2")),
     WomStringType
    ),
    ("""read_int("/etc/file1") + read_string("/bin/file2") + read_string("/bin/file3") + read_string("/bin/file4") + read_string("/bin/file5")""",
     Seq(
    ...
    (""" ["a", "b", "c"] """,
     Seq(WomSingleFile("a"), WomSingleFile("b"), WomSingleFile("c")),
    ...
    (""" [read_string("x"), read_string("y")] """,
     Seq(WomSingleFile("x"), WomSingleFile("y")),
     WomArrayType(WomStringType)
    ),
    (""" [fileNameAsStringInput, "${fileNameAsStringInput}.bai"] """,
     Seq(WomSingleFile("sommat.bam"), WomSingleFile("sommat.bam.bai")),
     WomArrayType(WomSingleFileType)
    ),
    (""" [ fileNameAsStringInput, mapToFileName["Chris"] ] """,
     Seq(WomSingleFile("sommat.bam"), WomSingleFile("sommatStupid.bam")),
     WomArrayType(WomSingleFileType)
    ),
    (""" {read_int("a"): read_string("x"), 4: read_string("y")} """,
     Seq(WomSingleFile("a"), WomSingleFile("x"), WomSingleFile("y")),
     WomMapType(WomIntegerType, WomStringType)
    ),
    ...
    (""" if read_int("i.txt") == 10 then "a.txt" else "b.txt" """,
     Seq(WomSingleFile("a.txt"), WomSingleFile("b.txt"), WomSingleFile("i.txt")),
     WomSingleFileType
    ),
  ...
  forAll(expressions) { (expression, files, anticipatedType) =>
    it should s"evaluate $expression (coerced to: $anticipatedType) => $files" in {
      WdlExpression
        ...
        .evaluateFiles(lookupFunction, PureStandardLibraryFunctions, anticipatedType)
        .get
        .toSet should be(files.toSet)
    }
  }
}
