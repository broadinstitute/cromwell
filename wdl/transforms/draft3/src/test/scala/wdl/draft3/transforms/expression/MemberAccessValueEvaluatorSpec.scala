...
class MemberAccessValueEvaluatorSpec extends FlatSpec with Matchers{
  ...
  it should "find the left and right hand sides of a pair expression" in {
    ...
    leftExpression.evaluateValue(Map.empty, NoIoFunctionSet, ExpressionEvaluationOptions.default) shouldBeValid EvaluatedValue(womFive, Seq.empty)

    val rightExpression: ExpressionElement = ExpressionMemberAccess(pair, NonEmptyList("right", List.empty))
    rightExpression.evaluateValue(Map.empty, NoIoFunctionSet, ExpressionEvaluationOptions.default) shouldBeValid EvaluatedValue(womSix, Seq.empty)
  }
  ...
  it should "find the appropriate value in a deeply nested Pair" in {
    val nestedPairLookup: ExpressionElement = ExpressionMemberAccess(
     ...
     nestedPairLookup.evaluateValue(Map.empty, NoIoFunctionSet, ExpressionEvaluationOptions.default) shouldBeValid EvaluatedValue(WomPair(womFive, womSix), Seq.empty)
  }
  ...
  it should "evaluate a nested member access on a call output" in {
    ...
    callOutputLookup.evaluateValue(inputs, NoIoFunctionSet, ExpressionEvaluationOptions.default) shouldBeValid EvaluatedValue(WomPair(womFive, womSix), Seq.empty)
  }
  ...
  it should "evaluate a nested member access on an object" in {
    ...
    objectLookup.evaluateValue(inputs, NoIoFunctionSet, ExpressionEvaluationOptions.default) shouldBeValid EvaluatedValue(WomPair(womFive, womSix), Seq.empty)
  }
  ...
  it should "evaluate an identifier lookup" in {
    ...
    identifierLookup.evaluateValue(inputs, NoIoFunctionSet, ExpressionEvaluationOptions.default) shouldBeValid EvaluatedValue(WomString("foo"), Seq.empty)
  }
