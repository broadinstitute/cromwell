...
case class Metadata(id: String,
  ...
  val totalJobsPerRootWf: Int =
    sumElementsInOptionSeq(calls.map(taskMap => taskMap.map(callsPerTask => callsPerTask._2.size)), addInt, 0)

  val avgCacheRetries: Int =
    if (totalJobsPerRootWf > 0) {
      val cacheRetriesList =
        calls.map(taskMap => taskMap.flatMap(callsPerTask => callsPerTask._2.map(call => call.cacheCopyRetries)))
      sumElementsInOptionSeq(cacheRetriesList, addInt, 0) / totalJobsPerRootWf
    } else 0

  val avgTimeInCallCachingState: Duration =
    if (totalJobsPerRootWf > 0) {
      val timeInCallCachingStateList =
        calls.map(taskMap => taskMap.flatMap(callsPerTask => callsPerTask._2.map(call => call.timeInCallCachingState)))
      sumElementsInOptionSeq(timeInCallCachingStateList, addDuration, Duration.ZERO).dividedBy(
        totalJobsPerRootWf.toLong
      )
    } else Duration.ZERO

  val avgTimeInCheckingCallCacheState: Duration =
    if (totalJobsPerRootWf > 0) {
      val timeInCheckingCallCacheStateList = calls.map(taskMap =>
        taskMap.flatMap(callsPerTask => callsPerTask._2.map(call => call.timeInCheckingCallCacheState))
      )
      sumElementsInOptionSeq(timeInCheckingCallCacheStateList, addDuration, Duration.ZERO).dividedBy(
        totalJobsPerRootWf.toLong
      )
    } else Duration.ZERO
  ...
  val avgTimeInJobPreparation: Duration =
    if (totalJobsPerRootWf > 0) {
      val timeInJobPreparationList =
        calls.map(taskMap => taskMap.flatMap(callsPerTask => callsPerTask._2.map(call => call.timeInJobPreparation)))
  ...
  val avgTimeForFetchingAndCopyingCacheHit: Duration =
    if (totalJobsPerRootWf > 0) {
      val timeInCopyingList = calls.map(taskMap =>
        taskMap.flatMap(callsPerTask => callsPerTask._2.map(call => call.timeForFetchingAndCopyingCacheHit))
      )
