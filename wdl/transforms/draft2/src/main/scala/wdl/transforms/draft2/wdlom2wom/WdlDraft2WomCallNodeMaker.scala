...
object WdlDraft2WomCallNodeMaker extends WomCallNodeMaker[WdlCall] {
  override def toWomCallNode(wdlCall: WdlCall,
  ...
  ): ErrorOr[CallNode.CallNodeAndNewNodes] = {
    ...
    def allInputsWereWantedValidation(callable: Callable): ErrorOr[Unit] = {
      val callableExpectedInputs = callable.inputs
        .collect {
      ...
      unexpectedInputs match {
        case None => ().validNel
        case Some(unexpectedInputsNel) =>
          (unexpectedInputsNel map { unexpectedInput =>
    ...
    def expressionNodeMappings: ErrorOr[Map[LocalName, AnonymousExpressionNode]] = {
      val precomputedOgins: Map[String, OutputPort] = outerLookup collect {
        case (name, port) if !localLookup.contains(name) =>
          name -> OuterGraphInputNode(WomIdentifier(name), port, preserveIndexForOuterLookups).singleOutputPort
      }
      val newLocalLookup = localLookup ++ precomputedOgins
      wdlCall.inputMappings traverse { case (inputName, wdlExpression) =>
        ...
        WdlWomExpression.toAnonymousExpressionNode(identifier,
                                                   WdlWomExpression(wdlExpression, wdlCall),
                                                   newLocalLookup,
                                                   Map.empty,
    ...
    def foldInputDefinitions(expressionNodes: Map[LocalName, ExpressionNode],
      ...
      callable.inputs.foldMap {
        ...
        case withDefault @ OverridableInputDefinitionWithDefault(_, _, expression, _, _)
            if inASubworkflow || expression.inputs.nonEmpty =>
          InputDefinitionFold(
    ...
    (expressionNodeMappings, wdlCall.callable.toWomCallable) flatMapN { case (mappings, callable) =>
      allInputsWereWantedValidation(callable) map { _ =>
        ...
        val callNodeAndNewNodes = callNodeBuilder.build(
          wdlCall.womIdentifier,
          callable,
