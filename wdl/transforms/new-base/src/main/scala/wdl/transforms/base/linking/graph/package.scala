...
package object graph {
  implicit val graphElementUnlinkedValueGenerator: UnlinkedValueGenerator[WorkflowGraphElement] =
    new UnlinkedValueGenerator[WorkflowGraphElement] {

      override def generatedValueHandles(a: WorkflowGraphElement,
                                         typeAliases: Map[String, WomType],
                                         callables: Map[String, Callable]
      ): ErrorOr[Set[GeneratedValueHandle]] = a match {
  ...
  implicit val scatterElementUnlinkedValueGenerator: UnlinkedValueGenerator[ScatterElement] =
    new UnlinkedValueGenerator[ScatterElement] {
      override def generatedValueHandles(a: ScatterElement,
                                         typeAliases: Map[String, WomType],
                                         callables: Map[String, Callable]
      ): ErrorOr[Set[GeneratedValueHandle]] =
        a.graphElements.toList.traverse(_.generatedValueHandles(typeAliases, callables)).map(_.toSet.flatten) map {
          _.map {
            case GeneratedIdentifierValueHandle(id, womType) =>
              GeneratedIdentifierValueHandle(id, WomArrayType(womType))
            case GeneratedCallOutputValueHandle(first, second, womType) =>
              GeneratedCallOutputValueHandle(first, second, WomArrayType(womType))
            case a: GeneratedCallFinishedHandle => a
          }
        }
    }

  implicit val IfElementUnlinkedValueGenerator: UnlinkedValueGenerator[IfElement] =
    new UnlinkedValueGenerator[IfElement] {
      override def generatedValueHandles(a: IfElement,
                                         typeAliases: Map[String, WomType],
                                         callables: Map[String, Callable]
      ): ErrorOr[Set[GeneratedValueHandle]] =
        a.graphElements.toList.traverse(_.generatedValueHandles(typeAliases, callables)).map(_.toSet.flatten) map {
          _.map {
            case GeneratedIdentifierValueHandle(id, womType) =>
              GeneratedIdentifierValueHandle(id, WomOptionalType(womType).flatOptionalType)
            case GeneratedCallOutputValueHandle(first, second, womType) =>
              GeneratedCallOutputValueHandle(first, second, WomOptionalType(womType).flatOptionalType)
            case a: GeneratedCallFinishedHandle => a
          }
        }
    }

  implicit val callElementUnlinkedValueGenerator: UnlinkedValueGenerator[CallElement] =
    new UnlinkedValueGenerator[CallElement] {
      override def generatedValueHandles(a: CallElement,
                                         typeAliases: Map[String, WomType],
                                         callables: Map[String, Callable]
      ): ErrorOr[Set[GeneratedValueHandle]] = {
        ...
        callables.get(a.callableReference) match {
          ...
          case None =>
            s"Cannot generate outputs for 'call ${a.callableReference}'. No such callable exists in [${callables.keySet
                .mkString(", ")}]".invalidNel
        }
      }
    }

  implicit val graphElementUnlinkedValueConsumer: GraphElementValueConsumer[WorkflowGraphElement] =
    new GraphElementValueConsumer[WorkflowGraphElement] {
      ...
      ): ErrorOr[Set[UnlinkedConsumedValueHook]] = a match {
  ...
  implicit val callElementUnlinkedValueConsumer: GraphElementValueConsumer[CallElement] =
    new GraphElementValueConsumer[CallElement] {
      override def graphElementConsumedValueHooks(a: CallElement,
        ...
        val consumedByInputSection: Set[UnlinkedConsumedValueHook] = a.body match {
          case Some(callBodyElement: CallBodyElement) =>
            callBodyElement.inputs.flatMap(_.expressionConsumedValueHooks).toSet
  ...
  implicit val scatterElementUnlinkedValueConsumer: GraphElementValueConsumer[ScatterElement] =
    new GraphElementValueConsumer[ScatterElement] {
      override def graphElementConsumedValueHooks(a: ScatterElement,
        ...
        val bodyConsumedValuesValidation: ErrorOr[Set[UnlinkedConsumedValueHook]] =
          a.graphElements.toList.traverse(_.graphElementConsumedValueHooks(typeAliases, callables)).map(_.toSet.flatten)
        val scatterExpressionHooks: Set[UnlinkedConsumedValueHook] = a.scatterExpression.expressionConsumedValueHooks

        val bodyGeneratedValuesValidation: ErrorOr[Set[String]] = a.graphElements.toList
          .traverse(_.generatedValueHandles(typeAliases, callables))
          .map(_.toSet.flatten.map(_.linkableName))

        (bodyConsumedValuesValidation, bodyGeneratedValuesValidation) mapN {
          (bodyConsumedValues, bodyGeneratedValues) =>
            val unsatisfiedBodyElementHooks = bodyConsumedValues.filterNot {
              ...
              case UnlinkedCallOutputOrIdentifierAndMemberAccessHook(first, second) =>
                bodyGeneratedValues.contains(first) || bodyGeneratedValues.contains(
                  s"$first.$second"
                ) || a.scatterVariableName == first
  ...
  implicit val ifElementUnlinkedValueConsumer: GraphElementValueConsumer[IfElement] =
    new GraphElementValueConsumer[IfElement] {
        ...
        (bodyConsumedValuesValidation, bodyGeneratedValuesValidation) mapN {
          (bodyConsumedValues, bodyGeneratedValues) =>
            val unsatisfiedBodyElementHooks = bodyConsumedValues.filterNot {
              ...
              case UnlinkedCallOutputOrIdentifierAndMemberAccessHook(first, second) =>
                bodyGeneratedValues.contains(first) || bodyGeneratedValues.contains(s"$first.$second")
              case after: UnlinkedAfterCallHook => bodyGeneratedValues.contains(after.linkString)
            }
