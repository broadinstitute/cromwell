...
object LookupEvaluators {
  ...
  implicit val identifierLookupTypeEvaluator: TypeEvaluator[IdentifierLookup] = new TypeEvaluator[IdentifierLookup] {
    override def evaluateType(a: IdentifierLookup, linkedValues: Map[UnlinkedConsumedValueHook, GeneratedValueHandle], typeAliases: Map[String, WomType])(
      ...
      linkedValues.collectFirst {
        ...
        case None =>
          s"Type evaluation failure. No suitable type found for identifier lookup '${a.identifier}' amongst {${linkedValues
              .map(_._2.linkableName)
              .mkString(", ")}}".invalidNel
      }
  }
    ...
    new TypeEvaluator[IdentifierMemberAccess] {
      override def evaluateType(a: IdentifierMemberAccess,
        ...
        generatedValueHandle match {
          case Some(GeneratedIdentifierValueHandle(a.first, womType)) =>
            doLookup(womType, NonEmptyList(a.second, a.memberAccessTail.toList))
          case Some(GeneratedCallOutputValueHandle(a.first, a.second, womType)) =>
            NonEmptyList.fromList(a.memberAccessTail.toList) match {
              case Some(tailList) => doLookup(womType, tailList)
              case None => womType.validNel
            }
          case _ =>
            s"Type evaluation failure. No suitable type found for identifier lookup '${a.first}' or '${a.first}.${a.second}' amongst {${linkedValues
                .map(_._2.linkableName)
                .mkString(", ")}}".invalidNel
        }
      }
  ...
  implicit val indexAccessTypeEvaluator: TypeEvaluator[IndexAccess] = new TypeEvaluator[IndexAccess] {
    override def evaluateType(a: IndexAccess, linkedValues: Map[UnlinkedConsumedValueHook, GeneratedValueHandle],typeAliases: Map[String, WomType])(
      implicit expressionTypeEvaluator: TypeEvaluator[ExpressionElement]
    ): ErrorOr[WomType] =
      (a.expressionElement.evaluateType(linkedValues, typeAliases), a.index.evaluateType(linkedValues, typeAliases), a.index.validNel) flatMapN {
        ...
        case (WomMapType(keyType, valueType), lookupType, _) if keyType.isCoerceableFrom(lookupType) =>
          valueType.validNel
        case (WomCompositeType(typeMap, _), WomStringType, StringLiteral(str)) =>
          typeMap.get(str) match {
        ...
        case (otherObject, otherKey, _) =>
          s"Type evaluation failed for $a. Cannot dereference a ${otherObject.stableName} value using a ${otherKey.stableName} key".invalidNel
      }
  }
  ...
  private def doLookup(womType: WomType, lookupChain: NonEmptyList[String]): ErrorOr[WomType] = {
    ...
    val thisValue: ErrorOr[WomType] = womType match {
      case WomCompositeType(typeMap, _) =>
        typeMap.get(key).toErrorOr(s"No such field '$key' on type ${womType.stableName}.")
