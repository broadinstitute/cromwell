...
object CromwellShutdown extends GracefulStopSupport {
  ...
  def registerShutdownTasks(
    ...
    def shutdownActor(actor: ActorRef, phase: String, message: AnyRef, customTimeout: Option[FiniteDuration] = None)(
    ...
    coordinatedShutdown.addTask(CoordinatedShutdown.PhaseServiceRequestsDone, "releaseWorkflowStoreEntries") { () =>
      EngineServicesStore.engineDatabaseInterface
        .releaseWorkflowStoreEntries(cromwellId)
        .map(count => logger.info("{} workflows released by {}", count, cromwellId))
        .as(Done)
    }
    coordinatedShutdown.addTask(CoordinatedShutdown.PhaseServiceRequestsDone,
                                "publishMetadataForReleasedWorkflowStoreEntries"
    ) { () =>
      EngineServicesStore.engineDatabaseInterface
        .findWorkflows(cromwellId)
        .map(ids =>
          ids foreach { id =>
            WorkflowProcessingEventPublishing
              .publish(WorkflowId.fromString(id), cromwellId, Released, serviceRegistryActor)
          }
        )
        .as(Done)
    }
    ...
    workflowCallbackActor.foreach(wca =>
      shutdownActor(wca, CoordinatedShutdown.PhaseServiceRequestsDone, Broadcast(ShutdownCommand))
    )
    ...
    coordinatedShutdown.addTask(CoordinatedShutdown.PhaseBeforeActorSystemTerminate, "shutdownMaterializer") { () =>
      ...
      logger.info("Stream materializer task complete (not needed in Akka 2.6)")
      Future.successful(Done)
    }
