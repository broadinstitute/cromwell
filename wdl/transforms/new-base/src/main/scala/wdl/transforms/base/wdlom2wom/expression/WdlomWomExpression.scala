...
final case class WdlomWomExpression private (expressionElement: ExpressionElement,
  ...
  def evaluateValueForPlaceholder(inputValues: Map[String, WomValue],
                                  ioFunctionSet: IoFunctionSet,
                                  forCommandInstantiationOptions: ForCommandInstantiationOptions
  ): ErrorOr[EvaluatedValue[_]] =
  ...
  override def evaluateFiles(inputs: Map[String, WomValue],
                             ioFunctionSet: IoFunctionSet,
                             coerceTo: WomType
  ): ErrorOr[Set[FileEvaluation]] =
    expressionElement.evaluateFilesNeededToEvaluate(inputs, ioFunctionSet, coerceTo) map {
      _ map {
