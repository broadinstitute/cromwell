...
cromwell::private::papi::setup_papi_gcr() {
    if command -v docker; then
        ...
        # We built a SNAPSHOT image of the current localizer script, pushed it and are setting it as a default here.
        # TODO: Once a stable image of cromwell-drs-localizer is pushed to Dockerhub, replace the default image with that
        CROMWELL_BUILD_PAPI_DOCKER_IMAGE_DRS="broadinstitute/cromwell-drs-localizer:53-db12deb-SNAP"
    fi
