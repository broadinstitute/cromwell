    ...
    override def evaluateValue(a: Size,
      ...
      def optionalSafeFileSize(value: WomValue): ErrorOr[Long] = value match {
        case f if f.isInstanceOf[WomSingleFile] || WomSingleFileType.isCoerceableFrom(f.womType) => {
          ...
          logger.info(f"($uuid) ($seconds%,.3f seconds) optionalSafeFileSize $value done ($getClass) (thread '${Thread.currentThread().getName}')")
          if (seconds > 5 * 60) {
            logger.warn(f"($uuid) ($seconds%,.3f seconds) FYI! 'optionalSafeFileSize $value done' IO IS OVER 5 MINUTES ($getClass) (thread '${Thread.currentThread().getName}')")
          }
          result
        }
