...
class BackendCallSpec extends CromwellTestkitSpec with ScalaFutures with WorkflowDescriptorBuilder {
  ...
  val backend = new OldStyleLocalBackend(CromwellTestkitSpec.DefaultLocalBackendConfigEntry, new CromwellTestkitSpec.TestWorkflowManagerSystem().actorSystem)

  "BackendCall hash function" should {
    "not hash by default" ignore {
      ...
      val jobDescriptor = OldStyleBackendCallJobDescriptor(descriptor, BackendCallKey(call, None, 1), descriptor.actualInputs)
      val actual = jobDescriptor.hash.futureValue.overallHash
      actual should be(empty)
    }

    "not change very often - if it changes, make sure it is for a good reason" ignore {
      val sources = SampleWdl.CallCachingHashingWdl.asWorkflowSources()
    ...
    "not change with a Docker image hash specified - if it changes, make sure it is for a good reason" ignore {
  ...
  "BackendCall adjustSharedInputPaths" should {
    "produce the correctly formatted exception message" ignore {
      ...
      val jobDescriptor = OldStyleBackendCallJobDescriptor(descriptor, BackendCallKey(call, None, 1), inputs)
