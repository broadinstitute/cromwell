    ...
    for log in logs:
        for entry in log_utils.filter_and_sort_log_entries(log):
            ...
            if log_utils.is_end_event(entry):
                ...
                # There are actually two timestamps in the JSON log entries which appear to represent different
                # concepts: time emitted ('jsonPayload.localTimestamp') versus time added to the log ('timestamp').
                # Time emitted would seem to be preferable but that value is not specified with a timezone and is
                # ambiguously interpreted by the parsing code as being EST when it's actually UTC which makes the report
                # a bit confusing. In practice the timestamps only seem to differ by a millisecond or so, so no big
                # deal to use 'timestamp' with its explicit UTC timezone.
