...
class StatsDInstrumentationServiceActorBenchmarkSpec
  ...
  it should "have good throughput for gauges" in {
    val instrumentationActor =
      TestActorRef(new StatsDInstrumentationServiceActor(config, ConfigFactory.load(), registryProbe))
    ...
    eventually {
      instrumentationActor.underlyingActor.metricRegistry.getGauges.asScala
        .get(gaugeName)
        .map(_.getValue) shouldBe Some(999999)
    }
  }
}
