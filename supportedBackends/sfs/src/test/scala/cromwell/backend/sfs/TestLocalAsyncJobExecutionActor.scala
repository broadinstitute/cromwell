...
class TestLocalAsyncJobExecutionActor(override val standardParams: StandardAsyncExecutionActorParams)
  ...
  override lazy val processArgs: SharedFileSystemCommand = {
    ...
    if (isDockerRun) {
      ...
      SharedFileSystemCommand("/bin/bash", "-c", s"docker run --rm -v $cwd:$dockerCwd -i $docker /bin/bash < $script")
    } else {
...
object TestLocalAsyncJobExecutionActor {
  ...
  def createBackendRef(jobDescriptor: BackendJobDescriptor, configurationDescriptor: BackendConfigurationDescriptor)(
    implicit system: ActorSystem
  ): TestActorRef[StandardSyncExecutionActor] = {
    val serviceRegistryActor =
      system.actorOf(Props(new InMemoryKvServiceActor)) // We only really need the KV store for now
    val ioActor = system.actorOf(SimpleIoActor.props)
    val workflowPaths =
      new WorkflowPathsWithDocker(jobDescriptor.workflowDescriptor, configurationDescriptor.backendConfig)
    val initializationData = new StandardInitializationData(
      workflowPaths,
