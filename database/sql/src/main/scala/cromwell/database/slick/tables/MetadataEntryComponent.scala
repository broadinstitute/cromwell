...
trait MetadataEntryComponent {
  ...
  class MetadataEntries(tag: Tag) extends Table[MetadataEntry](tag, "METADATA_ENTRY") {
    ...
    /**
      ...
      */
    def ixMetadataEntryWeuMk = index(
      "IX_METADATA_ENTRY_WEU_MK",
      (workflowExecutionUuid, metadataKey),
      unique = false
    )

    /**
     * Index designed to accelerate access for specific jobs/calls/attempts, such as:
     * - Get all metadata for a specific attempt
     * - Count how many attempts there were for a specific job
     * - Get info about all shards for specific call
     *
     * @return A reference to the index
     */
    def ixMetadataEntryWeuCfqnJsiJraMk = index(
      "IX_METADATA_ENTRY_WEU_CFQN_JSI_JRA_MK",
