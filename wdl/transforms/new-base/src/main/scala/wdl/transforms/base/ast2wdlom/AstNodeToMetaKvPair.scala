...
object AstNodeToMetaKvPair {
  def astNodeToMetaKvPair: CheckedAtoB[GenericAstNode, MetaKvPair] = {
    CheckedAtoB.fromErrorOr("convert AstNode to MetaKvPair")(convertMetaKvPair)
  }

  private def convertMetaKvPair(astNode: GenericAstNode): ErrorOr[MetaKvPair] = astNode match {
  ...
  private def convertMetaValue(astNode: GenericAstNode): ErrorOr[MetaValueElement] = {
