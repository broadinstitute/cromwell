...
/**
 * This actor handles the fetching and caching of the public Google Cost Catalog, which enables us to lookup SKUs,
 * which are necessary for cost calculations.
 */
class CostCatalogService(serviceConfig: Config,
  ...
  // Cached catalog, will be (maybe) refreshed whenever getOrFetchCachedCostCatalog is called.
  private var costCatalog: ExpiringCostCatalog = ExpiringCostCatalog(processCostCatalog(fetchNewCatalog), java.time.LocalDateTime.now())
  ...
  private def getOrFetchCachedCatalog(): Map[CostCatalogKey, CostCatalogValue] = {
    if(isCurrentCatalogExpired){
      costCatalog = ExpiringCostCatalog(processCostCatalog(fetchNewCatalog), java.time.LocalDateTime.now())
    }
    costCatalog.catalog
  }

  private def fetchNewCatalog: Iterable[Sku] = mockTestData.getOrElse(makeInitialWebRequest().iterateAll().asScala)
