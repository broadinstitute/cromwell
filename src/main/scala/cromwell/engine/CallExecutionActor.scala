...
//import cromwell.backend.model.{Subscribed, ExecutionEvent, Subscription}
...
//  }
//
//  def props(): Props = Props(new CallExecutionActor())
//}
//
///** Actor to manage the execution of a single call. */
//class CallExecutionActor() extends Actor with CromwellActor {
...
//
//
//  private def orchestrate(backendActor: ActorRef) = {
//    val prepareFuture = backendActor ?  BackendActor.Prepare
//    prepareFuture.onComplete(execute).onComplete
//
//  }
//
//  override def receive = LoggingReceive {
//    case backendActor: BackendActor =>
//      subscribe(sender, backendActor)
//
//    case Subscribed =>
//      sender ! Prepare
//
//    case TaskStatus(Status.Created, _)=>
//      sender ! cromwell.backend.BackendActor.Execute
//
//    case TaskStatus(Status.Canceled | Status.Failed, throwable@Throwable) =>
//      val failureHandle = new FailedExecutionHandle(throwable)
//      context.parent ! CallActor.ExecutionFinished(call, failureHandle)
//
//
