...
object AstTools {
  ...
  class InterpolatedTerminal(val rootTerminal: Terminal, innerTerminal: Terminal, columnOffset: Int)
      extends Terminal(
        innerTerminal.getId,
  ...
  implicit class EnhancedAstNode(val astNode: AstNode) extends AnyVal {
    ...
    private def findTerminalsInInterpolatedString(t: Terminal,
      ...
      interpolatedExpressionAstNodesAndTheirMatchPosition match {
        ...
        case Nil if t.getTerminalStr == terminalType =>
          val finalTerminal = parentTerminal map { parent =>
            new InterpolatedTerminal(parent, t, columnOffset)
          } getOrElse t
        ...
        case expressions =>
          expressions.flatMap { case (innerNode, offset) =>
            innerNode.findTerminalsWithTrail(terminalType,
                                             trail :+ t,
                                             Option(parentTerminal.getOrElse(t)),
                                             columnOffset + offset
            )
          }.toMap
      }
    ...
    def findTerminalsWithTrail(terminalType: String,
                               ...
                               columnOffset: Int = 0
    ): Map[Terminal, Seq[AstNode]] =
      astNode match {
        case o: Ast if o.isObjectLiteral =>
          o.getAttribute("map").astListAsVector flatMap {
            case a: Ast => a.getAttribute("value").findTerminalsWithTrail(terminalType, trail :+ o, parentTerminal)
            case _: AstNode => Seq.empty
          } toMap
        case a: Ast =>
          a.getAttributes.values.asScala flatMap { _.findTerminalsWithTrail(terminalType, trail :+ a) } toMap
    ...
    def optionalType(a: Ast, wdlSyntaxErrorFormatter: WdlSyntaxErrorFormatter) = WomOptionalType(
      a.getAttribute("innerType").womType(wdlSyntaxErrorFormatter)
    )
    ...
    def womValue(womType: WomType, wdlSyntaxErrorFormatter: WdlSyntaxErrorFormatter): WomValue = {
      ...
      def astTupleToValue(a: Ast): WomValue = {
        ...
        if (subElements.lengthCompare(1) == 0) {
          ...
          WomPair(subElements.head.womValue(pairType.leftType, wdlSyntaxErrorFormatter),
                  subElements(1).womValue(pairType.rightType, wdlSyntaxErrorFormatter)
          )
        } else {
          throw new SyntaxError(
            s"Could not convert AST to a $womType (${Option(astNode).map(_.toString).getOrElse("No AST")})"
          )
        }
      }
      ...
      astNode match {
        ...
        case t: Terminal if t.getTerminalStr == "integer" && womType == WomIntegerType =>
          WomInteger(t.getSourceString.toInt)
        case t: Terminal if t.getTerminalStr == "float" && womType == WomFloatType =>
          WomFloat(t.getSourceString.toDouble)
        case t: Terminal if t.getTerminalStr == "boolean" && womType == WomBooleanType =>
          t.getSourceString.toLowerCase match {
        ...
        case a: Ast if a.getName == "ArrayLiteral" && womType.isInstanceOf[WomArrayType] =>
          ...
          val elements = a.getAttribute("values").astListAsVector map { node =>
            node.womValue(arrType.memberType, wdlSyntaxErrorFormatter)
          }
  ...
  implicit class EnhancedAstSeq(val astSeq: Seq[Ast]) extends AnyVal {
    def duplicatesByName: Seq[Ast] =
      astSeq
        .groupBy(_.getAttribute("name").sourceString)
        .collect { case (_, v) if v.lengthCompare(1) > 0 => v.head }
        .toVector
  }
  ...
  /**
    ...
    */
  def findTopLevelMemberAccesses(expr: AstNode): Iterable[Ast] = expr
    .findAstsWithTrail("MemberAccess")
    .filterNot { case (_, v) =>
      v exists {
  ...
  final case class VariableReference private[wdl] (terminal: Terminal, trail: Iterable[AstNode], from: Scope) {
    ...
    private lazy val terminalSubIdentifier: Option[Terminal] = trail.collectFirst {
      case a: Ast
          if a.isMemberAccess
            ...
            && a.getAttribute("rhs").isTerminal =>
        a.getAttribute("rhs").asInstanceOf[Terminal]
    }

    private lazy val fullVariableReferenceString: String =
      terminal.getSourceString + (terminalSubIdentifier map { "." + _.getSourceString } getOrElse "")
    ...
    lazy val referencedVariableName: String =
      findResolvableSubstring(fullVariableReferenceString, fullVariableReferenceString).getOrElse(
        fullVariableReferenceString
      )
  }
  ...
  def findVariableReferences(expr: AstNode, from: Scope): Iterable[VariableReference] = {
    ...
    expr.findTerminalsWithTrail("identifier").collect {
      case (terminal, trail) if !isMemberAccessRhs(terminal, trail) && !isFunctionName(terminal, trail) =>
        VariableReference(terminal, trail, from)
    }
  ...
  def callInputSectionIOMappings(ast: Ast, wdlSyntaxErrorFormatter: WdlSyntaxErrorFormatter): Seq[Ast] = {
    ...
    val callInputSections = ast.findAsts(AstNodeName.Inputs).map { inputSectionAst =>
      inputSectionAst.getAttribute("map").findAsts(AstNodeName.IOMapping) match {
        case kvPairs: Seq[Ast] if kvPairs.isEmpty =>
          throw new SyntaxError(wdlSyntaxErrorFormatter.emptyInputSection(callTaskName))
    ...
    callInputSections match {
      ...
      case asts: Seq[Ast] =>
        ...
        val firstKeyTerminal =
          secondInputSectionIOMappings.head.asInstanceOf[Ast].getAttribute("key").asInstanceOf[Terminal]
  ...
  def wdlSectionToStringMap(ast: Ast,
                            node: String,
    ...
    ast.findAsts(node) match {
      ...
      case a if a.lengthCompare(1) == 0 =>
        ...
        a.head
          .findAsts(AstNodeName.RuntimeAttribute)
          .map { ast =>
      ...
      case _ =>
        throw new SyntaxError(
          wdlSyntaxErrorFormatter.expectedAtMostOneSectionPerTask(node, ast.getAttribute("name").asInstanceOf[Terminal])
        )
    }
