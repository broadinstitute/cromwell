...
import cromwell.backend.google.batch.authentication.{GcpBatchAuths, GcpBatchDockerCredentials}
...
class GcpBatchConfiguration(val configurationDescriptor: BackendConfigurationDescriptor,
  ...
  val jobShell: String = configurationDescriptor.backendConfig
    .as[Option[String]]("job-shell")
    .getOrElse(configurationDescriptor.globalConfig.getOrElse("system.job-shell", "/bin/bash"))
