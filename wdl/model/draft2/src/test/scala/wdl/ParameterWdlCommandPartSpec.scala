...
class ParameterWdlCommandPartSpec extends WdlTest {
  ...
  "Command parameter WDL" should {
    ...
    "honor defined optional values during evaluation" in {
      ...
      val command =
        task.instantiateCommand(Map(task.declarations.head -> WomString("world")), NoFunctions).toTry.get.head
    ...
    "replace undefined values by their default value after evaluation" in {
      ...
      val command = task
        .instantiateCommand(Map(task.declarations.head -> WomOptionalValue.none(WomStringType)), NoFunctions)
        .toTry
        .get
        .head
