...
object EngineFunctionEvaluators {
  ...
  private def evaluateToFile(forFunction: String,
                             outermostElement: ExpressionElement,
                             inputs: Map[String, WomValue],
                             ioFunctionSet: IoFunctionSet,
                             nestedElement: Option[ExpressionElement] = None
    ...
    (elementToExamine match {
      case IndexAccess(expressionElement, _) =>
        evaluateToFile(forFunction, outermostElement, inputs, ioFunctionSet, Option(expressionElement))
  ...
  def singleParameterPassthroughFileEvaluator[A <: OneParamFunctionCallElement]: FileEvaluator[A] =
    new FileEvaluator[A] {
  ...
  def singleParameterEvaluateToFileFileEvaluator[A <: OneParamFunctionCallElement](
    functionName: String
  ): FileEvaluator[A] = new FileEvaluator[A] {
    override def predictFilesNeededToEvaluate(a: A,
                                              inputs: Map[String, WomValue],
                                              ioFunctionSet: IoFunctionSet,
                                              coerceTo: WomType
  ...
  implicit val stdoutFunctionEvaluator: FileEvaluator[StdoutElement.type] = new FileEvaluator[StdoutElement.type] {
    override def predictFilesNeededToEvaluate(a: ExpressionElement.StdoutElement.type,
                                              inputs: Map[String, WomValue],
                                              ioFunctionSet: IoFunctionSet,
                                              coerceTo: WomType
    )(implicit
  ...
  implicit val stderrFunctionEvaluator: FileEvaluator[StderrElement.type] = new FileEvaluator[StderrElement.type] {
    override def predictFilesNeededToEvaluate(a: ExpressionElement.StderrElement.type,
                                              inputs: Map[String, WomValue],
                                              ioFunctionSet: IoFunctionSet,
                                              coerceTo: WomType
    )(implicit
  ...
  implicit val readLinesFunctionEvaluator: FileEvaluator[ReadLines] = singleParameterEvaluateToFileFileEvaluator(
    "read_lines"
  )
  ...
  implicit val readObjectFunctionEvaluator: FileEvaluator[ReadObject] = singleParameterEvaluateToFileFileEvaluator(
    "read_object"
  )

  implicit val readObjectsFunctionEvaluator: FileEvaluator[ReadObjects] = singleParameterEvaluateToFileFileEvaluator(
    "read_objects"
  )

  implicit val readJsonFunctionEvaluator: FileEvaluator[ReadJson] = singleParameterEvaluateToFileFileEvaluator(
    "read_json"
  )

  implicit val readIntFunctionEvaluator: FileEvaluator[ReadInt] = singleParameterEvaluateToFileFileEvaluator("read_int")

  implicit val readStringFunctionEvaluator: FileEvaluator[ReadString] = singleParameterEvaluateToFileFileEvaluator(
    "read_string"
  )

  implicit val readFloatFunctionEvaluator: FileEvaluator[ReadFloat] = singleParameterEvaluateToFileFileEvaluator(
    "read_float"
  )

  implicit val readBooleanFunctionEvaluator: FileEvaluator[ReadBoolean] = singleParameterEvaluateToFileFileEvaluator(
    "read_boolean"
  )
  ...
  implicit val globFunctionEvaluator: FileEvaluator[Glob] = new FileEvaluator[Glob] {
    override def predictFilesNeededToEvaluate(a: Glob,
                                              inputs: Map[String, WomValue],
                                              ioFunctionSet: IoFunctionSet,
                                              coerceTo: WomType
    )(implicit
      ...
      a.param.evaluateValue(inputs, ioFunctionSet, None) flatMap {
        ...
        case other =>
          s"Could not predict files to delocalize from '$a' for 'glob'. Expected a primitive but got ${other.getClass.getSimpleName}".invalidNel
      }
  }

  implicit val sizeFunctionEvaluator: FileEvaluator[Size] = new FileEvaluator[Size] {
    override def predictFilesNeededToEvaluate(a: Size,
                                              inputs: Map[String, WomValue],
                                              ioFunctionSet: IoFunctionSet,
                                              coerceTo: WomType
    )(implicit
      ...
      (evaluateToFile("size", a.file, inputs, ioFunctionSet): ErrorOr[Set[WomFile]],
       a.unit.fold(Set.empty[WomFile].validNel: ErrorOr[Set[WomFile]])(
         _.evaluateFilesNeededToEvaluate(inputs, ioFunctionSet, coerceTo)
       )
      ) mapN { _ ++ _ }
  }

  implicit val basenameFunctionEvaluator: FileEvaluator[Basename] = new FileEvaluator[Basename] {
    override def predictFilesNeededToEvaluate(a: Basename,
                                              inputs: Map[String, WomValue],
                                              ioFunctionSet: IoFunctionSet,
                                              coerceTo: WomType
    )(implicit
