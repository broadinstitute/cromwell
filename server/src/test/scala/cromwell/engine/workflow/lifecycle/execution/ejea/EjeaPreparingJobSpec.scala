...
class EjeaPreparingJobSpec extends EngineJobExecutionActorSpec with CanExpectHashingInitialization with Eventually {
  ...
  "An EJEA in PreparingJob state" should {
    ...
    CallCachingModes foreach { mode =>
      if (mode.readFromCache) {
        s"Start checking for a cache hit when job preparation succeeds and a docker hash is available ($mode)" in {
          ...
          ejea.stateData should be(
            ResponsePendingData(jobDescriptor, helper.bjeaProps, None, Option(helper.ejhaProbe.ref))
          )
        }
      ...
      s"Not proceed if Job Preparation fails ($mode)" in {
        val prepActorResponse =
          CallPreparationFailed(helper.jobDescriptorKey, new Exception("The goggles! They do nothing!"))
        val prepFailedEjeaResponse =
          JobFailedNonRetryableResponse(helper.jobDescriptorKey, prepActorResponse.throwable, None)
        ejea = ejeaInPreparingState(mode)
  ...
  def jobPrepSuccessResponse(jobDescriptor: BackendJobDescriptor) =
    BackendJobPreparationSucceeded(jobDescriptor, helper.bjeaProps)

  def ejeaInPreparingState(mode: CallCachingMode, restarting: Boolean = false) = helper
    .buildEJEA(restarting = restarting, callCachingMode = mode)
    .setStateInline(state = PreparingJob, data = NoData)
