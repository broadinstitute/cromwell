...
class ExecutionStoreSpec extends AnyFlatSpec with Matchers with BeforeAndAfter {
  ...
  it should "allow 10000 unconnected call keys to be enqueued and started in small batches" in {
    ...
    def jobKeys: Map[JobKey, ExecutionStatus] = (0.until(10000).toList map { i =>
      BackendJobDescriptorKey(noConnectionsGraphNode, Option(i), 1) -> NotStarted
    }).toMap
    ...
    while (store.needsUpdate) {
      ...
      store.store.getOrElse(NotStarted, List.empty).size should be(
        10000 - iterationNumber * ExecutionStore.MaxJobsToStartPerTick
      )
