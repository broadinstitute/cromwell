...
class IoActorRequesterSpec extends TestKitSuite with ImplicitSender with AnyFlatSpecLike with Matchers with Eventually {
  ...
  it should "Allow actors to request and receive a reference to the IO Actor" in {
    ...
    val requester = TestActorRef[SimpleIoActorRequester](simpleIoActorRequesterProps(serviceRegistry.ref),
                                                         "simple-io-requester-request-receive"
    )
  ...
  it should "keep asking if the IoActor is not available immediately" in {
    ...
    val ioActor = TestProbe()
    val requester = TestActorRef[SimpleIoActorRequester](simpleIoActorRequesterProps(serviceRegistry.ref),
                                                         "simple-io-requester-not-ready-resilience"
    )
    ...
    eventually {
      requester.underlyingActor.ioActor.value match {
        case Some(Failure(exception)) =>
          exception.getMessage should startWith("Programmer Error: Unexpected response to a RequestIoActor message")
...
object IoActorRequesterSpec {
  class SimpleIoActorRequester(override val serviceRegistryActor: ActorRef) extends Actor with IoActorRequester {
    ...
    override def receive: Receive = { case other =>
      throw new RuntimeException(s"Didn't expect a message but got: $other")
    }
  }

  def simpleIoActorRequesterProps(serviceRegistryActor: ActorRef): Props = Props(
    new SimpleIoActorRequester(serviceRegistryActor)
  )
}
