...
class LeasedOutputStream(cloudHost: String, cloudPath: String, outputStream: OutputStream, lease: Lease[FTPClient])
  ...
  override def close() = {
    outputStream.close()
    autoRelease(IO.pure(lease))(FtpCompletePendingCommand(cloudHost, cloudPath, "close input steam").run).void
      .unsafeRunSync()
  }
}
