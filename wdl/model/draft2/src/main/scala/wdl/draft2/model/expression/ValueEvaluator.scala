...
case class ValueEvaluator(override val lookup: String => WomValue, override val functions: WdlFunctions[WomValue])
    extends Evaluator {
  override type T = WomValue

  private val InterpolationTagPattern = "\\$\\{\\s*([^\\}]*)\\s*\\}".r

  private def interpolate(strToProcess: String,
                          resultSoFar: Try[WomString] = Success(WomString(""))
  ): Try[WomString] = {
          ...
          case other =>
            Failure(
              new WomExpressionException(
                "'if' expression must be given a boolean argument but got: " + other.toWomString
              )
            )
        }
      ...
      case a: Ast if a.isMemberAccess =>
        a.getAttribute("rhs") match {
          case rhs: Terminal if rhs.getTerminalStr == "identifier" =>
            ...
            Try(lookup(memberAccessAsString)).recoverWith { case _ =>
              evaluate(a.getAttribute("lhs")).flatMap {
                  ...
                  o.values.get(rhs.getSourceString) match {
                    ...
                    case None =>
                      o match {
                        ...
                        case _ =>
                          Failure(new WomExpressionException(s"Could not find key ${rhs.getSourceString} in WdlObject"))
                      }
                  }
                ...
                case ns: WdlNamespace =>
                  Success(
                    lookup(ns.importedAs.map(n => s"$n.${rhs.getSourceString}").getOrElse(rhs.getSourceString))
                  )
                case _ =>
          ...
          case (Success(a: WomArray), Success(i: WomInteger)) =>
            Try(a.value(i.value)) match {
              ...
              case Failure(ex) =>
                Failure(
                  new WomExpressionException(
                    s"Failed to find index $index on array:\n\n$mapOrArray\n\n${ex.getMessage}"
                  )
                )
            }
