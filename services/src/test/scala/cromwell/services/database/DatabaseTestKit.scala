...
object DatabaseTestKit extends StrictLogging {
  ...
  /**
    ...
    * @return The return value of the block.
    */
  def withConnections[Profile1 <: JdbcProfile, Profile2 <: JdbcProfile, A](profile1: Profile1,
                                                                           database1: Profile1#Backend#Database,
                                                                           profile2: Profile2,
                                                                           database2: Profile2#Backend#Database
   ...
   */
  def schemalessDatabaseFromContainerOptAndSystem(containerOpt: Option[Container],
                                                  databaseSystem: DatabaseSystem
  ): SchemalessSlickDatabase with TestSlickDatabase =
    containerOpt match {
      ...
      case Some(cont) if cont.isInstanceOf[JdbcDatabaseContainer] =>
        new SchemalessSlickDatabase(getConfig(databaseSystem, Option(cont.asInstanceOf[JdbcDatabaseContainer])))
          with TestSlickDatabase
  ...
  def initializeDatabaseByContainerOptTypeAndSystem[A <: SlickDatabase](containerOpt: Option[Container],
    ...
    containerOpt match {
      ...
      case Some(cont) if cont.isInstanceOf[JdbcDatabaseContainer] =>
        initializedDatabaseFromConfig(databaseType,
                                      getConfig(databaseSystem, Option(cont.asInstanceOf[JdbcDatabaseContainer]))
        )
      case Some(_) => throw new RuntimeException("ERROR: container is not a JdbcDatabaseContainer.")
    }
    ...
    dbContainerOpt match {
      ...
      case None =>
        throw new RuntimeException(
          "ERROR: dbContainer option must be passed into `getConfig` method for all databases except HSQLDB."
        )
      case Some(dbContainer) =>
        val (slickProfile, jdbcDriver) = databaseSystem.platform match {
          case HsqldbDatabasePlatform =>
            throw new RuntimeException("ERROR: dbContainer option cannot be defined for HSQLDB database.")
  ...
  def connectionMetadata(database: SlickDatabase): ConnectionMetadata =
    withConnection(database.dataAccess.driver, database.database) { connection =>
