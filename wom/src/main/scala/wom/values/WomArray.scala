  ...
  def apply(womType: WomArrayType, value: Seq[WomValue]): WomArray = {
    if (womType == WomMaybeEmptyArrayType.EmptyArrayType && value.nonEmpty) {
      throw new UnsupportedOperationException(
        s"An ${womType.stableName} must be empty but instead has value: ${value.mkString(", ")}"
      )
    }
    ...
    coercedValue match {
      ...
      case Failure(f) =>
        throw new UnsupportedOperationException(s"Could not construct array of type $womType with this value: $value",
                                                f
        )
    }
  }

  def apply(value: Seq[WomValue]): WomArray =
    WomArray.apply(WomArrayType(WomType.homogeneousTypeFromValues(value)), value)
...
sealed abstract case class WomArray(womType: WomArrayType, value: Seq[WomValue])
    extends WomValue
    with WomArrayLike
      ...
      case _ =>
        Failure(
          new UnsupportedOperationException(
            s"Cannot TSV serialize a ${this.womType.stableName} (valid types are Array[Primitive], Array[Array[Primitive]], or Array[Object])"
          )
        )
    }
