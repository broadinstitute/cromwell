  ...
  def convert(b: TaskDefinitionElementToWomInputs)
    ...
    val conversion = (
      ...
      (validRuntimeAttributes, validCommand) mapN { (runtime, command) =>
        CallableTaskDefinition(a.taskDefinitionElement.name, Function.const(command.validNel), runtime, meta, parameterMeta, taskGraph.outputs, taskGraph.inputs, Set.empty, Map.empty, srcLoc = a.taskDefinitionElement.srcLoc)
      }
    }
