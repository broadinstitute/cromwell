...
object Executable {
  ...
  def withInputs(entryPoint: ExecutableCallable,
                 inputParsingFunction: InputParsingFunction,
                 inputFile: Option[String],
                 ioFunctions: IoFunctionSet,
                 strictValidation: Boolean
  ): Checked[Executable] =
    validateExecutable(entryPoint,
                       inputParsingFunction,
                       parseGraphInputs(strictValidation),
                       inputFile,
                       ioFunctions
    ).toChecked
  ...
  /**
    ...
    */
  private def parseGraphInputs(strictValidation: Boolean)(graph: Graph,
                                                          inputCoercionMap: Map[String, DelayedCoercionFunction],
                                                          ioFunctions: IoFunctionSet
    ...
    def fromInputMapping(gin: ExternalGraphInputNode): Option[IOChecked[ResolvedExecutableInput]] =
      inputCoercionMap
        ...
        .map {
          _.apply(gin.womType).toIOChecked
    ...
    def fallBack(gin: ExternalGraphInputNode): IOChecked[ResolvedExecutableInput] = gin match {
      case required: RequiredGraphInputNode =>
        s"Required workflow input '${required.nameInInputSet}' not specified".invalidIOChecked
    ...
    val providedInputsValidation: IOChecked[ResolvedExecutableInputs] = graph.inputNodes.toList
      .parTraverse[IOChecked, Option[(OutputPort, ResolvedExecutableInput)]] {
    ...
    val wantedInputsValidation: ErrorOr[Unit] = NonEmptyList.fromList(unwantedInputs.toList) match {
      ...
      case Some(unwanteds) =>
        Invalid(
          unwanteds.map(unwanted =>
            s"WARNING: Unexpected input provided: $unwanted (expected inputs: [${wantedInputs.mkString(", ")}])"
          )
        )
    }
