...
trait WdlStandardLibraryFunctions extends WdlFunctions[WomValue] {
  ...
  private def writeToTsv[A <: WomValue with TsvSerializable](functionName: String,
                                                             params: Seq[Try[WomValue]],
  ...
  def read_objects(params: Seq[Try[WomValue]]): Try[WomArray] = extractObjects("read_objects", params) map { a =>
    WomArray(WomArrayType(WomObjectType), a.toIndexedSeq)
  }
  def read_string(params: Seq[Try[WomValue]]): Try[WomString] =
    readContentsFromSingleFileParameter("read_string", params, fileSizeLimitationConfig.readStringLimit).map(s =>
      WomString(s.trim)
    )
  def read_json(params: Seq[Try[WomValue]]): Try[WomValue] = readContentsFromSingleFileParameter(
    "read_json",
    params,
    fileSizeLimitationConfig.readJsonLimit
  ).map(_.parseJson).flatMap(WomObjectType.coerceRawValue)
  def read_int(params: Seq[Try[WomValue]]): Try[WomInteger] =
    readContentsFromSingleFileParameter("read_int", params, fileSizeLimitationConfig.readIntLimit).map(s =>
      WomString(s.trim)
    ) map { s => WomInteger(s.value.trim.toInt) }
  def read_float(params: Seq[Try[WomValue]]): Try[WomFloat] =
    readContentsFromSingleFileParameter("read_float", params, fileSizeLimitationConfig.readFloatLimit).map(s =>
      WomString(s.trim)
    ) map { s => WomFloat(s.value.trim.toDouble) }

  def write_lines(params: Seq[Try[WomValue]]): Try[WomFile] =
    writeToTsv("write_lines", params, WomArray(WomArrayType(WomStringType), List.empty[WomValue]))
  def write_map(params: Seq[Try[WomValue]]): Try[WomFile] =
    writeToTsv("write_map", params, WomMap(WomMapType(WomStringType, WomStringType), Map.empty[WomValue, WomValue]))
  def write_object(params: Seq[Try[WomValue]]): Try[WomFile] =
    writeToTsv("write_object", params, WomObject(Map.empty[String, WomValue]))
  def write_objects(params: Seq[Try[WomValue]]): Try[WomFile] =
    writeToTsv("write_objects", params, WomArray(WomArrayType(WomObjectType), List.empty[WomObject]))
  def write_tsv(params: Seq[Try[WomValue]]): Try[WomFile] =
    writeToTsv("write_tsv", params, WomArray(WomArrayType(WomStringType), List.empty[WomValue]))
    ...
    extractObjects("read_object", params) map {
      ...
      case _ =>
        throw new IllegalArgumentException(
          "read_object yields an Object and thus can only read 2-rows TSV files. Try using read_objects instead."
        )
    }
  ...
  def read_boolean(params: Seq[Try[WomValue]]): Try[WomBoolean] =
    readContentsFromSingleFileParameter("read_boolean", params, fileSizeLimitationConfig.readBoolLimit).map(s =>
      WomString(s.trim)
    ) map { s => WomBoolean(java.lang.Boolean.parseBoolean(s.value.trim.toLowerCase)) }
  ...
  def basename(params: Seq[Try[WomValue]]): Try[WomString] = {
    ...
    val arguments: Try[(WomValue, WomValue)] = params.toList match {
      ...
      case _ =>
        val failures = params collect { case Failure(e) =>
          e
        }
  ...
  def floor(params: Seq[Try[WomValue]]): Try[WomInteger] =
    extractSingleArgument("floor", params) flatMap { f => WomFloatType.coerceRawValue(f) } map { f =>
      WomInteger(Math.floor(f.asInstanceOf[WomFloat].value).toInt)
    }

  def round(params: Seq[Try[WomValue]]): Try[WomInteger] =
    extractSingleArgument("round", params) flatMap { f => WomFloatType.coerceRawValue(f) } map { f =>
      WomInteger(Math.round(f.asInstanceOf[WomFloat].value).toInt)
    }

  def ceil(params: Seq[Try[WomValue]]): Try[WomInteger] =
    extractSingleArgument("ceil", params) flatMap { f => WomFloatType.coerceRawValue(f) } map { f =>
      WomInteger(Math.ceil(f.asInstanceOf[WomFloat].value).toInt)
    }
      ...
      case n =>
        Failure(
          new IllegalArgumentException(
            s"Invalid number of parameters for engine function transpose: $n. Ensure transpose(x: Array[Array[X]]) takes exactly 1 parameters."
          )
        )
    }
  ...
  def length(params: Seq[Try[WomValue]]): Try[WomInteger] = {
    def extractArguments: Try[WomValue] = params.size match {
      ...
      case n =>
        Failure(
          new IllegalArgumentException(
            s"Invalid number of parameters for engine function length(): $n. length() takes exactly 1 parameter."
          )
        )
    }
      ...
      case bad =>
        Failure(
          new UnsupportedOperationException(
            s"length() expects one parameter of type Array but got one parameter of type ${bad.womType.stableName}"
          )
        )
    }
  ...
  def flatten(params: Seq[Try[WomValue]]): Try[WomValue] = {
    def getFlatValues(v: WomValue): Try[Seq[WomValue]] = v match {
      ...
      case other =>
        Failure(
          new IllegalArgumentException(
            s"Invalid argument to flatten: ${other.womType.stableName}, flatten requires an Array[Array[_]]"
          )
        )
    }
    ...
    arg flatMap {
      ...
      case bad =>
        Failure(
          new UnsupportedOperationException(
            s"flatten() expects one parameter of type Array[Array[T]] but got one parameter of type ${bad.womType.stableName}"
          )
        )
    }
  }
  ...
  def prefix(params: Seq[Try[WomValue]]): Try[WomArray] = {
    ...
    val makePrefixedString = (prefixString: WomValue, elements: WomValue) =>
      (prefixString, elements) match {
        ...
        case (_, _) =>
          Failure(
            new UnsupportedOperationException(
              s"The function prefix expect arguments (String, Array[X]) where X is a primitive type, but got (${prefixString.womType.stableName}, ${elements.womType.stableName})"
            )
          )
      }
  ...
  def range(params: Seq[Try[WomValue]]): Try[WomArray] = {
    def extractAndValidateArguments = params.size match {
      ...
      case n =>
        Failure(
          new IllegalArgumentException(
            s"Invalid number of parameters for engine function range: $n. Ensure range(x: WdlInteger) takes exactly 1 parameters."
          )
        )
    }
    ...
    def validateArguments(value: Try[WomValue]) = value match {
      case Success(intValue: WomValue) if WomIntegerType.isCoerceableFrom(intValue.womType) =>
        Integer.valueOf(intValue.valueString) match {
          ...
          case n =>
            Failure(new IllegalArgumentException(s"Parameter to seq must be greater than or equal to 0 (but got $n)"))
        }
      case _ => Failure(new IllegalArgumentException(s"Invalid parameter for engine function seq: $value."))
    }

    extractAndValidateArguments map { intValue =>
      WomArray(WomArrayType(WomIntegerType), (0 until intValue).map(WomInteger))
    }
  }
  ...
  def sub(params: Seq[Try[WomValue]]): Try[WomString] = {
    def extractArguments = params.size match {
      ...
      case n =>
        Failure(
          new IllegalArgumentException(
            s"Invalid number of parameters for engine function sub: $n. sub takes exactly 3 parameters."
          )
        )
    }
  ...
  private val SelectFirstEmptyInput = Failure(
    new IllegalArgumentException("select_first failed. The input array was empty.")
  )
      ...
      if (arrayValue.isEmpty) SelectFirstEmptyInput
        ...
        (arrayValue collectFirst {
          ...
          case womValue if memberType.isCoerceableFrom(womValue.womType) =>
            memberType.coerceRawValue(womValue).get
        }).map(Success(_))
          .getOrElse(Failure(new IllegalArgumentException("select_first failed. All provided values were empty.")))
    case WomArray(WomArrayType(_), arrayValue) =>
      if (arrayValue.isEmpty) SelectFirstEmptyInput else Success(arrayValue.head)
    case other =>
      Failure(
        new IllegalArgumentException(
          s"select_first must take an array but got ${other.womType.stableName}: ${other.toWomString}"
        )
      )
  }
  ...
  def zip(params: Seq[Try[WomValue]]): Try[WomArray] = {
    val badArgsFailure = Failure(
      new IllegalArgumentException(
        s"Invalid parameters for engine function zip: $params. Requires exactly two evaluated array values of equal length."
      )
    )
  ...
  def cross(params: Seq[Try[WomValue]]): Try[WomArray] = {
    val badArgsFailure = Failure(
      new IllegalArgumentException(
        s"Invalid parameters for engine function cross: $params. Requires exactly two evaluated array values of equal length."
      )
    )
  ...
  /**
    ...
    * as a String
    */
  private def readContentsFromSingleFileParameter(functionName: String,
                                                  params: Seq[Try[WomValue]],
                                                  sizeLimit: Int
  ): Try[String] =
    for {
...
object WdlStandardLibraryFunctions {
  def fromIoFunctionSet(ioFunctionSet: IoFunctionSet, _fileSizeLimitationConfig: FileSizeLimitationConfig) =
    new WdlStandardLibraryFunctions {
      ...
      override def size(params: Seq[Try[WomValue]]): Try[WomFloat] = {
        ...
        def optionalSafeFileSize(value: WomValue): Try[Long] = value match {
          case f if f.isInstanceOf[WomSingleFile] || WomSingleFileType.isCoerceableFrom(f.womType) =>
            Try(Await.result(ioFunctionSet.size(f.valueString), Duration.Inf))
          case WomOptionalValue(_, Some(o)) => optionalSafeFileSize(o)
          case WomOptionalValue(f, None) if isOptionalOfFileType(f) => Success(0L)
          case _ =>
            Failure(
              new Exception(
                s"The 'size' method expects a 'File' or 'File?' argument but instead got ${value.womType.stableName}."
              )
            )
        }
        ...
        params match {
          ...
          case _ =>
            Failure(
              new UnsupportedOperationException(s"Expected one or two parameters but got ${params.length} instead.")
            )
        }
      }
  ...
  def assertEquallySizedArrays[A](values: (WomValue, WomValue),
                                  badArgsFailure: Failure[Nothing]
  ): Try[(WomArray, WomArray)] = values match {
    case (leftArray: WomArray, rightArray: WomArray) if leftArray.value.lengthCompare(rightArray.value.size) == 0 =>
      Success((leftArray, rightArray))
    case _ => badArgsFailure
  }

  def assertArrays(values: (WomValue, WomValue), badArgsFailure: Failure[Nothing]): Try[(WomArray, WomArray)] =
    values match {
...
trait PureStandardLibraryFunctionsLike extends WdlStandardLibraryFunctions {
  ...
  override def readFile(path: String, sizeLimit: Int): String = throw new UnsupportedOperationException(
    s"readFile not available in $className."
  )
  override def writeFile(path: String, content: String): Try[WomFile] = throw new UnsupportedOperationException(
    s"writeFile not available in $className."
  )
  override def read_json(params: Seq[Try[WomValue]]): Try[WomValue] = Failure(
    new UnsupportedOperationException(s"read_json not available in $className.")
  )
  override def write_json(params: Seq[Try[WomValue]]): Try[WomFile] = Failure(
    new UnsupportedOperationException(s"write_json not available in $className.")
  )
  override def size(params: Seq[Try[WomValue]]): Try[WomFloat] = Failure(
    new UnsupportedOperationException(s"size not available in $className.")
  )
  override def write_tsv(params: Seq[Try[WomValue]]): Try[WomFile] = Failure(
    new UnsupportedOperationException(s"write_tsv not available in $className.")
  )
  override def stdout(params: Seq[Try[WomValue]]): Try[WomFile] = Failure(
    new UnsupportedOperationException(s"stdout not available in $className.")
  )
  override def globHelper(pattern: String): Seq[String] = throw new UnsupportedOperationException(
    s"glob not available in $className."
  )
  override def stderr(params: Seq[Try[WomValue]]): Try[WomFile] = Failure(
    new UnsupportedOperationException(s"stderr not available in $className.")
  )
...
class WdlStandardLibraryFunctionsType extends WdlFunctions[WomType] {
  ...
  def size(params: Seq[Try[WomType]]): Try[WomType] = {
    ...
    params.toList match {
      ...
      case other =>
        Failure(
          new Exception(
            s"Unexpected arguments to function `size`. Expected 'size(file: File [, unit: String])' but got 'size(${other
                .map(_.map(_.stableName))
                .mkString(", ")})'"
          )
        )
    }
  ...
  def length(params: Seq[Try[WomType]]): Try[WomType] = params.toList match {
    ...
    case _ =>
      ...
      Failure(
        new Exception(
          s"Unexpected arguments to function `length`. `length` takes a parameter of type Array but got: $badArgs"
        )
      )
  }
  def prefix(params: Seq[Try[WomType]]): Try[WomType] = params.toList match {
    case Success(WomStringType) :: Success(WomArrayType(_: WomPrimitiveType)) :: Nil =>
      Success(WomArrayType(WomStringType))
    case _ =>
      val badArgs = params.mkString(", ")
      Failure(
        new Exception(
          s"Unexpected arguments to function `prefix`.  `prefix` takes parameters of type String and Array[<primitive>] but got: $badArgs"
        )
      )
  }
  ...
  def basename(params: Seq[Try[WomType]]): Try[WomType] = params.toList match {
    ...
    case Success(fType) :: Success(sType) :: Nil
        if WomStringType.isCoerceableFrom(fType) && WomStringType.isCoerceableFrom(sType) =>
  ...
  def select_first(params: Seq[Try[WomType]]): Try[WomType] = extractSingleArgument("select_first", params) flatMap {
    ...
    case other =>
      Failure(
        new IllegalArgumentException(
          s"select_first failed. It expects an array of optional values but got ${other.stableName}."
        )
      )
  }
  def select_all(params: Seq[Try[WomType]]): Try[WomType] = extractSingleArgument("select_all", params) flatMap {
    ...
    case other =>
      Failure(
        new IllegalArgumentException(
          s"select_all failed. It expects an array of optional values but got ${other.stableName}."
        )
      )
  }
  def defined(params: Seq[Try[WomType]]): Try[WomType] =
    extractSingleArgument("defined", params).map(_ => WomBooleanType)
  def zip(params: Seq[Try[WomType]]): Try[WomType] = {
    ...
    WdlStandardLibraryFunctions.extractTwoParams(params, badArgsFailure) flatMap {
      case (arrayType1: WomArrayType, arrayType2: WomArrayType) =>
        Success(WomArrayType(WomPairType(arrayType1.memberType, arrayType2.memberType)))
      case _ => badArgsFailure
    }
  }
  ...
  def flatten(params: Seq[Try[WomType]]): Try[WomType] = extractSingleArgument("flatten", params) flatMap {
    ...
    case otherType =>
      Failure(new Exception(s"flatten requires an Array[Array[_]] argument but instead got ${otherType.stableName}"))
  }
}
