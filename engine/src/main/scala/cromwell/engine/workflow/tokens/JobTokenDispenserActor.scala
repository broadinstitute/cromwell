...
class JobTokenDispenserActor(override val serviceRegistryActor: ActorRef,
  ...
  lazy val tokenEventLogger: TokenEventLogger = effectiveLogInterval match {
  ...
  private def distribute(n: Int): Unit = if (tokenQueues.nonEmpty) {
  ...
  private def release(actor: ActorRef): Unit = {
    tokenAssignments.get(actor) match {
      case Some(leasedToken) =>
        ...
        ()
      case None =>
        log.error("Job token returned from incorrect actor: {}", actor.path.name)
    }
  }
  ...
  private def onTerminate(terminee: ActorRef): Unit = {
    tokenAssignments.get(terminee) match {
      case Some(_) =>
        log.debug("Actor {} stopped without returning its Job Token. Reclaiming it!", terminee)
...
object JobTokenDispenserActor {
  ...
  def props(serviceRegistryActor: ActorRef, rate: DynamicRateLimiter.Rate, logInterval: Option[FiniteDuration],
    ...
    Props(new JobTokenDispenserActor(serviceRegistryActor, rate, logInterval, tokenTypeDescription, tokenAllocatedDescription)).
  ...
  implicit val tokenEncoder: Encoder.AsObject[JobTokenType] = deriveEncoder[JobTokenType]
