...
class SprayDockerRegistryApiClient()(implicit system: ActorSystem) extends DockerRegistryApiClient {
  ...
  override def getV1Token(taggedIdentifier: DockerTagIdentifier) = {
    ...
    pipeline(Get(taggedIdentifier.imagesUri)) map toDockerV1Token
  }

  private def toDockerV1Token(response: HttpResponse): DockerV1Token = {
  ...
  override def getV2TokenRequest(taggedIdentifier: DockerTagIdentifier) = {
    val pipeline: HttpRequest => Future[HttpResponse] = logSendReceive

    pipeline(Get(taggedIdentifier.manifestUri)) map toDockerV2TokenRequest
  }

  private def toDockerV2TokenRequest(response: HttpResponse): DockerV2TokenRequest = {
