...
import wom.graph.{CommandCallNode, Graph, WomIdentifier, WorkflowCallNode}
import wom.types.{WomStringType, WomType}
import wom.values.WomValue

object WomMocks {
  val EmptyTaskDefinition = CallableTaskDefinition(
    "emptyTask",
    Function.const(List.empty.validNel),
  ...
  def mockTaskCall(identifier: WomIdentifier, definition: CommandTaskDefinition = EmptyTaskDefinition) =
    CommandCallNode(identifier,
                    ...
                    List.empty,
                    Set.empty,
                    (_, localName) => WomIdentifier(localName = localName),
                    None
    )

  def mockWorkflowCall(identifier: WomIdentifier, definition: WorkflowDefinition = EmptyWorkflowDefinition) =
    WorkflowCallNode(identifier,
                     definition,
                     Set.empty,
                     List.empty,
                     Set.empty,
                     (_, localName) => identifier.combine(localName),
                     None
    )
  ...
  def mockOutputExpectations(outputs: Map[String, WomValue]): CallOutputs =
    CallOutputs(outputs.map { case (key, value) =>
      WomMocks.mockOutputPort(key, value.womType) -> value
    })
}
