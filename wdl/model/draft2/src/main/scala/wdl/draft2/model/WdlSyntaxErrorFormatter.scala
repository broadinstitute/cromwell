...
case class WdlSyntaxErrorFormatter(terminalMap: Map[Terminal, WorkflowSource]) extends SyntaxErrorFormatter {
  ...
  private def line(t: Terminal): String = getTerminal(t)
    .map(_.split("\n")(t.getLine - 1))
    .getOrElse(s"Cannot highlight line. It was probably in an imported file.")

  def unexpectedEof(method: String,
                    expected: java.util.List[TerminalIdentifier],
                    nt_rules: java.util.List[String]
  ): String = "ERROR: Unexpected end of file"
  ...
  def unexpectedSymbol(method: String,
                       actual: Terminal,
                       expected: java.util.List[TerminalIdentifier],
                       rule: String
  ): String = {
  ...
  def tooManyWorkflows(workflowAsts: java.util.List[Ast]): String = {
    val otherWorkflows = workflowAsts.asScala
      .map { ast =>
  ...
  def duplicateTask(taskAsts: Seq[Ast]): String = {
    ...
    s"""ERROR: Two tasks defined with the name '${taskAsts.head
        .getAttribute("name")
  ...
  def callReferencesAbsentTaskInput(callInputAst: Ast,
                                    taskAst: Ast,
                                    missingInput: String,
                                    callName: String,
                                    forSubworkflowCall: Boolean
  ): String = {
  ...
  def twoSiblingScopesHaveTheSameName(firstScopeType: String,
                                      firstScopeName: Terminal,
                                      secondScopeType: String,
                                      secondScopeName: Terminal
  ): String =
  ...
  def memberAccessReferencesAbsentCallOutput(memberAccessAst: Ast, call: WdlCall): String = {
    ...
    val goodOutputs =
      s" (current outputs of '$taskName': " + call.outputs.map("'" + _.unqualifiedName + "'").mkString(", ") + ")"
  ...
  def taskOutputExpressionTypeDoesNotMatchDeclaredType(outputName: Terminal,
                                                       outputType: WomType,
                                                       expressionType: WomType
  ) =
  ...
  def scatterCollectionContainsInvalidVariableReference(scatter: Scatter, variable: Terminal) =
    invalidVariableReference(variable, "value", "workflow")
