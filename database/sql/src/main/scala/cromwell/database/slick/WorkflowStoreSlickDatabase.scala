  ...
  override def fetchWorkflowsInState(limit: Int,
    ...
    def fetchAndUpdateStartableWfs(hogGroup: Option[String]) = {
      for {
        ...
        _ = if (workflowStoreEntries.isEmpty && shouldLog) logger.warn("Cromwell " + cromwellId + " found no workflow store entries to update for hog group " + hogGroup)
        _ <- DBIO.sequence(
    ...
    val action = for {
      ...
      workflowStoreEntries <- hogGroupOption match {
        ...
        case None =>
          if (shouldLog) logger.warn("Cromwell " + cromwellId + " found no hog groups with startable workflows")
