  ...
  def importAuthProvider(token: String): ImportAuthProvider = new GithubImportAuthProvider {
    override def authHeader(): Future[Map[String, String]] = {
      serviceRegistryActor
        ...
        .recoverWith {
          case e: AskTimeoutException => Future.failed(new Exception(s"Unable to resolve github auth token within allowed time", e))
          case e: Throwable =>
        ...
        case GithubAuthVendingFailure(error) => Future.failed(new Exception("Failed to resolve github auth token", error))
      }
    }
  }
}
