...
final case class GcsBatchCommandContext[T, U](request: GcsBatchIoCommand[T, U],
  ...
  /**
    ...
    */
  lazy val retryIn: Option[FiniteDuration] =
    if (currentAttempt >= maxAttemptsNumber) None else Option(backoff.backoffMillis milliseconds)
  ...
  lazy val callback: JsonBatchCallback[U] = new JsonBatchCallback[U]() {
    ...
    def onSuccess(response: U, httpHeaders: HttpHeaders): Unit =
      tryCallbackOrFail("onSuccessCallback", onSuccessCallback(response, httpHeaders))
    def onFailure(googleJsonError: GoogleJsonError, httpHeaders: HttpHeaders): Unit =
      tryCallbackOrFail(callbackName = "onFailureCallback", onFailureCallback(googleJsonError, httpHeaders))
  }
  ...
  private def onSuccessCallback(response: U, httpHeaders: HttpHeaders)(): Unit = {
    request.logIOMsgOverLimit(
      s"GcsBatchCommandContext.onSuccessCallback '${response.toPrettyElidedString(limit = 1000)}'"
    )
  ...
  private def onFailureCallback(googleJsonError: GoogleJsonError, httpHeaders: HttpHeaders)(): Unit = {
    request.logIOMsgOverLimit(
      s"GcsBatchCommandContext.onFailureCallback '${googleJsonError.toPrettyElidedString(limit = 1000)}'"
    )
