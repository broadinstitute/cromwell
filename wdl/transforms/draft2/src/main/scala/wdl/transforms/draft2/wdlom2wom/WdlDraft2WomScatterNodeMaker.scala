...
object WdlDraft2WomScatterNodeMaker extends WomScatterNodeMaker[Scatter] {
  override def toWomScatterNode(scatter: Scatter,
    ...
    val scatterCollectionExpressionNode =
      WdlWomExpression.toAnonymousExpressionNode(
        WomIdentifier(scatter.item),
        scatterCollectionExpression,
    ...
    val scatterItemTypeValidation = scatterCollectionExpression.evaluateType((localLookup ++ outerLookup).map {
      case (k, v) => k -> v.womType
    }) flatMap {
    ...
    for {
      ...
      g <- (scatter: Scope).toWomGraph(Set(womInnerGraphScatterVariableInput),
                                       localLookup ++ possiblyNeededNestedOginPorts,
                                       preserveIndexForOuterLookups = false,
                                       inASubworkflow
      )
