...
trait WdlStandardLibraryFunctions extends WdlFunctions[WomValue] {
  ...
  def length(params: Seq[Try[WomValue]]): Try[WomInteger] = {
    ...
    def arrayLength(value: WomValue): Try[WomInteger] = value match {
      ...
      case bad => Failure(new UnsupportedOperationException(s"length() expects one parameter of type Array but got one parameter of type ${bad.womType.callCachingName}"))
    }
  ...
  def flatten(params: Seq[Try[WomValue]]): Try[WomValue] = {
    def getFlatValues(v: WomValue): Try[Seq[WomValue]] = v match {
      ...
      case other => Failure(new IllegalArgumentException(s"Invalid argument to flatten: ${other.womType.callCachingName}, flatten requires an Array[Array[_]]"))
    }
    ...
    arg flatMap {
      ...
      case bad =>
        Failure(new UnsupportedOperationException(s"flatten() expects one parameter of type Array[Array[T]] but got one parameter of type ${bad.womType.callCachingName}"))
    }
  }
  ...
  def prefix(params: Seq[Try[WomValue]]): Try[WomArray] = {
    ...
    val makePrefixedString = (prefixString: WomValue, elements: WomValue) => (prefixString, elements) match {
      ...
      case (_, _) =>
        Failure(new UnsupportedOperationException(s"The function prefix expect arguments (String, Array[X]) where X is a primitive type, but got (${prefixString.womType.callCachingName}, ${elements.womType.callCachingName})"))
    }
  ...
  def select_first(params: Seq[Try[WomValue]]): Try[WomValue] = extractSingleArgument("select_first", params) flatMap {
    ...
    case other => Failure(new IllegalArgumentException(s"select_first must take an array but got ${other.womType.callCachingName}: ${other.toWomString}"))
  }
  ...
  def fromIoFunctionSet(ioFunctionSet: IoFunctionSet, _fileSizeLimitationConfig: FileSizeLimitationConfig) = new WdlStandardLibraryFunctions {
    ...
    override def size(params: Seq[Try[WomValue]]): Try[WomFloat] = {
        ...
        def optionalSafeFileSize(value: WomValue): Try[Long] = value match {
          ...
          case _ => Failure(new Exception(s"The 'size' method expects a 'File' or 'File?' argument but instead got ${value.womType.callCachingName}."))
        }
...
class WdlStandardLibraryFunctionsType extends WdlFunctions[WomType] {
  ...
  def size(params: Seq[Try[WomType]]): Try[WomType] = {
    ...
    params.toList match {
      ...
      case other => Failure(new Exception(s"Unexpected arguments to function `size`. Expected 'size(file: File [, unit: String])' but got 'size(${other.map(_.map(_.callCachingName)).mkString(", ")})'"))
    }
  }
  ...
  def select_first(params: Seq[Try[WomType]]): Try[WomType] = extractSingleArgument("select_first", params) flatMap {
    ...
    case other => Failure(new IllegalArgumentException(s"select_first failed. It expects an array of optional values but got ${other.callCachingName}."))
  }
  def select_all(params: Seq[Try[WomType]]): Try[WomType] = extractSingleArgument("select_all", params) flatMap {
    ...
    case other => Failure(new IllegalArgumentException(s"select_all failed. It expects an array of optional values but got ${other.callCachingName}."))
  }
  ...
  def flatten(params: Seq[Try[WomType]]): Try[WomType] = extractSingleArgument("flatten", params) flatMap {
    ...
    case otherType => Failure(new Exception(s"flatten requires an Array[Array[_]] argument but instead got ${otherType.callCachingName}"))
  }
}
