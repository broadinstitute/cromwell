...
if [[ -n ${CROMWELL_BRANCH} ]]; then
    ...
    sbt --error assembly >> "${ASSEMBLY_LOG}" 2>&1
    cd ..
...
if [[ "${CENTAUR_SBT_COVERAGE}" == "true" ]]; then
    sbt --error coverage centaur/it:compile
    CP=$(sbt --error coverage "export centaur/it:dependencyClasspath" -error)
else
    sbt --error centaur/it:compile
    CP=$(sbt --error "export centaur/it:dependencyClasspath" -error)
fi
