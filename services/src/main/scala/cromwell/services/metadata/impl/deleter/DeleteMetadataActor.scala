...
class DeleteMetadataActor(deleteMetadataConfig: DeleteMetadataConfig, override val serviceRegistryActor: ActorRef)
    extends Actor
    with ActorLogging
  ...
  private val deleterMetricsBasePath: NonEmptyList[String] =
    MetadataServiceActor.MetadataInstrumentationPrefix :+ "deleter"
  private val rowsDeletedMetricPath: NonEmptyList[String] = deleterMetricsBasePath :+ "rows_deleted"
  private val workflowsDeletedSuccessMetricPath: NonEmptyList[String] =
    deleterMetricsBasePath :+ "workflows_deleted" :+ "success"
  private val workflowsDeletedFailureMetricPath: NonEmptyList[String] =
    deleterMetricsBasePath :+ "workflows_deleted" :+ "failure"
  private val workflowDeleteTotalTimeMetricPath: NonEmptyList[String] =
    deleterMetricsBasePath :+ "workflow_delete_total_time"
  ...
  override def receive: Receive = {
    case DeleteNextWorkflowMessage =>
      ...
      deleteNextWorkflow().onComplete {
        ...
        case Success(false) =>
          ...
          if (deleteMetadataConfig.debugLogging)
            log.info(
              s"No archived workflows which finished over ${deleteMetadataConfig.delayAfterWorkflowCompletion} ago remain to be deleted. Scheduling next poll in ${deleteMetadataConfig.backoffInterval}."
            )
        case Failure(error) =>
          count(rowsDeletedMetricPath, 0L, ServicesPrefix)
          count(workflowsDeletedSuccessMetricPath, 0L, ServicesPrefix)
    ...
    case other =>
      log.info(
        s"Programmer Error! The DeleteMetadataSchedulerActor received unexpected message! (${sender()} sent ${other.toPrettyElidedString(1000)}})"
      )
  }

  def workflowsLeftToDeleteMetric(): Unit = {
    val currentTimestampMinusDelay =
      OffsetDateTime.now().minusSeconds(deleteMetadataConfig.delayAfterWorkflowCompletion.toSeconds)
    countWorkflowsLeftToDeleteThatEndedOnOrBeforeThresholdTimestamp(currentTimestampMinusDelay).onComplete {
      case Success(workflowsToDelete) =>
        ...
        context.system.scheduler.scheduleOnce(deleteMetadataConfig.instrumentationInterval)(
          workflowsLeftToDeleteMetric()
        )
      case Failure(exception) =>
        log.error(
          exception,
          s"Something went wrong while fetching number of workflows left to delete. " +
  ...
  def deleteNextWorkflow(): Future[Boolean] = for {
    ...
    result <- maybeWorkflowId match {
      case Some(id) =>
        ...
        for {
          rowsDeleted <- deleteAllMetadataEntriesForWorkflowAndUpdateArchiveStatus(
            id,
            MetadataArchiveStatus.toDatabaseValue(ArchivedAndDeleted)
          )
...
object DeleteMetadataActor {
  ...
  def props(config: DeleteMetadataConfig, serviceRegistryActor: ActorRef): Props = Props(
    new DeleteMetadataActor(config, serviceRegistryActor)
  )
}
