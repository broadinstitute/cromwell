...
import wdl.transforms.biscayne.linking.expression.values.cascadesValueEvaluators._
  ...
  implicit val expressionEvaluator: ValueEvaluator[ExpressionElement] = new ValueEvaluator[ExpressionElement] {
    override def evaluateValue(a: ExpressionElement,
      ...
      a match {
        ...
        case a: CollectByKey =>
          a.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)(expressionValueEvaluator)
        case a: Sep => a.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)(expressionValueEvaluator)
