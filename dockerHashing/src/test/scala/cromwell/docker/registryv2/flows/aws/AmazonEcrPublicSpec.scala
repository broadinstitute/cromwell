...
import software.amazon.awssdk.services.ecrpublic.model.{
  AuthorizationData,
...
class AmazonEcrPublicSpec
  ...
  val mediaType: MediaType =
    MediaType.parse(DockerRegistryV2Abstract.DockerManifestV2MediaType).getOrElse(fail("Cant parse mediatype"))
  ...
  it should "have public.ecr.aws as registryHostName" in {
    ...
    registry invokePrivate registryHostNameMethod(
      DockerImageIdentifier.fromString(goodUri).get
    ) shouldEqual "public.ecr.aws"
  }
