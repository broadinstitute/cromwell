...
class ArchiveMetadataSchedulerActor(archiveMetadataConfig: ArchiveMetadataConfig,
  ...
  private val archiverMetricsBasePath: NonEmptyList[String] =
    MetadataServiceActor.MetadataInstrumentationPrefix :+ "archiver"
  ...
  private val workflowsProcessedSuccessMetricPath: NonEmptyList[String] =
    archiverMetricsBasePath :+ "workflows_processed" :+ "success"
  private val workflowsProcessedFailureMetricPath: NonEmptyList[String] =
    archiverMetricsBasePath :+ "workflows_processed" :+ "failure"
  private val timeBehindExpectedDelayMetricPath: NonEmptyList[String] =
    archiverMetricsBasePath :+ "time_behind_expected_delay"
  private val workflowArchiveTotalTimeMetricPath: NonEmptyList[String] =
    archiverMetricsBasePath :+ "workflow_archive_total_time"
  ...
  private val TerminalWorkflowStatuses: List[String] =
    List(WorkflowSucceeded, WorkflowAborted, WorkflowFailed).map(_.toString)
  ...
  override def receive: Receive = {
      ...
      archiveNextWorkflows.onComplete {
        case Success(cnt) =>
          ...
          if (cnt > 0) {
            ...
            if (archiveMetadataConfig.debugLogging)
              log.info(
                s"No complete workflows which finished over ${archiveMetadataConfig.archiveDelay} ago remain to be archived. Scheduling next poll in ${archiveMetadataConfig.backoffInterval}."
              )
    ...
    case other =>
      log.info(
        s"Programmer Error! The ArchiveMetadataSchedulerActor received unexpected message! (${sender()} sent ${other.toPrettyElidedString(1000)}})"
      )
  }
  ...
  def workflowsLeftToArchiveMetric(): Unit = {
    ...
    countWorkflowsLeftToArchiveThatEndedOnOrBeforeThresholdTimestamp(
      ...
      case Success(workflowsToArchive) =>
        ...
        context.system.scheduler.scheduleOnce(archiveMetadataConfig.instrumentationInterval)(
          workflowsLeftToArchiveMetric()
        )
      case Failure(exception) =>
        log.error(
          exception,
          s"Something went wrong while fetching number of workflows left to archive. " +
  ...
  def archiveNextWorkflows: Future[Long] = {
    ...
    for {
      ...
      _ = sendTiming(archiverTimingMetricsBasePath :+ "lookup_next_workflows",
                     calculateTimeDifference(batchLookupStartTime, batchLookupEndTime),
                     ServicesPrefix
      )
      _ = if (archiveMetadataConfig.debugLogging)
        log.info(s"About to archive batch of ${workflowSummaryEntries.size} workflows.")
      result <- archiveSummaryEntries(workflowSummaryEntries)
      _ = sendTiming(archiverTimingMetricsBasePath :+ "batch_lookup_and_archive_time",
                     calculateTimeDifference(batchLookupStartTime, OffsetDateTime.now()),
                     ServicesPrefix
      )
    } yield result
  }
  ...
  private def archiveSummaryEntry(entry: WorkflowMetadataSummaryEntry): Future[Long] = {
    entry.endTimestamp.foreach { workflowEndTime =>
      sendGauge(
        timeBehindExpectedDelayMetricPath,
        calculateTimeSince(
          workflowEndTime.toSystemOffsetDateTime
        ).toMillis - archiveMetadataConfig.archiveDelay.toMillis,
        ServicesPrefix
      )
    }
    ...
    for {
      ...
      _ = sendTiming(archiverTimingMetricsBasePath :+ "prepare_to_stream",
                     calculateTimeDifference(archiveStartTime, readyToStreamTime),
                     ServicesPrefix
      )
      _ <- streamMetadataToGcs(path, dbStream)
      streamCompleteTime = OffsetDateTime.now()
      _ = sendTiming(archiverTimingMetricsBasePath :+ "stream_to_gcs",
                     calculateTimeDifference(readyToStreamTime, streamCompleteTime),
                     ServicesPrefix
      )
      _ <- updateMetadataArchiveStatus(WorkflowId(UUID.fromString(workflowId)), Archived)
      statusUpdatedTime = OffsetDateTime.now()
      _ = sendTiming(archiverTimingMetricsBasePath :+ "archive_status_update",
                     calculateTimeDifference(streamCompleteTime, statusUpdatedTime),
                     ServicesPrefix
      )
      _ = sendTiming(workflowArchiveTotalTimeMetricPath,
                     calculateTimeDifference(archiveStartTime, statusUpdatedTime),
                     ServicesPrefix
      )
  ...
  def fetchStreamFromDatabase(workflowId: WorkflowId): Future[DatabasePublisher[MetadataEntry]] =
    (serviceRegistryActor ? GetMetadataStreamAction(workflowId)) flatMap {
      ...
      case MetadataLookupStreamFailed(_, reason) =>
        Future.failed(new Exception(s"Failed to get metadata stream", reason))
  ...
  def streamMetadataToGcs(path: Path, stream: DatabasePublisher[MetadataEntry]): Future[Unit] = {
    ...
    val rowsCounter = new CounterAndProgressiveLogger(
      logFunction = (newRows, totalRows) => {
        if (archiveMetadataConfig.debugLogging)
          logger.info(s"Uploaded $newRows new rows to ${path.pathAsString}. Total uploaded is now ${totalRows}")
        else ()
    ...
    for {
      ...
      _ = sendTiming(archiverStreamTimingMetricsBasePath :+ "get_async_io",
                     calculateTimeDifference(streamStartTime, gotAsyncIoTime),
                     ServicesPrefix
      )
      gcsStream = Files.newOutputStream(path.nioPath, StandardOpenOption.CREATE)
      gcsStreamCreatedTime = OffsetDateTime.now()
      _ = sendTiming(archiverStreamTimingMetricsBasePath :+ "create_gcs_stream",
                     calculateTimeDifference(gotAsyncIoTime, gcsStreamCreatedTime),
                     ServicesPrefix
      )
      ...
      _ = sendTiming(archiverStreamTimingMetricsBasePath :+ "create_csv_printer",
                     calculateTimeDifference(gcsStreamCreatedTime, csvPrinterCreatedTime),
                     ServicesPrefix
      )
      _ <- stream.foreach { me =>
      ...
      _ = sendTiming(archiverStreamTimingMetricsBasePath :+ "stream_data_to_gcs",
                     calculateTimeDifference(csvPrinterCreatedTime, streamingCompleteTime),
                     ServicesPrefix
      )
      ...
      _ <-
        if (uploadedChecksum == expectedChecksum) Future.successful(())
        else
          Future.failed(
            new Exception(
              s"Uploaded checksum '$uploadedChecksum' did not match local calculation ('$expectedChecksum')"
            )
          )
    } yield ()
  }
  ...
  final class ByteCountingOutputStream() extends OutputStream {
    val byteCounter = new CounterAndProgressiveLogger(
      logFunction = (newBytes, totalBytes) => {
        if (archiveMetadataConfig.debugLogging)
          logger.info(s"Uploaded $newBytes new bytes. Total uploaded is now $totalBytes")
        else ()
