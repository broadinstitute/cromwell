...
abstract class ExpressionNode(override val identifier: WomIdentifier,
                              ...
                              val womType: WomType,
                              val inputMapping: Map[String, InputPort]
) extends GraphNodeWithSingleOutputPort
  ...
  final override def evaluate(outputPortLookup: OutputPort => ErrorOr[WomValue],
...
object ExpressionNode {
  ...
  /**
    ...
    */
  type ExpressionNodeConstructor[E <: ExpressionNode] =
    (WomIdentifier, WomExpression, WomType, Map[String, InputPort]) => E
  ...
  /**
    ...
    * output ports.
    */
  def buildFromConstructor[E <: ExpressionNode](
    constructor: ExpressionNodeConstructor[E]
    ...
    def safeSourceString(e: WomExpression) =
      Try(expression.sourceString).getOrElse("<<expected an expression, none found>>")

    builtExpressionNode.leftMap(
      _.map(e =>
        s"Cannot build expression for '${identifier.fullyQualifiedName.value} = ${safeSourceString(expression)}': $e"
      )
    )
  }
  ...
  /**
    ...
    */
  private def linkWithInputs(graphNodeSetter: GraphNode.GraphNodeSetter[ExpressionNode],
                             expression: WomExpression,
                             inputMapping: Map[String, OutputPort]
  ): ErrorOr[(WomType, Map[String, InputPort])] = {
    def linkInput(input: String): ErrorOr[(String, InputPort)] = inputMapping.get(input) match {
      case Some(upstreamPort) =>
        (input, ConnectedInputPort(input, upstreamPort.womType, upstreamPort, graphNodeSetter.get)).validNel
      case None =>
