...
object CromwellBuild extends Build {
  ...
  lazy val root = Project(id = "Cromwell",
    base = file(".")).aggregate(CromwellBackend).dependsOn(CromwellBackend)
 ...
 */
  ...
  lazy val CromwellBackend = Project(id = "CromwellBackend",
    base = file("cromwell-backend"))

  CromwellBackend.disablePlugins(sbtassembly.AssemblyPlugin)
