...
trait SharedFileSystem {
  ...
  def useCachedCall(cachedBackendCall: LocalFileSystemBackendCall, backendCall: LocalFileSystemBackendCall)(implicit ec: ExecutionContext): Future[ExecutionHandle] = Future {
    ...
    outputs match {
      case Success(o) =>
        cachedBackendCall.hash map { h => CompletedExecutionHandle(SuccessfulBackendCallExecution(o, Seq.empty[ExecutionEventEntry], ScriptReturnCode(cachedBackendCall.returnCode.contentAsString.stripLineEnd.toInt), h, Option(cachedBackendCall))) }
