...
class WdlPairTypeSpec extends FlatSpec with Matchers {
  ...
  coerceables foreach { case (fromValue, toType, coercedValue) =>
    ...
    it should s"${notString}allow coercion from ${fromValue.womType.callCachingName} to ${toType.callCachingName}" in {
      toType.isCoerceableFrom(fromValue.womType) should be(coercionDefined)
    }

    it should s"generate the expected result when converting from ${fromValue.toWomString} to ${toType.callCachingName}" in {
