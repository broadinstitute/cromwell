...
class DrsResolverSpec extends AnyFlatSpec with CromwellTimeoutSpec with Matchers {
  ...
  it should "find DRS path from a localization path" in {
    ...
    DrsResolver.getContainerRelativePath(drsPath).unsafeRunSync() should be(
      MockDrsPaths.gcsRelativePathWithFileNameFromLocalizationPath
    )
  }
  ...
  it should "find DRS path from all the paths" in {
    ...
    DrsResolver.getContainerRelativePath(drsPath).unsafeRunSync() should be(
      MockDrsPaths.gcsRelativePathWithFileNameFromAllThePaths
    )
  }
