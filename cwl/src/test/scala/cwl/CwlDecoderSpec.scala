...
class CwlDecoderSpec extends AnyFlatSpec with CromwellTimeoutSpec with Matchers {
  ...
  it should "fail to parse broken linked cwl" in {
    ...
    withClue(s"Actual message: ${result.swap.toOption.get.head}") {
      "Field `run` contains undefined reference to `file://.+/wrong.cwl`".r.findAllIn(result.swap.toOption.get.head).size shouldBe 1
      "Field `run` contains undefined reference to `file://.+/wrong2.cwl`".r.findAllIn(result.swap.toOption.get.head).size shouldBe 1
    }
  }
