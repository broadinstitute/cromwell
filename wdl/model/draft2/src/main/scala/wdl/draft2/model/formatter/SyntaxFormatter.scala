...
class SyntaxFormatter(highlighter: SyntaxHighlighter = NullSyntaxHighlighter) {
  ...
  def format(namespace: WdlNamespace): String = {
    ...
    val taskDefinitions = namespaceDefinitions collect {
      case a: Ast if a.getName == "Task" =>
        formatTask(
          namespace
            .findTask(a.getAttribute("name").sourceString)
            .getOrElse(throw new UnsupportedOperationException("Shouldn't happen"))
        )
    }

    val workflowDefinitions = namespace match {
      case n: WdlNamespaceWithWorkflow =>
        namespaceDefinitions collect { case a: Ast if a.getName == "Workflow" => formatWorkflow(n.workflow) }
  ...
  private def formatMetaSection(section: String, attrs: Map[String, String], level: Int): String =
    attrs match {
      case m: Map[String, String] if m.nonEmpty =>
        ...
        indent(s"""${highlighter.keyword(section)} {
                  |${wdlAttrs.mkString("\n")}
                  |}""".stripMargin,
               level
        )
  ...
  private def formatRuntimeSection(runtimeAttributes: WdlRuntimeAttributes, level: Int): String =
    runtimeAttributes.attrs match {
      case m if m.nonEmpty =>
        ...
        indent(s"""${highlighter.keyword("runtime")} {
                  |${attrs.mkString("\n")}
                  |}""".stripMargin,
               level
        )
  ...
  private def formatOutput(output: TaskOutput, level: Int): String =
    indent(s"${highlighter.womType(output.womType)} ${highlighter
               .variable(output.unqualifiedName)} = ${output.requiredExpression.toString(highlighter)}",
           level
    )
  ...
  private def formatCall(call: WdlTaskCall, level: Int): String = {
    ...
    if (call.inputMappings.isEmpty) {
      ...
      val inputString = call.inputMappings
        .map { case (k, v) =>
