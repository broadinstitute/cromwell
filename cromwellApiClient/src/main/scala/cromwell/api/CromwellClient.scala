...
class CromwellClient(val cromwellUrl: URL,
  ...
  def outputsEndpoint(workflowId: WorkflowId, args: Option[Map[String, List[String]]] = None): Uri =
    workflowSpecificGetEndpoint(workflowsEndpoint, workflowId, "outputs", args)
  def labelsEndpoint(workflowId: WorkflowId): Uri = workflowSpecificGetEndpoint(workflowsEndpoint, workflowId, "labels")
  def logsEndpoint(workflowId: WorkflowId, args: Option[Map[String, List[String]]] = None): Uri =
    workflowSpecificGetEndpoint(workflowsEndpoint, workflowId, "logs", args)
  def diffEndpoint(workflowA: WorkflowId,
                   callA: String,
                   indexA: ShardIndex,
  ...
  def submit(workflow: WorkflowSubmission)(implicit ec: ExecutionContext): FailureResponseOrT[SubmittedWorkflow] = {
    ...
    makeRequest[CromwellStatus](
      HttpRequest(HttpMethods.POST, submitEndpoint, List.empty[HttpHeader], requestEntity)
    ) map { status =>
  ...
  def describe(
    workflow: WorkflowDescribeRequest
  ...
  def submitBatch(
    workflow: WorkflowBatchSubmission
    ...
    val submissionSet = workflow.inputsBatch.map(inputs =>
    ...
    makeRequest[List[CromwellStatus]](
      HttpRequest(HttpMethods.POST, batchSubmitEndpoint, List.empty[HttpHeader], requestEntity)
    ) map { statuses =>
  ...
  def outputs(workflowId: WorkflowId, args: Option[Map[String, List[String]]] = None)(implicit
    ec: ExecutionContext
  ): FailureResponseOrT[WorkflowOutputs] =
    simpleRequest[WorkflowOutputs](outputsEndpoint(workflowId, args))

  def labels(workflowId: WorkflowId, headers: List[HttpHeader] = defaultHeaders)(implicit
    ec: ExecutionContext
  ): FailureResponseOrT[WorkflowLabels] =
    simpleRequest[WorkflowLabels](labelsEndpoint(workflowId), headers = headers)

  def addLabels(workflowId: WorkflowId, newLabels: List[Label], headers: List[HttpHeader] = defaultHeaders)(implicit
    ec: ExecutionContext
  ): FailureResponseOrT[WorkflowLabels] = {
  ...
  def logs(workflowId: WorkflowId, args: Option[Map[String, List[String]]] = None)(implicit
    ec: ExecutionContext
  ): FailureResponseOrT[WorkflowMetadata] =
  ...
  private[api] def executeRequest(request: HttpRequest, headers: List[HttpHeader]) =
  ...
  /**
    ...
    */
  private def makeRequest[A](request: HttpRequest, headers: List[HttpHeader] = defaultHeaders)(implicit
  ...
  private def simpleRequest[A](uri: Uri,
                               ...
                               headers: List[HttpHeader] = defaultHeaders
  )(implicit um: Unmarshaller[ResponseEntity, A], ec: ExecutionContext): FailureResponseOrT[A] =
...
object CromwellClient {
  implicit final class EnhancedHttpResponse(val response: HttpResponse) extends AnyVal {
  ...
  private[api] def requestEntityForSubmit(workflowSubmission: WorkflowSubmission): MessageEntity = {
    ...
    ) collect { case (name, Some(source: String)) =>
  ...
  /**
    ...
    */
  private[api] def workflowSpecificGetEndpoint(submitEndpoint: String,
    ...
    args.getOrElse(Map.empty).foreach { case (key, l) =>
      l.foreach(v => queryBuilder.+=(key -> v))
    }
