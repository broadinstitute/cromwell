...
object WomtoolCommandLineParser {
  ...
  def validateCommandLine(args: PartialWomtoolCommandLineArguments): Option[ValidatedWomtoolCommandLine] = args match {
    ...
    case PartialWomtoolCommandLineArguments(Some(Upgrade), Some(mainFile), None, None, None, None) =>
      Option(WomtoolWdlUpgradeCommandLine(mainFile))
    case _ => None
  }
}
...
class WomtoolCommandLineParser extends scopt.OptionParser[PartialWomtoolCommandLineArguments]("java -jar womtool.jar") {
  ...
  cmd("validate")
    ...
    .text(
      "Validate a workflow source file. If inputs are provided then 'validate' also checks that the inputs file is a valid set of inputs for the workflow." + System.lineSeparator
    )

  cmd("inputs")
    .action((_, c) => c.copy(command = Option(Inputs)))
    .text("Generate and output a new inputs JSON for this workflow." + System.lineSeparator)

  cmd("outputs")
    .action((_, c) => c.copy(command = Option(Outputs)))
  ...
  cmd("parse")
    ...
    .text(
      "(Deprecated; WDL draft 2 only) Print out the Hermes parser's abstract syntax tree for the source file." + System.lineSeparator
    )

  cmd("highlight")
    .action((_, c) => c.copy(command = Option(Highlight)))
    .text(
      "(Deprecated; WDL draft 2 only) Print out the Hermes parser's abstract syntax tree for the source file. Requires at least one of 'html' or 'console'" + System.lineSeparator
    )
