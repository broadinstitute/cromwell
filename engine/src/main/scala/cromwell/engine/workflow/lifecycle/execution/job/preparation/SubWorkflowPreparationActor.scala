...
class SubWorkflowPreparationActor(workflowDescriptor: EngineWorkflowDescriptor,
  ...
  /**
    ...
    */
  private def evaluateStartingKnownValues(inputEvaluation: WomEvaluatedCallInputs, workflowInputs: Set[GraphInputNode]): ErrorOr[Map[OutputPort, WomValue]] = {
    ...
    NonEmptyList.fromList((workflowInputs.toSet[GraphNode] diff providedInputs.keySet.map(_.graphNode) diff optionalsAndDefaults).toList) match {
      case None => Valid(providedInputs)
      case Some(missingNodeNel) => Invalid(missingNodeNel map (n => s"Couldn't find starting value for subworkflow input: ${n.identifier.localName}"))
    }
  }
