...
sealed abstract class CentaurTestFormat(val name: String) {
  ...
  def testSpecString: String = this match {
    ...
    case _: PapiUpgradeTest =>
      "make sure a PAPI upgrade preserves call caching when the `name-for-call-caching-purposes` attribute is used"
    ...
    case _: WorkflowFailureRestartWithRecover =>
      "survive a Cromwell restart when a workflow was failing and recover jobs"
...
object CentaurTestFormat {
  ...
  sealed trait WithCallMarker { this: CentaurTestFormat =>
    val build: CallMarker => CentaurTestFormat
  }
  ...
  case object RunFailingTwiceExpectingNoCallCachingTest
      extends CentaurTestFormat("RunFailingTwiceExpectingNoCallCaching")
  ...
  case class CromwellRestartWithRecover(callMarker: CallMarker)
      extends CentaurTestFormat(CromwellRestartWithRecover.name)
      with RestartFormat
  ...
  case class CromwellRestartWithoutRecover(callMarker: CallMarker)
      extends CentaurTestFormat(CromwellRestartWithoutRecover.name)
      with RestartFormat

  object ScheduledAbort extends CentaurTestFormat("ScheduledAbort") with WithCallMarker {
    val build = ScheduledAbort.apply _
  }
  case class ScheduledAbort(callMarker: CallMarker)
      extends CentaurTestFormat(ScheduledAbort.name)
      with SequentialTestFormat
  ...
  case class ScheduledAbortWithRestart(callMarker: CallMarker)
      extends CentaurTestFormat(ScheduledAbortWithRestart.name)
      with RestartFormat

  object WorkflowFailureRestartWithRecover
      extends CentaurTestFormat("WorkflowFailureRestartWithRecover")
      with WithCallMarker {
    val build = WorkflowFailureRestartWithRecover.apply _
  }
  case class WorkflowFailureRestartWithRecover(callMarker: CallMarker)
      extends CentaurTestFormat(WorkflowFailureRestartWithRecover.name)
      with RestartFormat

  object WorkflowFailureRestartWithoutRecover
      extends CentaurTestFormat("WorkflowFailureRestartWithoutRecover")
      with WithCallMarker {
    val build = WorkflowFailureRestartWithoutRecover.apply _
  }
  case class WorkflowFailureRestartWithoutRecover(callMarker: CallMarker)
  ...
  private def fromString(testFormat: String, callMarker: Option[CallMarker]): Checked[CentaurTestFormat] = {
    def withCallMarker(name: String, constructor: CallMarker => CentaurTestFormat) = callMarker match {
      ...
      case None =>
        s"$name needs a callMarker to know on which call to trigger the restart".invalidNelCheck[CentaurTestFormat]
    }
    ...
    List(
      ...
      case format: WithCallMarker if format.name.equalsIgnoreCase(testFormat) =>
        withCallMarker(format.name, format.build)
