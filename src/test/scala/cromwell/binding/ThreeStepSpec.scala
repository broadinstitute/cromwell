...
class ThreeStepSpec extends FlatSpec with Matchers {
  ...
  it should "Have a task with name 'wc'" in {
    ...
    task.inputs shouldEqual Vector(TaskInput("in_file", List(WdlFileType), postfixQuantifier=None))
  ...
  it should "Have a task with name 'cgrep'" in {
    ...
    task.inputs shouldEqual Vector(
      TaskInput("pattern", List(WdlStringType), postfixQuantifier=None),
      TaskInput("in_file", List(WdlFileType), postfixQuantifier=None)
    )
