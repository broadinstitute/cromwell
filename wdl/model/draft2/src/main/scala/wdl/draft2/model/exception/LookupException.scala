...
/**
  ...
  */
sealed abstract case class VariableNotFoundException(variable: String, quoteName: Boolean = true)
    extends Exception(s"Variable $variable not found")
    with LookupException
...
object VariableNotFoundException {
  ...
  def apply(variable: String, variableType: WomType): VariableNotFoundException = new VariableNotFoundException(
    s"'$variable': ${variableType.stableName}"
  ) {}
  def apply(declaration: Declaration): VariableNotFoundException =
    VariableNotFoundException.apply(declaration.fullyQualifiedName, declaration.womType)
}
...
/**
  ...
  */
class VariableLookupException(override val exceptionContext: String,
                              override val throwables: List[Throwable] = List.empty
...
/**
  ...
  */
final case class OutputVariableLookupException(node: WdlGraphNode, index: Option[Int])
    extends VariableLookupException(s"Could not find outputs for call ${node.fullyQualifiedName} at index $index")
