...
object RetryableBackendJobExecutionActor {
  def props(jobDescriptor: BackendJobDescriptor, configurationDescriptor: BackendConfigurationDescriptor) = Props(
    RetryableBackendJobExecutionActor(jobDescriptor, configurationDescriptor)
  )
}

final case class RetryableBackendJobExecutionActor(override val jobDescriptor: BackendJobDescriptor,
  ...
  override def execute: Future[BackendJobExecutionResponse] =
    if (jobDescriptor.key.attempt < attempts) {
      Future.successful(
        JobFailedRetryableResponse(jobDescriptor.key, new RuntimeException("An apparent transient Exception!"), None)
      )
    } else {
      Future.successful(
        JobFailedNonRetryableResponse(jobDescriptor.key,
                                      new RuntimeException("A permanent Exception! Yikes, what a pickle!"),
                                      None
        )
      )
    }
