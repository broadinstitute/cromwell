...
class WomArrayTypeSpec extends FlatSpec with Matchers  {
  ...
  List(WomStringType, WomArrayType(WomIntegerType), WomPairType(WomIntegerType, WomPairType(WomIntegerType, WomIntegerType)), WomOptionalType(WomStringType)) foreach { desiredMemberType =>
    it should s"be able to construct an empty Array[${desiredMemberType.callCachingName}] value" in {
    ...
    it should s"be able to coerce ${arrayType.callCachingName} to ${optionalArrayType.callCachingName} and vice versa" in {
  ...
  List(WomNonEmptyArrayType(WomIntegerType), WomOptionalType(WomNonEmptyArrayType(WomIntegerType))) foreach { targetType =>
    it should s"be able to coerce an array literal into ${targetType.callCachingName}" in {
      ...
      targetType.coerceRawValue(evaluatedLiteral) match {
        ...
        case Failure(e) => fail(s"Unable to coerce $evaluatedLiteral (${evaluatedLiteral.womType.callCachingName}) into ${targetType.callCachingName}", e)
      }
    }
  }
