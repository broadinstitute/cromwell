...
class JesBackend extends Backend with LazyLogging with ProductionJesAuthentication with ProductionJesConfiguration {
  ...
  override def cleanUpForWorkflow(workflow: WorkflowDescriptor)(implicit ec: ExecutionContext): Future[Unit] = authenticated { connection =>
    ...
    withRetry(gcsCheckAuthFileExists, s"Workflow ${workflow.shortId} - failed to query for auth file: $gcsAuthFile") match {
      case Success(exists) if exists =>
        withRetry(gcsAttemptToDeleteObject, s"Workflow ${workflow.shortId} - failed to delete auth file: $gcsAuthFile") match {
          case Success(_) =>
            Future.successful(Unit)
          case Failure(ex) =>
            logger.error(s"Workflow ${workflow.shortId} - Could not delete the auth file $gcsAuthFile", ex)
            Future.failed(ex)
        }
      case Failure(ex) =>
        logger.error(s"Workflow ${workflow.shortId} - Could not query for the existence of the auth file $gcsAuthFile", ex)
        Future.failed(ex)
