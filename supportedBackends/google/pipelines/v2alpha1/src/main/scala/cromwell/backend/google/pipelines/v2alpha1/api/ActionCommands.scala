...
object ActionCommands {
  ...
  def delocalizeDirectory(containerPath: Path, cloudPath: Path, contentType: Option[ContentType]) = {
    s"gsutil -u ${cloudPath.projectId} ${contentType |> makeContentTypeFlag} -m rsync -r ${containerPath.escape} ${cloudPath.escape}"
  }
  ...
  def delocalizeFile(containerPath: Path, cloudPath: Path, contentType: Option[ContentType]) = {
    s"gsutil -u ${cloudPath.projectId} ${contentType |> makeContentTypeFlag} cp ${containerPath.escape} ${cloudPath.parent.escape.ensureSlashed}"
  }
  ...
  def delocalizeFileTo(containerPath: Path, cloudPath: Path, contentType: Option[ContentType]) = {
    s"gsutil -u ${cloudPath.projectId} ${contentType |> makeContentTypeFlag} cp ${containerPath.escape} ${cloudPath.escape}"
  }
  ...
  def localizeDirectory(cloudPath: Path, containerPath: Path) = {
    s"${containerPath |> makeContainerDirectory} && gsutil -u ${cloudPath.projectId} -m rsync -r ${cloudPath.escape} ${containerPath.escape}"
  }

  def localizeFile(cloudPath: Path, containerPath: Path) = {
    s"gsutil -u ${cloudPath.projectId} cp ${cloudPath.escape} ${containerPath.escape}"
  }
}
