...
case class WomCoproductType(types: NonEmptyList[WomType]) extends WomType {
  ...
  override def coercion: PartialFunction[Any, WomValue] = {
    ...
    case any =>
      ...
      triedToCoerce.getOrElse(
        throw new WomTypeException(
          s"unable to coerce $any to a member of the set of types ${types.toList.mkString(", ")}"
        )
      ) |> (WomCoproductValue(this, _))
  }
      ...
      case _ =>
        Failure(
          new WomExpressionException(
            s"Type equality assertion failed because $tpe was not found in the coproduct of ${stableName}"
          )
        )
    }
  ...
  override def equalsType(rhs: WomType): Try[WomType] =
    rhs match {
      ...
      case _ =>
        Failure(
          new WomExpressionException(
            s"Type equality could not be asserted because $rhs was found in the coproduct of $stableName"
          )
        )
    }
