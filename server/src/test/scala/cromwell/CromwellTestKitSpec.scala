...
import cromwell.engine.workflow.lifecycle.execution.callcaching.CallCacheReadActor.{
  CacheLookupNoHit,
  CacheLookupRequest
}
...
case class OutputNotFoundException(outputFqn: String, actualOutputs: String)
    extends RuntimeException(s"Expected output $outputFqn was not found in: '$actualOutputs'")
...
object CromwellTestKitSpec {
  ...
  /**
    ...
    */
  lazy val AnyValueIsFine: WomValue = WomString(
  ...
  def replaceVariables(value: String, workflowId: WorkflowId): String = {
    ...
    variables.foldLeft(value) { case (result, (variableName, variableValue)) =>
      result.replace(s"<<$variableName>>", s"$variableValue")
    }
  }
  ...
  lazy val NooPServiceActorConfig: Config = DefaultConfig.withValue(
    "services.LoadController.class",
    ConfigValueFactory.fromAnyRef("cromwell.services.NooPServiceActor")
  )
  ...
  val ServiceRegistryActorInstance: ActorRef =
    ServiceRegistryActorSystem.actorOf(ServiceRegistryActor.props(CromwellTestKitSpec.DefaultConfig),
                                       "ServiceRegistryActor"
    )
  ...
  class TestCromwellRootActor(config: Config)(implicit materializer: ActorMaterializer)
    ...
    def submitWorkflow(sources: WorkflowSourceFilesCollection): WorkflowId = {
      ...
      val result = Await
        .result(workflowStoreActor.ask(submitMessage)(TimeoutDuration), Duration.Inf)
        .asInstanceOf[WorkflowSubmittedToStore]
        .workflowId
...
abstract class CromwellTestKitSpec
    ...
    override def areEqual(a: WomValue, b: Any): Boolean = {
      ...
      val valueEquality = (a, b) match {
        ...
        case (map: WomMap, expectedMap: WomMap) =>
          val mapped = map.value.map { case (k, v) =>
            expectedMap.value.contains(k) && areEqual(v, expectedMap.value(k))
          }
  ...
  def runWdl(sampleWdl: SampleWdl,
    ...
    eventually(verifyWorkflowComplete(rootActor.underlyingActor.serviceRegistryActor, workflowId))(
  ...
  def runWdlAndAssertOutputs(sampleWdl: SampleWdl,
    ...
    if (!allowOtherOutputs) {
      outputs foreach { case (actualFqn, actualValue) =>
        val expectedValue = expectedOutputs.getOrElse(
          actualFqn,
          throw new RuntimeException(s"Actual output $actualFqn was not wanted in '$expectedOutputNames'")
        )
  ...
  private def getWorkflowState(workflowId: WorkflowId, serviceRegistryActor: ActorRef)(implicit
    ec: ExecutionContext
  ): WorkflowState = {
    val statusResponse = serviceRegistryActor.ask(GetStatus(workflowId))(TimeoutDuration).collect {
      case SuccessfulMetadataJsonResponse(_, jsObject) =>
        WorkflowState.withName(jsObject.fields("status").asInstanceOf[JsString].value)
  ...
  /**
    ...
    */
  protected def verifyWorkflowComplete(serviceRegistryActor: ActorRef, workflowId: WorkflowId)(implicit
    ec: ExecutionContext
  ): Unit = {
    List(WorkflowSucceeded, WorkflowFailed, WorkflowAborted) should contain(
      getWorkflowState(workflowId, serviceRegistryActor)
    )
    ()
  }
  ...
  /**
    ...
    */
  protected def verifyWorkflowState(serviceRegistryActor: ActorRef,
  ...
  private def getWorkflowOutputsFromMetadata(id: WorkflowId,
                                             serviceRegistryActor: ActorRef
  ): Map[FullyQualifiedName, WomValue] = {
    ...
    jsValue match {
      ...
      case map: JsObject =>
        ...
        val valuesMap: Map[WomValue, WomValue] = map.fields.map { case (fieldName, fieldValue) =>
          (WomString(fieldName), jsValueToWdlValue(fieldValue))
        }
...
class AlwaysHappyJobStoreActor extends Actor {
  override def receive: Receive = { case x: JobStoreWriterCommand =>
    sender() ! JobStoreWriteSuccess(x)
  }
}
...
class EmptyCallCacheReadActor extends Actor {
  override def receive: Receive = { case _: CacheLookupRequest =>
    sender() ! CacheLookupNoHit
  }
}

class EmptyCallCacheWriteActor extends Actor {
  override def receive: Receive = { case SaveCallCacheHashes =>
...
class EmptyDockerHashActor extends Actor {
  override def receive: Receive = { case request: DockerInfoRequest =>
    sender() ! DockerInfoSuccessResponse(DockerInformation(DockerHashResult("alg", "hash"), None), request)
  }
}
