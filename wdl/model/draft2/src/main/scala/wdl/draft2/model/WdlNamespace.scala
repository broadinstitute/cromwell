...
case class WdlNamespaceWithoutWorkflow(importedAs: Option[String],
                                       ...
                                       resolvedImportRecords: Set[ResolvedImportRecord] =
                                         Set.empty[ResolvedImportRecord]
) extends WdlNamespace {
...
case class WdlNamespaceWithWorkflow(importedAs: Option[String],
  ...
  /**
    ...
    */
  def staticDeclarationsRecursive(userInputs: WorkflowCoercedInputs,
                                  wdlFunctions: WdlStandardLibraryFunctions
  ): Try[WorkflowCoercedInputs] = {
    import common.exception.Aggregation._

    def evalDeclaration(accumulated: Map[FullyQualifiedName, Try[WomValue]],
                        current: Declaration
    ): Map[FullyQualifiedName, Try[WomValue]] =
      current.expression match {
        case Some(expr) =>
          val successfulAccumulated = accumulated.collect { case (k, v) if v.isSuccess => k -> v.get }
          val value = expr.evaluate(current.lookupFunction(successfulAccumulated ++ userInputs,
                                                           wdlFunctions,
                                                           NoOutputResolver,
                                                           Map.empty[Scatter, Int]
                                    ),
                                    wdlFunctions
          )
    ...
    val filteredExceptions: Set[Class[_ <: Throwable]] =
      Set(classOf[OutputVariableLookupException], classOf[ScatterIndexNotFound])
...
object WdlNamespace {
  ...
  def loadUsingPath(wdlFile: Path,
                    resource: Option[String],
                    importResolver: Option[Seq[Draft2ImportResolver]]
  ): Try[WdlNamespace] =
    load(readFile(wdlFile), resource.getOrElse(wdlFile.toString), importResolver.getOrElse(Seq(fileResolver)), None)

  def loadUsingSource(workflowSource: WorkflowSource,
                      resource: Option[String],
                      importResolver: Option[Seq[Draft2ImportResolver]]
  ): Try[WdlNamespace] =
    ...
    maybeAst match {
      ...
      case None =>
        throw new IllegalArgumentException(
          "Could not build AST from workflow source. Source is empty or contains only comments and whitespace."
        )
    }
  }
  ...
  def apply(ast: Ast,
    ...
    def tryResolve(str: String,
                   remainingResolvers: Seq[Draft2ImportResolver],
                   errors: List[Throwable]
    ): Draft2ResolvedImportBundle =
    ...
    def getChildren(scopeAst: Ast, scope: Option[Scope]): Seq[Scope] = {
      val ScopeAstNames = Seq(
        AstNodeName.Call,
        AstNodeName.Workflow,
        AstNodeName.Namespace,
        AstNodeName.Scatter,
        AstNodeName.If,
      ...
      def getScopeAsts(root: Ast, astAttribute: String): Seq[Ast] =
        root.getAttribute(astAttribute).astListAsVector.collect {
          case a: Ast if ScopeAstNames.contains(a.getName) => a
        }
      ...
      scopeAst.getName match {
        ...
        case AstNodeName.Call =>
          val referencedTask =
            findCallable(scopeAst.getAttribute("task").sourceString, namespaces, topLevelTasks ++ workflows)
    ...
    val children =
      topLevelTasks ++ namespaces ++ workflows ++ topLevelDeclarationScopes.map(ast => getScope(ast, parent = None))
    ...
    val namespace = workflows match {
      ...
      case _ =>
        throw new SyntaxError(wdlSyntaxErrorFormatter.tooManyWorkflows(ast.findAsts(AstNodeName.Workflow).asJava))
    }
    ...
    // SYNTAX CHECKS
    val callInputSectionErrors = namespace.descendants
      .collect { case c: WdlCall => c }
      .flatMap(
    ...
    } yield new SyntaxError(
      wdlSyntaxErrorFormatter.scatterCollectionContainsInvalidVariableReference(scatter, badVariable)
    )
    ...
    val taskCommandReferenceErrors = for {
      ...
      wdlSyntaxErrorFormatter.commandExpressionContainsInvalidVariableReference(
        task.ast.getAttribute("name").asInstanceOf[Terminal],
        variable.terminal
      )
    )

    val all =
      workflowOutputErrors ++ declarationErrors ++ runtimeErrors ++ scatterErrors ++ callInputSectionErrors ++ taskCommandReferenceErrors ++ duplicateSiblingScopeNameErrors
  ...
  private def declarationName(declarationAst: Ast): Terminal =
    declarationAst.getAttribute("name").asInstanceOf[Terminal]
  ...
  private def referencesToAbsentValues(container: Scope, expression: WdlExpression): Iterable[Terminal] =
    expression.variableReferences(container) collect {
      case variable if container.resolveVariable(variable.terminal.sourceString).isEmpty => variable.terminal
    }

  private def validateDeclaration(declaration: DeclarationInterface,
                                  wdlSyntaxErrorFormatter: WdlSyntaxErrorFormatter
  ): Seq[SyntaxError] = {
    val invalidVariableReferences = for {
      ...
      wdlSyntaxErrorFormatter.declarationContainsReferenceToAbsentValue(
        declaration.parent,
        variable
      )
    )
  ...
  private def validateRuntime(attributeExpression: WdlExpression,
                              task: WdlTask,
                              wdlSyntaxErrorFormatter: WdlSyntaxErrorFormatter
  ): Seq[SyntaxError] = {
    val invalidVariableReferences = for {
  ...
  private[wdl] def lookupType(from: Scope)(n: String): WomType = {
    ...
    resolved match {
      ...
      case Some(s: Scatter) =>
        s.collection.evaluateType(lookupType(s), new WdlStandardLibraryFunctionsType, Option(from)) match {
          ...
          case _ =>
            throw new VariableLookupException(
              s"Variable $n references a scatter block ${s.fullyQualifiedName}, but the collection does not evaluate to an array"
            )
        }
  ...
  private def typeCheckDeclaration(decl: DeclarationInterface,
                                   wdlSyntaxErrorFormatter: WdlSyntaxErrorFormatter
  ): Option[SyntaxError] =
      ...
      expr.evaluateType(lookupType(decl), new WdlStandardLibraryFunctionsType, Option(decl)) match {
        case Success(womType) =>
          if (!decl.womType.isCoerceableFrom(womType)) {
            Option(
              new SyntaxError(
                wdlSyntaxErrorFormatter.taskOutputExpressionTypeDoesNotMatchDeclaredType(
                  declarationName(decl.ast),
                  decl.womType,
                  womType
                )
              )
            ...
            expr.evaluate(NoLookup, NoFunctions) match {
              case Success(value) if decl.womType.coerceRawValue(value).isFailure =>
                Option(
                  new SyntaxError(
                    wdlSyntaxErrorFormatter.declarationExpressionNotCoerceableToTargetType(declarationName(decl.ast),
                                                                                           decl.womType,
                                                                                           value.womType
                    )
                  )
  ...
  private def validateCallInputSection(call: WdlCall,
                                       wdlSyntaxErrorFormatter: WdlSyntaxErrorFormatter
  ): Seq[SyntaxError] = {
    ...
    val invalidCallInputReferences = callInputSections flatMap { ast =>
      ...
      call.callable.inputNames.find(_ == lhs) match {
        ...
        case None =>
          Option(
            new SyntaxError(
              wdlSyntaxErrorFormatter.callReferencesAbsentTaskInput(ast,
                                                                    call.callable.ast,
                                                                    lhs,
                                                                    call.unqualifiedName,
                                                                    call.isInstanceOf[WdlWorkflowCall]
              )
            )
          )
      }
      ...
      resolvedScope match {
        ...
        case Some(s: Scatter) =>
          s.collection.evaluateType(lookupType(s), new WdlStandardLibraryFunctionsType) map {
            ...
            case WomArrayType(_: WomPairType)
                if memberAccess.rhsString == "left" || memberAccess.rhsString == "right" =>
              None
            ...
            case other =>
              Option(new SyntaxError(wdlSyntaxErrorFormatter.badTargetTypeForMemberAccess(memberAccess, other)))
          } getOrElse None
        case Some(d: DeclarationInterface) =>
          d.womType match {
        ...
        case Some(other) =>
          Option(new SyntaxError(wdlSyntaxErrorFormatter.badTargetScopeForMemberAccess(memberAccess, other)))
        case None =>
          None
          ...
          memberAccess.lhsAst match {
            case outerMemberAccessAst: Ast if outerMemberAccessAst.getName == "MemberAccess" =>
              checkValidityOfMemberAccess(outerMemberAccessAst)
  ...
  def findCallable(name: String, namespaces: Seq[WdlNamespace], callables: Seq[WdlCallable]): Option[WdlCallable] =
    if (name.contains(".")) {
      ...
      namespaces find (_.importedAs.contains(parts(0))) flatMap { x =>
        findCallable(parts(1), x.namespaces, x.workflows ++ x.tasks)
      }
...
object WdlNamespaceWithWorkflow {
  ...
  @deprecated(
    "To avoid unexpected default resolutions, I recommend using the load(String, Seq[ImportResolver] method of loading.",
    "23"
  )
  def load(workflowSource: WorkflowSource): Try[WdlNamespaceWithWorkflow] = from(
    WdlNamespace.loadUsingSource(workflowSource, None, None)
  )
  ...
  def load(workflowSource: WorkflowSource, importsDirectory: File): Try[WdlNamespaceWithWorkflow] = {
    val resolvers: Seq[Draft2ImportResolver] =
      Seq(WdlNamespace.directoryResolver(importsDirectory), WdlNamespace.fileResolver)
  ...
  def load(wdlFile: Path, importResolver: Draft2ImportResolver): Try[WdlNamespaceWithWorkflow] = from(
    WdlNamespace.loadUsingPath(wdlFile, None, Option(Seq(importResolver)))
  )
  ...
  def apply(ast: Ast,
    ...
    new WdlNamespaceWithWorkflow(namespace,
                                 workflow,
                                 imports,
                                 namespaces,
                                 tasks,
                                 terminalMap,
                                 wdlSyntaxErrorFormatter,
                                 ast,
                                 sourceString,
                                 importUri,
