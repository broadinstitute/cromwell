...
object InformationValidation {
  def instance(attributeName: String = RuntimeAttributesKeys.MemoryKey,
               defaultUnit: MemoryUnit,
  ...
  def withDefaultMemory(attributeName: String = RuntimeAttributesKeys.MemoryKey,
    ...
    MemorySize.parse(memorySize) match {
      ...
      case Failure(_) =>
        instance(attributeName, defaultUnit, allowZero).withDefault(BadDefaultAttribute(WomString(memorySize.toString)))
    }
  ...
  private[validation] def validateString(attributeName: String,
                                         wdlString: WomString,
                                         allowZero: Boolean
  ): ErrorOr[MemorySize] =
  ...
  private[validation] def validateString(attributeName: String,
    ...
    MemorySize.parse(value) match {
      case scala.util.Success(memorySize: MemorySize)
          if memorySize.amount > 0 || (memorySize.amount == 0 && allowZero) =>
  ...
  private[validation] def validateInteger(attributeName: String,
                                          wdlInteger: WomInteger,
                                          defaultUnit: MemoryUnit,
                                          allowZero: Boolean
  ): ErrorOr[MemorySize] =
  ...
  def validateLong(attributeName: String,
                   value: Long,
                   defaultUnit: MemoryUnit,
                   allowZero: Boolean
  ): ErrorOr[MemorySize] =
...
class InformationValidation(attributeName: String, defaultUnit: MemoryUnit, allowZero: Boolean = false)
    extends RuntimeAttributesValidation[MemorySize] {
