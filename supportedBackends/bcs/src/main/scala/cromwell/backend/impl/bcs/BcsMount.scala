  ...
  def parse(s: String): Try[BcsMount] = {
    val validation: ErrorOr[BcsMount] = s match {
      case inputMountPattern(oss, local, writeSupport) =>
        (validateOss(oss) |@| validateLocal(oss, local) |@| validateBoolean(writeSupport)) map { (src, dest, ws)
        => new BcsInputMount(src, dest, ws)
        }
      case outputMountPattern(local, oss, writeSupport) =>
        (validateLocal(oss, local) |@| validateOss(oss) |@| validateBoolean(writeSupport)) map { (src, dest, ws)
        => new BcsOutputMount(src, dest, ws)
        }
