...
object MetadataEvent {
  ...
  def apply(key: MetadataKey, optionalValue: Option[MetadataValue]) =
    new MetadataEvent(key, optionalValue, OffsetDateTime.now)
...
object MetadataValue {
  def apply(value: Any): MetadataValue =
    Option(value).getOrElse("") match {
      ...
      case _: Double | Float | _: BigDecimal | _: java.lang.Double | _: java.lang.Float =>
        new MetadataValue(value.toString, MetadataNumber)
...
object MetadataQueryJobKey {
  def forMetadataJobKey(jobKey: MetadataJobKey) =
    MetadataQueryJobKey(jobKey.callFqn, jobKey.index, Option(jobKey.attempt))
}
...
object MetadataQuery {
  def forWorkflow(workflowId: WorkflowId) =
  ...
  def forJob(workflowId: WorkflowId, jobKey: MetadataJobKey): MetadataQuery =
    MetadataQuery(workflowId,
                  Option(MetadataQueryJobKey.forMetadataJobKey(jobKey)),
                  None,
                  None,
                  None,
