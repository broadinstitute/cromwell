...
trait WorkflowMetadataSummaryEntryComponent {
  ...
  class WorkflowMetadataSummaryEntries(tag: Tag)
    ...
    def baseProjection = (workflowExecutionUuid,
                          workflowName,
                          workflowStatus,
                          startTimestamp,
                          endTimestamp,
  ...
  val workflowIdsByArchiveStatusAndEndedOnOrBeforeThresholdTimestamp = Compiled {
    (metadataArchiveStatus: Rep[Option[String]],
     workflowEndTimestampThreshold: Rep[Timestamp],
     batchSize: ConstColumn[Long]
    ) =>
      (for {
  ...
  val workflowMetadataSummaryEntriesForWorkflowExecutionUuid = Compiled((workflowExecutionUuid: Rep[String]) =>
    for {
  ...
  val workflowMetadataSummaryEntryExistsForWorkflowExecutionUuid = Compiled((workflowExecutionUuid: Rep[String]) =>
    (for {
  ...
  val workflowStatusesForWorkflowExecutionUuid = Compiled((workflowExecutionUuid: Rep[String]) =>
    for {
      workflowMetadataSummaryEntry <- workflowMetadataSummaryEntries
      if workflowMetadataSummaryEntry.workflowExecutionUuid === workflowExecutionUuid
  ...
  val rootWorkflowId = Compiled((workflowId: Rep[String]) =>
    for {
      summary <- workflowMetadataSummaryEntries
  ...
  val metadataArchiveStatusByWorkflowId = Compiled((workflowExecutionUuid: Rep[String]) =>
    for {
      workflowMetadataSummaryEntry <- workflowMetadataSummaryEntries
      if workflowMetadataSummaryEntry.workflowExecutionUuid === workflowExecutionUuid
  ...
  val metadataArchiveStatusAndEndTimeByWorkflowId = Compiled((workflowExecutionUuid: Rep[String]) =>
    for {
      workflowMetadataSummaryEntry <- workflowMetadataSummaryEntries
      if workflowMetadataSummaryEntry.workflowExecutionUuid === workflowExecutionUuid
  ...
  ): Query[WorkflowMetadataSummaryEntries, WorkflowMetadataSummaryEntry, Seq] =
    for {
      summaryEntry <- workflowMetadataSummaryEntries
  ...
  private def fetchAllWorkflowsToDeleteThatEndedOnOrBeforeThresholdTimestamp(
    workflowEndTimestampThreshold: Timestamp
  ): Query[WorkflowMetadataSummaryEntries, WorkflowMetadataSummaryEntry, Seq] =
    for {
      summaryEntry <- workflowMetadataSummaryEntries
  ...
  def concatNel(nel: NonEmptyList[SQLActionBuilder]): SQLActionBuilder = nel.tail.foldLeft(nel.head) { (acc, next) =>
    concat(acc, next)
  }

  def and(list: NonEmptyList[SQLActionBuilder]): SQLActionBuilder = if (list.size == 1) list.head
  else {
    val fullList = data.NonEmptyList.of(sql"(") ++ list.init.flatMap(x => List(x, sql" AND ")) :+ list.last :+ sql")"
    concatNel(fullList)
  }

  def or(list: NonEmptyList[SQLActionBuilder]): SQLActionBuilder = if (list.size == 1) list.head
  else {
  ...
  def buildQueryAction(selectOrCount: SelectOrCount,
    ...
    val labelsAndTableAliases = labelAndKeyLabelValues.zipWithIndex.map { case (labelPair, i) =>
      quoted(s"labelAndTable$i") -> labelPair
    }.toMap
    ...
    val labelOrJoin = if (labelOrKeyLabelValues.nonEmpty) {
      Option(sql"""|  JOIN #$customLabelEntryTable #$labelsOrTableAlias
    ...
    val from = concatNel(NonEmptyList.of(sql"""|FROM #$workflowMetadataSummaryEntryTable #$summaryTableAlias
    ...
    val labelsAndConstraint = NonEmptyList
      .fromList(labelsAndTableAliases.toList.map { case (labelsAndTableAlias, (labelKey, labelValue)) =>
        and(
          NonEmptyList.of(
    ...
    val labelOrConstraint = NonEmptyList
      .fromList(labelOrKeyLabelValues.toList.map { case (labelKey, labelValue) =>
        and(
          NonEmptyList.of(
    ...
    val excludeLabelsOrConstraint = NonEmptyList
      .fromList(excludeLabelOrValues.toList map { case (labelKey, labelValue) =>
        not(labelExists(labelKey, labelValue))
      })
      .map(and)
      .toList

    // At least one of the excludeLabelAnd list of KV pairs must *NOT* exist:
    val excludeLabelsAndConstraint = NonEmptyList
      .fromList(excludeLabelAndValues.toList.map { case (labelKey, labelValue) =>
        not(labelExists(labelKey, labelValue))
      })
      .map(or)
      .toList

    val includeSubworkflowsConstraint =
      if (includeSubworkflows) List.empty
      else {
