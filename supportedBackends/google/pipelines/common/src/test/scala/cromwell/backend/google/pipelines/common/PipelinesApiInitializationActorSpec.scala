...
import cromwell.backend.google.pipelines.common.PipelinesApiTestConfig.{genomicsFactory, googleConfiguration, jesAttributes}
import cromwell.backend.google.pipelines.common.authentication.{GcsLocalizing, JesAuthObject}
...
class PipelinesApiInitializationActorSpec extends TestKitSuite("JesInitializationActorSpec") with FlatSpecLike with Matchers
  ...
  private case class TestingBits(actorRef: TestActorRef[JesInitializationActor], jesConfiguration: PipelinesApiConfiguration)
