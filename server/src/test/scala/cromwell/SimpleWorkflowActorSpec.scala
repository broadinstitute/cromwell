  ...
  private def buildWorkflowActor(sampleWdl: SampleWdl,
    ...
    val workflowActor = TestFSMRef(
      factory = new WorkflowActor(workflowToStart, config,
        ...
        jobStoreCheckTokenDispenserActor = system.actorOf(JobTokenDispenserActor.props(serviceRegistry, Rate(100, 1.second), None, "JobStore", "CheckingJobStore"), "JobStoreCheckTokenDispenser"),
        jobExecutionTokenDispenserActor = system.actorOf(JobTokenDispenserActor.props(serviceRegistry, Rate(100, 1.second), None, "Execution", "Running"), name = "JobExecutionTokenDispenser"),
