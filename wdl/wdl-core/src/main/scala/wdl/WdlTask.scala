...
object WdlTask {
  ...
  def apply(ast: Ast, wdlSyntaxErrorFormatter: WdlSyntaxErrorFormatter)(implicit wdlVersionSpecifics: WdlVersionSpecifics): WdlTask = {
  ...
  def empty(implicit wdlVersionSpecifics: WdlVersionSpecifics): WdlTask = new WdlTask("taskName", Seq.empty, WdlRuntimeAttributes(Map.empty[String, WdlExpression]), Map.empty, Map.empty, null)
...
case class WdlTask(name: String,
                   ...
                   ast: Ast)(implicit implicitVersionSpecifics: WdlVersionSpecifics) extends WdlCallable {
  ...
  private def buildWomTaskDefinition: CommandTaskDefinition = CallableTaskDefinition(
    name = name,
