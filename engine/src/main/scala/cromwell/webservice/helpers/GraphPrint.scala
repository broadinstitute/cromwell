package cromwell.webservice.helpers
...
object GraphPrint {
  ...
  def generateWorkflowDagString(workflowSource: WorkflowSource): String = {
  ...
  private def generateWorkflowDigraph(workflowSource: WorkflowSource): WorkflowDigraph = {
    // It's ok to use .get here, we're happy to throw an exception and crash the program!
    val namespace = WdlNamespaceWithWorkflow.load(workflowSource, Seq.empty).get
