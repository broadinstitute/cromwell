...
trait IoCommandBuilder {
  ...
  def deleteCommand(path: Path, swallowIoExceptions: Boolean = false): IoDeleteCommand
  def copyCommand(src: Path, dest: Path, overwrite: Boolean = true): IoCopyCommand
...
trait DefaultIoCommandBuilder extends IoCommandBuilder {
  ...
  def deleteCommand(path: Path, swallowIoExceptions: Boolean = false) = new IoDeleteCommand(path, swallowIoExceptions)
  def copyCommand(src: Path, dest: Path, overwrite: Boolean = true) = new IoCopyCommand(src, dest, overwrite)
