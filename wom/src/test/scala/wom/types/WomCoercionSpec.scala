...
abstract class WomCoercionSpec(val goodCoercionTable: TableFor2[_ <: Any, WomValue],
  ...
  forAll(goodCoercionTable) { (fromValue, toValue) =>
    ...
    it should s"Allow coercion from ${fromValue.displayString} (${fromValue.typeDisplayString}) to ${toValue.toWomString} (${toValue.womType.callCachingName})" in {
    ...
    it should s"correctly coerce ${fromValue.displayString} (${fromValue.typeDisplayString}) to ${toValue.toWomString} (${toValue.womType.callCachingName})" in {
  ...
  forAll(badCoercionTable) { (fromValue, toType) =>
    ...
    it should s"Not allow coercion from ${fromValue.displayString} to ${toType.callCachingName}" in {
    ...
    it should s"Fail when coercing ${fromValue.displayString} to ${toType.callCachingName}" in {
...
private object WomCoercionSpec {
  implicit class StringableAny(val a: Any) extends AnyVal {
    ...
    def typeDisplayString: String = a match {
      case wv: WomValue => wv.womType.callCachingName
