...
class MetadataDatabaseAccessSpec
    extends AnyFlatSpec
  ...
  DatabaseSystem.All foreach { databaseSystem =>
    ...
    lazy val dataAccess: MetadataDatabaseAccess with MetadataServicesStore =
      new MetadataDatabaseAccess with MetadataServicesStore {
        override val metadataDatabaseInterface: MetadataSlickDatabase =
          DatabaseTestKit.initializeDatabaseByContainerOptTypeAndSystem(containerOpt,
    ...
    def baseWorkflowMetadata(name: String,
      ...
      val labelMetadata =
        (labels ++ defaultLabels).map(label => (s"${WorkflowMetadataKeys.Labels}:${label.key}", label.value)).toArray
    ...
    def subworkflowMetadata(parentWorkflowId: WorkflowId,
    ...
    it should "properly get next workflow to archive and number of workflows left to archive" taggedAs DbmsTest in {
      ...
      eventually(Timeout(2.minutes)) {
        val workflowsLeftToArchiveFuture = dataAccess.countWorkflowsLeftToArchiveThatEndedOnOrBeforeThresholdTimestamp(
      ...
      val workflowToArchiveResponse1 = eventually(Timeout(2.minutes)) {
        val workflowToArchiveFuture =
          dataAccess.queryWorkflowsToArchiveThatEndedOnOrBeforeThresholdTimestamp(terminalWorkflowStatuses,
                                                                                  OffsetDateTime.now().minusSeconds(10),
                                                                                  1
          )
      ...
      val updateMetadataArchiveStatusFuture =
        dataAccess.updateMetadataArchiveStatus(workflowId1, MetadataArchiveStatus.Archived)
    ...
    it should "return pagination metadata only when page and pagesize query params are specified" taggedAs DbmsTest in {
      (for {
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(Seq(WorkflowQueryKey.Page.name -> "1", WorkflowQueryKey.PageSize.name -> "50"))
        ) map { case (_, meta) =>
          meta match {
            case Some(_) =>
        ...
        _ <- dataAccess.queryWorkflowSummaries(WorkflowQueryParameters(Seq())) map { case (_, meta) =>
          meta match {
    ...
    it should "sort metadata events by timestamp from older to newer" taggedAs DbmsTest in {
      ...
      def unorderedEvents(id: WorkflowId): Future[Vector[MetadataEvent]] = {
        ...
        val yesterdayEvent = MetadataEvent(workflowKey.copy(key = WorkflowMetadataKeys.WorkflowRoot),
                                           Option(MetadataValue("A")),
                                           yesterday
        )
        val nowEvent =
          MetadataEvent(workflowKey.copy(key = WorkflowMetadataKeys.WorkflowRoot), Option(MetadataValue("B")), now)
        val tomorrowEvent =
          MetadataEvent(workflowKey.copy(key = WorkflowMetadataKeys.WorkflowRoot), Option(MetadataValue("C")), tomorrow)
      ...
      (for {
        ...
        response <- dataAccess.queryMetadataEvents(MetadataQuery(workflow1Id,
                                                                 None,
                                                                 Option(WorkflowMetadataKeys.WorkflowRoot),
                                                                 None,
                                                                 None,
      ...
      (for {
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(
            Seq(WorkflowQueryKey.Name.name -> Workflow1Name, WorkflowQueryKey.Name.name -> Workflow2Name)
          )
        ) map { case (response, _) =>
          val resultsByName = response.results groupBy {
            _.name
          }
          withClue("Filter by multiple names") {
            resultsByName.keys.toSet.flatten should equal(Set(Workflow1Name, Workflow2Name))
          }
        }
        // Filter by workflow id
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(Seq(WorkflowQueryKey.Id.name -> workflow1Id.toString))
        ) map { case (response, _) =>
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(Seq(workflow1Id, workflow2Id).map(id => WorkflowQueryKey.Id.name -> id.toString))
        ) map { case (response, _) =>
          val resultsById = response.results groupBy {
            _.name
          }
          withClue("Filter by multiple workflow IDs") {
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          ...
          withClue("Filter by workflow ID within random IDs") {
            resultsById.keys.toSet.flatten should equal(Set(Workflow1Name))
          }
        }
        // Filter by status
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(Seq(WorkflowQueryKey.Status.name -> "Submitted"))
        ) map { case (response, _) =>
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(
            Seq(WorkflowQueryKey.Status.name -> "Submitted", WorkflowQueryKey.Status.name -> "Succeeded")
          )
        ) map { case (response, _) =>
          val resultsByStatus = response.results groupBy (_.status)
          withClue("Filter by multiple statuses") {
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(
            Seq(WorkflowQueryKey.LabelAndKeyValue.name -> s"${testLabel2.key}:${testLabel2.value}")
          )
        ) map { case (response, _) =>
          val resultByName = response.results groupBy (_.name)
          withClue("Filter by label using AND") {
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          ...
          val resultByName = response.results groupBy (_.name)
          withClue("Filter by multiple labels using AND") {
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(
            Seq(WorkflowQueryKey.LabelOrKeyValue.name -> s"${testLabel2.key}:${testLabel2.value}")
          )
        ) map { case (response, _) =>
          val resultByName = response.results groupBy (_.name)
          withClue("Filter by label using OR") {
            resultByName.keys.toSet.flatten should equal(Set(Workflow1Name, Workflow2Name))
          }
        }
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          ...
          val resultByName = response.results groupBy (_.name)
          withClue("Filter by multiple label using OR") {
            resultByName.keys.toSet.flatten should equal(Set(Workflow1Name, Workflow2Name))
          }
        }
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(
            Seq(WorkflowQueryKey.ExcludeLabelAndKeyValue.name -> s"${testLabel2.key}:${testLabel2.value}")
          )
        ) map { case (response, _) =>
          val resultByName = response.results groupBy (_.name)
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(
            Seq(WorkflowQueryKey.ExcludeLabelOrKeyValue.name -> s"${testLabel2.key}:${testLabel2.value}")
          )
        ) map { case (response, _) =>
          val resultByName = response.results groupBy (_.name)
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(
            Seq(WorkflowQueryKey.StartDate.name -> workflowQueryResult2.start.get.toUtcMilliString)
          )
        ) map { case (response, _) =>
          response.results partition { r =>
            r.start.isDefined && r.start.get.compareTo(workflowQueryResult.start.get) >= 0
          } match {
        ...
        // Filter by end date
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(Seq(WorkflowQueryKey.EndDate.name -> workflowQueryResult.end.get.toUtcMilliString))
        ) map { case (response, _) =>
          response.results partition { r =>
            r.end.isDefined && r.end.get.compareTo(workflowQueryResult.end.get) <= 0
          } match {
            case (y, n) if y.nonEmpty && n.isEmpty => // good
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(
            Seq(WorkflowQueryKey.SubmissionTime.name -> workflowQueryResult2.submission.get.toUtcMilliString)
          )
        ) map { case (response, _) =>
          response.results partition { r =>
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(Seq(WorkflowQueryKey.AdditionalQueryResultFields.name -> "labels"))
        ) map { case (response, _) =>
          response.results filter { workflowQueryResult =>
            List(workflow1Id.toString, workflow1Id.toString).contains(workflowQueryResult.id)
          } partition { r => r.labels.isDefined } match {
            case (y, n) if y.nonEmpty && n.isEmpty => // good
            case (y, n) =>
              fail(
                s"Something went horribly wrong since labels were populated for ${y.size} and were missing for ${n.size} workflow(s)!"
              )
          }
        }
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(
            Seq(
              ...
              WorkflowQueryKey.Page.name -> "1",
              WorkflowQueryKey.PageSize.name -> "1"
            )
          )
        ) map { case (resp, _) =>
          resp.totalResultsCount match {
            ...
            case 1 =>
              fail(
                "totalResultsCount is suspiciously equal to the pageSize and not the expected total results count. Please fix!"
              )
            case other =>
              fail(s"totalResultsCount is expected to be 2 but is actually $other. Something has gone horribly wrong!")
          }
        }
      } yield ()).futureValue
    }
    ...
    it should "include/exclude subworkflows and check for parentWorkflowId in query response" taggedAs DbmsTest in {
      ...
      (for {
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(Seq(WorkflowQueryKey.IncludeSubworkflows.name -> true.toString))
        ) map { case (resp, _) =>
          val resultByName = resp.results groupBy (_.name)
          withClue("include subworkflows:") {
            List(ParentWorkflowName, SubworkflowName, ParentWorkflow2Name, Subworkflow2Name).foreach { n =>
        ...
        _ <- dataAccess.queryWorkflowSummaries(WorkflowQueryParameters(excludeSubworkflowQueryParams2)) map {
          case (resp, _) =>
            ...
            withClue("exclude subworkflows and assert page size:") {
              resultByName.keys.flatten should be(Set(ParentWorkflowName, ParentWorkflow2Name))
            }
        }
        ...
        _ <- dataAccess.queryWorkflowSummaries(
          WorkflowQueryParameters(
            Seq(WorkflowQueryKey.AdditionalQueryResultFields.name -> "parentWorkflowId",
                WorkflowQueryKey.Name.name -> SubworkflowName
            )
          )
        ) map { case (response, _) =>
          response.results partition { r => r.parentWorkflowId.isDefined } match {
            ...
            case (y, n) =>
              fail(
                s"parentWorkflow1 should be populated for a subworkflow. It was populated correctly for ${y.size} " +
    ...
    it should "properly query metadata summaries based on archived status and timestamp and update archive status after metadata deletion" taggedAs DbmsTest in {
      ...
      val workflowId1Future = succeededWorkflowMetadata(WorkflowId.fromString("11111111-1111-1111-1111-111111111112"))
      val workflowId2Future = succeededWorkflowMetadata(WorkflowId.fromString("11111111-1111-1111-1111-111111111113"))
      val workflowId1 = Await.result(workflowId1Future, defaultTimeout)
      val workflowId2 = Await.result(workflowId2Future, defaultTimeout)

      val refreshSummariesFuture = dataAccess.refreshWorkflowMetadataSummaries(1000)
      Await.result(refreshSummariesFuture, defaultTimeout)

      val updateMetadataArchiveStatusFuture =
        dataAccess.updateMetadataArchiveStatus(workflowId1, MetadataArchiveStatus.Archived)
      Await.result(updateMetadataArchiveStatusFuture, defaultTimeout)

      val responseEmptyFuture =
        dataAccess.queryWorkflowIdsByArchiveStatusAndOlderThanTimestamp(Option("Archived"),
      ...
      val deleteMetadataFuture =
        dataAccess.deleteAllMetadataEntriesForWorkflowAndUpdateArchiveStatus(workflowId1, Option("ArchivedAndDeleted"))
      Await.result(deleteMetadataFuture, defaultTimeout)

      val summaryResponseFuture = dataAccess.queryWorkflowSummaries(
        WorkflowQueryParameters(Seq(WorkflowQueryKey.Id.name -> workflowId1.toString))
      )
    ...
    it should "error when deleting metadata for a root workflow that does not exist" taggedAs DbmsTest in {
      dataAccess
        .deleteAllMetadataEntriesForWorkflowAndUpdateArchiveStatus(
          WorkflowId.fromString("00000000-0000-0000-0000-000000000000"),
          None
        )
        .failed
        .futureValue(Timeout(10.seconds))
        .getMessage should be(
        """Metadata deletion precondition failed: workflow ID "00000000-0000-0000-0000-000000000000" did not have a status in the summary table"""
      )
    }

    it should "refuse to delete a non-terminal workflow" taggedAs DbmsTest in {
      val test = for {
        rootWorkflowId <- baseWorkflowMetadata("root workflow name",
                                               workflowId =
                                                 WorkflowId.fromString("33333333-3333-3333-3333-333333333333")
        )
        _ <- dataAccess.refreshWorkflowMetadataSummaries(1000)
        _ <- dataAccess.deleteAllMetadataEntriesForWorkflowAndUpdateArchiveStatus(rootWorkflowId, None)
      } yield ()

      test.failed.futureValue(Timeout(10.seconds)).getMessage should be(
        s"""Metadata deletion precondition failed: workflow ID "33333333-3333-3333-3333-333333333333" was in non-terminal status "Submitted""""
      )
    }
