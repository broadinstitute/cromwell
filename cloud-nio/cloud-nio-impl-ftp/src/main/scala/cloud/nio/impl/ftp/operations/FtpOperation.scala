...
case class FtpListFiles(cloudHost: String, cloudPath: String, description: String = "List files") extends FtpValueOperation[Array[FTPFile]] {
  override val action = _.listFiles(cloudPath.ensureSlashedPrefix)
}

case class FtpListDirectories(cloudHost: String, cloudPath: String, description: String = "List files") extends FtpValueOperation[Array[FTPFile]] {
  // We need to list the directories in the parent and see if any matches the name, hence the string manipulations
  lazy val parts = cloudPath.ensureSlashedPrefix.stripSuffix(CloudNioFileSystem.Separator).split(CloudNioFileSystem.Separator)
...
case class FtpDeleteFile(cloudHost: String, cloudPath: String, description: String = "delete file") extends FtpBooleanOperation {
  override val action = _.deleteFile(cloudPath.ensureSlashedPrefix)
  override val failOnFalse = false
}

case class FtpInputStream(cloudHost: String, cloudPath: String, offset: Long, description: String = "read") extends FtpValueOperation[InputStream] {
...
case class FtpOutputStream(cloudHost: String, cloudPath: String, description: String = "write") extends FtpValueOperation[OutputStream] {
  override val action = _.storeFileStream(cloudPath.ensureSlashedPrefix)
}

case class FtpCreateDirectory(cloudHost: String, cloudPath: String, description: String = "create directory") extends FtpBooleanOperation {
  override val action = _.makeDirectory(cloudPath.ensureSlashedPrefix)
}

case class FtpCompletePendingCommand(cloudHost: String, cloudPath: String, description: String = "close stream") extends FtpBooleanOperation {
