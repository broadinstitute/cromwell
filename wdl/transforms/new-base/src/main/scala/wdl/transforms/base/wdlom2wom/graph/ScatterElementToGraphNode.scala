...
object ScatterElementToGraphNode {
  def convert(a: ScatterNodeMakerInputs)(implicit
  ...
  def convertOuterScatter(a: ScatterNodeMakerInputs)(implicit
    expressionValueConsumer: ExpressionValueConsumer[ExpressionElement],
    fileEvaluator: FileEvaluator[ExpressionElement],
    typeEvaluator: TypeEvaluator[ExpressionElement],
    valueEvaluator: ValueEvaluator[ExpressionElement]
      ...
      (required, generated) mapN { (r, g) =>
        val requiredOuterValues = r collect {
          ...
          case hook @ UnlinkedCallOutputOrIdentifierAndMemberAccessHook(first, second)
              if first != scatterVariableName && !g
                .exists(_.linkableName == first) && !g.exists(_.linkableName == s"$first.$second") =>
            makeLink(hook)
        }
    ...
    (scatterExpressionNodeValidation, scatterVariableTypeValidation, foundOuterGeneratorsValidation) flatMapN {
      (expressionNode, scatterVariableType, foundOuterGenerators) =>
        val womInnerGraphScatterVariableInput =
          ScatterVariableNode(WomIdentifier(scatterVariableName), expressionNode, scatterVariableType)
        ...
        val graphLikeConvertInputs = GraphLikeConvertInputs(
          graphElements.toSet,
          ogins ++ Set(womInnerGraphScatterVariableInput),
  ...
  def convertInnerScatter(a: ScatterNodeMakerInputs)(implicit
    expressionValueConsumer: ExpressionValueConsumer[ExpressionElement],
    fileEvaluator: FileEvaluator[ExpressionElement],
    typeEvaluator: TypeEvaluator[ExpressionElement],
    valueEvaluator: ValueEvaluator[ExpressionElement]
    ...
    val requiredOuterValuesValidation: ErrorOr[Set[UnlinkedConsumedValueHook]] =
      a.node.graphElementConsumedValueHooks(a.availableTypeAliases, a.callables)

    val subWorkflowInputsValidation: ErrorOr[Set[GraphNode]] = requiredOuterValuesValidation map {
      requiredOuterValues =>
        val requiredLinkableNames: Set[(String, WomType)] = requiredOuterValues map { hook =>
          (a.linkableValues(hook).linkableName, a.linkableValues(hook).womType)
        }
    ...
    val scatterableGraphValidation = subWorkflowDefinitionValidation map { subWorkflowDefinition =>
      ...
      def inputNodeIdentifier(input: InputDefinition) =
        WomIdentifier(localName = input.name, fullyQualifiedName = a.workflowName + "." + input.name)
      val newInputNodes: Map[String, ExternalGraphInputNode] = (unsatisfiedInputs collect {
        case i: RequiredInputDefinition =>
          i.name -> RequiredGraphInputNode(inputNodeIdentifier(i),
                                           i.womType,
                                           a.workflowName + "." + i.name,
                                           Callable.InputDefinition.IdentityValueMapper
          )
        case i: OptionalInputDefinition =>
          i.name -> OptionalGraphInputNode(inputNodeIdentifier(i),
                                           i.womType,
                                           a.workflowName + "." + i.name,
                                           Callable.InputDefinition.IdentityValueMapper
          )
      ...
      val mappingAndPorts: List[((InputDefinition, InputDefinitionPointer), InputPort)] =
        subWorkflowDefinition.inputs map { i =>
      ...
      val result = callNodeBuilder.build(
        WomIdentifier(a.node.scatterName),
