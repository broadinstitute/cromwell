...
trait DockerHashStoreEntryComponent {
  ...
  class DockerHashStoreEntries(tag: Tag) extends Table[DockerHashStoreEntry](tag, "DOCKER_HASH_STORE_ENTRY") {
    ...
    override def * = (workflowExecutionUuid, dockerTag, dockerHash, dockerSize, dockerHashStoreEntryId.?) <> (
      DockerHashStoreEntry.tupled,
      DockerHashStoreEntry.unapply
    )

    def ucDockerHashStoreEntryWeuDt =
      index("UC_DOCKER_HASH_STORE_ENTRY_WEU_DT", (workflowExecutionUuid, dockerTag), unique = true)
  }

  val dockerHashStoreEntries = TableQuery[DockerHashStoreEntries]

  val dockerHashStoreEntryIdsAutoInc =
    dockerHashStoreEntries returning dockerHashStoreEntries.map(_.dockerHashStoreEntryId)

  /**
    * Useful for finding the docker hash store for a given workflow execution UUID
    */
  val dockerHashStoreEntriesForWorkflowExecutionUuid = Compiled((workflowExecutionUuid: Rep[String]) =>
    for {
