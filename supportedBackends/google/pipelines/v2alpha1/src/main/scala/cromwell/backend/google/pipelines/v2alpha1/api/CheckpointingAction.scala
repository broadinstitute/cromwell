...
trait CheckpointingAction {
  def checkpointingSetupActions(createPipelineParameters: CreatePipelineParameters, mounts: List[Mount]): List[Action] =
    createPipelineParameters.runtimeAttributes.checkpointFilename map { checkpointFilename =>
      ...
      val checkpointingCommand =
        createPipelineParameters.checkpointingConfiguration.checkpointingCommand(checkpointFilename,
                                                                                 ActionCommands.multiLineBinBashCommand
        )
      ...
      val describeInitialCheckpointingSyncAction =
        ActionBuilder.describeDocker("initial checkpointing sync", initialCheckpointSyncAction)
      ...
      List(describeInitialCheckpointingSyncAction,
           initialCheckpointSyncAction,
           describeBackgroundCheckpointingAction,
           backgroundCheckpointingAction
      )
    } getOrElse Nil
  ...
  def checkpointingShutdownActions(createPipelineParameters: CreatePipelineParameters): List[Action] =
    createPipelineParameters.runtimeAttributes.checkpointFilename map { checkpointFilename =>
      ...
      val deleteCheckpointAction = ActionBuilder.gcsFileDeletionAction(
        createPipelineParameters.checkpointingConfiguration.checkpointFileCloud(checkpointFilename)
      )
      val deleteTmpCheckpointAction = ActionBuilder.gcsFileDeletionAction(
        createPipelineParameters.checkpointingConfiguration.tmpCheckpointFileCloud(checkpointFilename)
      )
