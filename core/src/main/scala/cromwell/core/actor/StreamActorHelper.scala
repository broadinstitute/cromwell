...
trait StreamActorHelper[T <: StreamContext] { this: Actor with ActorLogging =>
  ...
  private val replySink = Sink.foreach[(Any, T)] { case (response, commandContext) =>
  ...
  def sendToStream(commandContext: T) = {
    val enqueue = stream offer commandContext map {
      ...
      Future.successful(FailedToEnqueue(t, commandContext))
    }
  ...
  private def backpressure(commandContext: StreamContext) = {
    val originalRequest = commandContext.clientContext map {
      _ -> commandContext.request
    } getOrElse commandContext.request
