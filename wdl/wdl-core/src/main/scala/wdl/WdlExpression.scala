...
/**
  ...
  */
final case class WdlWomExpression(wdlExpression: WdlExpression, from: Scope)(implicit implicitVersionSpecifics: WdlVersionSpecifics) extends WomExpression {
  ...
  /**
    * Answers the question: "if this expression is used as an output expression, what files will I need to delocalize in order to evaluate it".
    *
    * This includes the trivial answer of "it's a simple file value expression so that's the one you'll need!"
    */
...
//      override val wdlVersionSpecifics: WdlVersionSpecifics = implicitVersionSpecifics
//    }

    wdlExpression.evaluateFiles(inputTypes.apply, NoFunctions, coerceTo).toErrorOr.map(_.toSet[WomFile])
  }
}
