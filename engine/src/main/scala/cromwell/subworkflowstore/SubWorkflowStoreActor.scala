...
class SubWorkflowStoreActor(database: SubWorkflowStore) extends Actor with ActorLogging with MonitoringCompanionHelper {
  ...
  private def registerSubWorkflow(replyTo: ActorRef, command: RegisterSubWorkflow) = {
    ...
    database.addSubWorkflowStoreEntry(
      ...
      command.jobKey.node.fullyQualifiedName,
  ...
  private def querySubWorkflow(replyTo: ActorRef, command: QuerySubWorkflow) = {
    ...
    database.querySubWorkflowStore(command.parentWorkflowExecutionUuid.toString,
                                   jobKey.node.fullyQualifiedName,
                                   jobKey.index.fromIndex,
                                   jobKey.attempt
    ) onComplete {
...
object SubWorkflowStoreActor {
  ...
  case class RegisterSubWorkflow(rootWorkflowExecutionUuid: WorkflowId,
                                 parentWorkflowExecutionUuid: WorkflowId,
                                 jobKey: JobKey,
                                 subWorkflowExecutionUuid: WorkflowId
  ) extends SubWorkflowStoreActorCommand
  case class QuerySubWorkflow(parentWorkflowExecutionUuid: WorkflowId, jobKey: JobKey)
      extends SubWorkflowStoreActorCommand
  ...
  case class SubWorkflowStoreFailure(command: SubWorkflowStoreActorCommand, failure: Throwable)
      extends SubWorkflowStoreActorResponse
