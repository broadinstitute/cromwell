...
object LookupEvaluators {
  ...
  implicit val expressionMemberAccessEvaluator: ValueEvaluator[ExpressionMemberAccess] =
    new ValueEvaluator[ExpressionMemberAccess] {
  ...
  implicit val identifierMemberAccessEvaluator: ValueEvaluator[IdentifierMemberAccess] =
    new ValueEvaluator[IdentifierMemberAccess] {
      override def evaluateValue(a: IdentifierMemberAccess,
                                 inputs: Map[String, WomValue],
                                 ioFunctionSet: IoFunctionSet,
                                 forCommandInstantiationOptions: Option[ForCommandInstantiationOptions]
      )(implicit
        expressionValueEvaluator: ValueEvaluator[ExpressionElement]
      ): ErrorOr[EvaluatedValue[_ <: WomValue]] = {
        ...
        val generatedValueAndLookups: ErrorOr[(WomValue, Seq[String])] = {
          ...
          if (inputs.keySet.contains(a.first)) { (inputs(a.first), List(a.second) ++ a.memberAccessTail).validNel }
            ...
            s"No value found for member access lookup. Report this bug: Insufficient input values supplied by engine. Needed '${a.first}' or '$callOutputKey' but only received: '${inputs.keys
                .mkString(", ")}'".invalidNel
        }

        generatedValueAndLookups flatMap { case (foundValue, lookups) =>
          NonEmptyList.fromList(lookups.toList) match {
  ...
  implicit val indexAccessValueEvaluator: ValueEvaluator[IndexAccess] = new ValueEvaluator[IndexAccess] {
    override def evaluateValue(a: IndexAccess,
                               inputs: Map[String, WomValue],
                               ioFunctionSet: IoFunctionSet,
                               forCommandInstantiationOptions: Option[ForCommandInstantiationOptions]
    )(implicit expressionValueEvaluator: ValueEvaluator[ExpressionElement]): ErrorOr[EvaluatedValue[_ <: WomValue]] =
      (a.expressionElement.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions),
       a.index.evaluateValue(inputs, ioFunctionSet, forCommandInstantiationOptions)
      ) flatMapN { (lhs, rhs) =>
  ...
  private def doLookup(womValue: WomValue, lookupChain: NonEmptyList[String]): ErrorOr[WomValue] = {
    ...
    val thisValue: ErrorOr[WomValue] = womValue match {
      ...
      case WomObject(_, WomCompositeType(typeMap, _)) if typeMap.contains(key) =>
        typeMap(key) match {
          case WomOptionalType(innerType) => WomOptionalValue(innerType, None).validNel
          case other =>
            s"Composite value was unexpectedly missing a field: '$key' (expected type ${other.stableName}). Report this bug! Static validation failed.".invalidNel
        }
      case WomObject(_, _: WomCompositeType) =>
        s"No such field '$key' on type ${womValue.womType.stableName}. Report this bug! Static validation failed.".invalidNel
      ...
      case WomMap(_, value) =>
        Validated.fromOption(
          ...
          ifNone = NonEmptyList(
            s"Requested key '$key' not found in the Map. Available keys were: ${value.keySet.mkString("[ ", ",", "]")}",
            Nil
          )
        )
      case _ =>
