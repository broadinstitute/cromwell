...
object BcsClusterIdOrConfiguration {
  ...
  def parse(cluster: String): Try[BcsClusterIdOrConfiguration] = {
    cluster match {
      ...
      case attachClusterSimplePattern(instanceType, imageId, clusterId) =>Success(Right(AutoClusterConfiguration(defaultResourceType, instanceType, imageId, clusterId=Some(clusterId))))
      ...
      case attachClusterPattern(resourceType, instanceType, imageId, clusterId) => Success(Right(AutoClusterConfiguration(resourceType, instanceType, imageId, clusterId = Some(clusterId))))
      ...
      case attachCllusterSpotPattern(resourceType, instanceType, imageId, spotStrategy, spotPriceLimit, clusterId) => Success(Right(AutoClusterConfiguration(resourceType, instanceType, imageId, Some(spotStrategy), Some(spotPriceLimit.toFloat), Some(clusterId))))
