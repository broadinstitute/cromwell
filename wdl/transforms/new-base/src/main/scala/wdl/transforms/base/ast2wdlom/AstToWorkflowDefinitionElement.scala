...
object AstToWorkflowDefinitionElement {
  ...
  def astToWorkflowDefinitionElement(implicit
    astNodeToWorkflowBodyElement: CheckedAtoB[GenericAstNode, WorkflowBodyElement]
    ...
    val bodyElementsValidation: ErrorOr[Vector[WorkflowBodyElement]] =
      a.getAttributeAsVector[WorkflowBodyElement]("body")(astNodeToWorkflowBodyElement).toValidated
  ...
  private def combineElements(name: String,
    ...
    val inputsSectionValidation: ErrorOr[Option[InputsSectionElement]] = for {
      inputValidateElement <- validateSize(bodyElements.filterByType[InputsSectionElement], "inputs", 1): ErrorOr[
        Option[InputsSectionElement]
      ]
    ...
    val intermediateValueDeclarationStdoutCheck = checkDisallowedIntermediates(
      bodyElements.filterByType[IntermediateValueDeclarationElement],
      StdoutElement,
      "stdout"
    )
    val intermediateValueDeclarationStderrCheck = checkDisallowedIntermediates(
      bodyElements.filterByType[IntermediateValueDeclarationElement],
      StderrElement,
      "stderr"
    )
    ...
    val outputsSectionValidation: ErrorOr[Option[OutputsSectionElement]] = for {
      outputValidateElement <- validateSize(bodyElements.filterByType[OutputsSectionElement], "outputs", 1): ErrorOr[
        Option[OutputsSectionElement]
      ]
    ...
    val metaSectionValidation: ErrorOr[Option[MetaSectionElement]] =
      validateSize(bodyElements.filterByType[MetaSectionElement], "meta", 1)
    val parameterMetaSectionValidation: ErrorOr[Option[ParameterMetaSectionElement]] =
      validateSize(bodyElements.filterByType[ParameterMetaSectionElement], "parameterMeta", 1)

    (inputsSectionValidation,
     outputsSectionValidation,
     metaSectionValidation,
     parameterMetaSectionValidation,
     intermediateValueDeclarationStdoutCheck,
     intermediateValueDeclarationStderrCheck
    ) mapN { (validInputs, validOutputs, meta, parameterMeta, _, _) =>
      WorkflowDefinitionElement(name,
                                validInputs,
                                graphSections.toSet,
                                validOutputs,
                                meta,
  ...
  def checkDisallowedInputElement(inputSection: Option[InputsSectionElement],
                                  expressionType: FunctionCallElement,
  ...
  def checkDisallowedOutputElement(outputSection: Option[OutputsSectionElement],
                                   expressionType: FunctionCallElement,
                                   expressionName: String
  ): ErrorOr[Unit] =
    outputSection match {
  ...
  def checkDisallowedIntermediates(intermediate: Vector[IntermediateValueDeclarationElement],
                                   expressionType: FunctionCallElement,
                                   expressionName: String
  ): ErrorOr[Unit] =
