...
class EjeaFetchingCachedOutputsFromDatabaseSpec extends EngineJobExecutionActorSpec with HasJobSuccessResponse {
  ...
  "An EJEA in FetchingCachedOutputsFromDatabase state" should {
    ...
    val possibleEjhaResponses = List(("no hashes", None),
  ...
  def initialData = ResponsePendingData(helper.backendJobDescriptor,
                                        helper.bjeaProps,
                                        None,
                                        None,
                                        Some(EJEACacheHit(CacheHit(callCachingEntryId), 2, None)),
                                        None
  )
  def ejeaInFetchingCachedOutputsFromDatabaseState(restarting: Boolean = false) = helper
    .buildEJEA(restarting = restarting, callCachingMode = CallCachingActivity(ReadAndWriteCache))
    .setStateInline(data = initialData)
}
