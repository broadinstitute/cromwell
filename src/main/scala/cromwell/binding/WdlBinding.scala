...
object WdlBinding {
  ...
  def findTopLevelMemberAccesses(expr: AstNode) = WdlBinding.findAstsWithTrail(expr, "MemberAccess").filter {
...
case class WdlBinding(ast: Ast, source: WdlSource, importResolver: ImportResolver, namespace: Option[String]) extends WdlValue {
  ...
  private def validate(): Unit = {
    ...
    workflowAsts foreach { workflowAst =>
      ...
      WdlBinding.findAsts(workflowAst, AstNodeName.Call) foreach { callAst =>
        ...
        val callName =
          if(callAst.getAttribute("alias") != null) sourceString(callAst.getAttribute("alias"))
          else sourceString(callAst.getAttribute("task"))
        ...
        call.inputMappings foreach { inputKv =>
          ...
          /* All MemberAccess ASTs that are not contained in other MemberAccess ASTs */
          WdlBinding.findTopLevelMemberAccesses(inputKv._2.ast).map(partialEvalMemberAccess).map {_.get}
        }
      }
    }
  }
  ...
  def partialEvalMemberAccess(ast: Ast): Try[Call] = {
    def callFromName(name: String): Try[Call] = {
      val callList = workflows.head.calls.filter {_.name == name}
      if (callList.isEmpty) {
        Failure(new SyntaxError(wdlSyntaxErrorFormatter.undefinedMemberAccess(ast)))
      } else {
        Success(callList.head)
      }
    }
    val rhs = sourceString(ast.getAttribute("rhs"))
    val lhs = ast.getAttribute("lhs") match {
      // Treat any MemberAccess in the left-hand-side to be interpreted as a "namespace identifier"
      // For example, "a.b.c.d" would invoke callFromName("a.b.c")
      case a: Ast => callFromName(WdlExpression.toString(a))
      case terminal: Terminal => callFromName(sourceString(terminal))
    }
