...
object WomToWdlom {
  ...
  def graphOutputNodeToWorkflowGraphElement: CheckedAtoB[GraphOutputNode, WorkflowGraphElement] =
    CheckedAtoB.fromCheck { a: GraphOutputNode =>
      a match {
        case a: ExpressionBasedGraphOutputNode =>
          ...
          } yield OutputDeclarationElement(typeElement, a.identifier.localName.value, expression)
        case _ =>
  ...
  def womBundleToFileElement: CheckedAtoB[WomBundle, FileElement] =
    CheckedAtoB.fromCheck { a: WomBundle =>
      ...
      for {
        workflows <- workflows
          .map(workflowDefinitionToWorkflowDefinitionElement(_))
          .toList
          .sequence[Checked, WorkflowDefinitionElement]
        tasks <- tasks
          .map(callableTaskDefinitionToTaskDefinitionElement(_))
          .toList
          .sequence[Checked, TaskDefinitionElement]
      } yield FileElement(
  ...
  def mapToParameterMetaSectionElement
    : CheckedAtoB[Map[String, MetaValueElement], Option[ParameterMetaSectionElement]] =
    CheckedAtoB.fromCheck { a: Map[String, MetaValueElement] =>
      if (a.nonEmpty)
  ...
  def runtimeAttributesToRuntimeAttributesSectionElement
    : CheckedAtoB[RuntimeAttributes, Option[RuntimeAttributesSectionElement]] =
    CheckedAtoB.fromCheck { a: RuntimeAttributes =>
      def tupleToKvPair(tuple: (String, WomExpression)): Checked[ExpressionElement.KvPair] =
        ...
        } yield ExpressionElement.KvPair(tuple._1, expressionElement)
  ...
  def outputDefinitionToOutputDeclarationElement: CheckedAtoB[OutputDefinition, OutputDeclarationElement] =
    CheckedAtoB.fromCheck { a: OutputDefinition =>
      ...
      } yield OutputDeclarationElement(typeElement, a.name, expression)
    }
  ...
  def callableTaskDefinitionToTaskDefinitionElement: CheckedAtoB[CallableTaskDefinition, TaskDefinitionElement] =
    CheckedAtoB.fromCheck { a: CallableTaskDefinition =>
  ...
  def workflowDefinitionToWorkflowDefinitionElement: CheckedAtoB[WorkflowDefinition, WorkflowDefinitionElement] =
    CheckedAtoB.fromCheck { a: WorkflowDefinition =>
      ...
      val inputs =
        a.inputs
          .filter(!_.localName.value.contains("."))
          .map(inputDefinitionToInputDeclarationElement(_))
          .sequence[Checked, InputDeclarationElement]
      val outputs =
        a.outputs
          .filter(!_.localName.value.contains("."))
          .map(outputDefinitionToOutputDeclarationElement(_))
          .sequence[Checked, OutputDeclarationElement]
      ...
      for {
        ...
        inputs <- inputs
        outputs <- outputs
        nodes <- selectWdlomRepresentableNodes(a.graph.nodes)
          .map(graphNodeToWorkflowGraphElement(_))
          .toList
          .sequence[Checked, WorkflowGraphElement]
      } yield WorkflowDefinitionElement(
        a.name,
        if (inputs.nonEmpty) Some(InputsSectionElement(inputs)) else None,
        nodes.toSet,
        if (outputs.nonEmpty) Some(OutputsSectionElement(outputs)) else None,
  ...
  def expressionNodeLikeToWorkflowGraphElement: CheckedAtoB[ExpressionNodeLike, WorkflowGraphElement] =
    CheckedAtoB.fromCheck {
      case a: ExpressionNode =>
        ...
        } yield IntermediateValueDeclarationElement(typeElement = typeElement,
    ...
    CheckedAtoB.fromCheck {
      ...
      case a: ConditionalNode =>
        for {
          ...
          nodes <- selectWdlomRepresentableNodes(a.innerGraph.nodes).toList
            .map(graphNodeToWorkflowGraphElement(_))
            .sequence[Checked, WorkflowGraphElement]
        } yield IfElement(
      ...
      case a: ScatterNode =>
        ...
        if (a.scatterVariableNodes.size != 1)
          ...
          for {
            ...
            graph <- selectWdlomRepresentableNodes(a.innerGraph.nodes).toList
              .map(graphNodeToWorkflowGraphElement(_))
              .sequence[Checked, WorkflowGraphElement]
          } yield ScatterElement(
  ...
  def graphNodeWithSingleOutputPortToWorkflowGraphElement
    : CheckedAtoB[GraphNodeWithSingleOutputPort, WorkflowGraphElement] =
    CheckedAtoB.fromCheck {
      case a: GraphInputNode =>
        womTypeToTypeElement(a.womType) map { typeElement =>
          InputDeclarationElement(typeElement, a.identifier.localName.value, None)
        }
      case a: ExpressionNode =>
        for {
          typeElement <- womTypeToTypeElement(a.womType)
          expression <- womExpressionToExpressionElement(a.womExpression)
        } yield IntermediateValueDeclarationElement(typeElement, a.identifier.localName.value, expression)
    }
  ...
  def womTypeToTypeElement: CheckedAtoB[WomType, TypeElement] =
    CheckedAtoB.fromCheck {
      ...
      case a: WomMapType =>
        ...
        } yield MapTypeElement(keyType, valueType)
      ...
      case a: WomPairType =>
        ...
        } yield PairTypeElement(leftType, rightType)
  ...
  def expressionNodeToExpressionElement: CheckedAtoB[ExpressionNode, ExpressionElement] =
    CheckedAtoB.fromCheck { a: ExpressionNode =>
      womExpressionToExpressionElement(a.womExpression)
    }
