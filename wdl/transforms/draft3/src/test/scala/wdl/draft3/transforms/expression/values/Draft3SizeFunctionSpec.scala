...
class Draft3SizeFunctionSpec extends FlatSpec with Matchers {
  ...
  it should "correctly report a 2048 byte file, in bytes by default" in {
    validate(Size(PrimitiveLiteralExpressionElement(WomSingleFile("blah")), None).evaluateValue(Map.empty, testFunctions(Success(2048l)))) {
      res => assert(res == WomFloat(2048d))
    }
  }

  it should "correctly report a 2048 byte file, in bytes" in {
    validate(Size(PrimitiveLiteralExpressionElement(WomSingleFile("blah")), Some(PrimitiveLiteralExpressionElement(WomString("B")))).evaluateValue(Map.empty, testFunctions(Success(2048l)))) {
      res => assert(res == WomFloat(2048d))
    }
  }

  it should "correctly report a 2048 byte file, in KB" in {
    validate(Size(PrimitiveLiteralExpressionElement(WomSingleFile("blah")), Some(PrimitiveLiteralExpressionElement(WomString("KB")))).evaluateValue(Map.empty, testFunctions(Success(2048l)))) {
      res => assert(res == WomFloat(2.048d))
    }
  }

  it should "correctly report a 2048 byte file, in KiB" in {
    validate(Size(PrimitiveLiteralExpressionElement(WomSingleFile("blah")), Some(PrimitiveLiteralExpressionElement(WomString("KiB")))).evaluateValue(Map.empty, testFunctions(Success(2048l)))) {
  ...
  it should "correctly report the size of a supplied, optional, 2048 byte file" in {
    ...
    validate(Size(IdentifierLookup("x"), None).evaluateValue(Map("x" -> value), testFunctions(Success(2048l)))) {
      res => assert(res == WomFloat(2048d))
    }
  }
  ...
  it should "correctly report the size of a supplied, optional, 2048 byte file, in MB" in {
    ...
    validate(Size(IdentifierLookup("x"), Some(PrimitiveLiteralExpressionElement(WomString("MB")))).evaluateValue(Map("x" -> value), testFunctions(Success(2048l)))) {
  ...
  it should "correctly report the size of an array of files, in GiB" in {
    ...
    validate(Size(IdentifierLookup("x"), Some(PrimitiveLiteralExpressionElement(WomString("GiB")))).evaluateValue(Map("x" -> value), testFunctions(Success(2048l)))) {
