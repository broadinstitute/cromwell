...
trait PipelinesApiDockerCacheMappingOperations {
  ...
  def generateDockerImageToDiskImageMapping(auth: GoogleAuthMode,
                                            dockerImageCacheManifestFile: ValidFullGcsPath
  ): Map[String, DockerImageCacheEntry] = {
    ...
    dockerImageToCacheDiskImageMappingOpt
      ...
      .filter { cachedDockerImageDigestAndDiskName =>
        ...
        if (hashStartingPositionInActualDockerImage != -1) {
          val actualDigestOfDesiredDockerImage =
            dockerImageWithDigest.substring(hashStartingPositionInActualDockerImage + 1)
          if (cachedDockerImageDigestAndDiskName.dockerImageDigest == actualDigestOfDesiredDockerImage) {
            true
          } else {
            jobLogger.info(
              s"Cached Docker image digest mismatch. Requested docker image $dockerImageAsSpecifiedByUser has different digest than " +
          ...
          jobLogger.error(
            s"Programmer error ! Odd docker image name where supposed to be name with digest: $dockerImageWithDigest"
          )
          false
        }
      }
  ...
  private[common] def readDockerImageCacheManifestFileFromGCS(gcsClient: Storage,
                                                              gcsPath: ValidFullGcsPath
  ): IO[DockerImageCacheManifest] = {
        ...
        decode[DockerImageCacheManifest](jsonStr) match {
          ...
          case Right(parsedManifest) =>
            if (parsedManifest.manifestFormatVersion == CURRENT_SUPPORTED_MANIFEST_FORMAT_VERSION) {
              ...
              IO.raiseError(
                new DockerImageManifestVersionError(
                  s"Current supported docker image cache manifest format version " +
