...
class MetadataStatisticsRecorderSpec extends AnyFlatSpec with Matchers {
  ...
  def uninterestingWriteEvent(workflowId: WorkflowId)(): MetadataEvent =
    MetadataEvent(MetadataKey(workflowId, None, "foo"), MetadataValue(WomInteger(1)))
  def parentNotificationEvent(rootWorkflowId: WorkflowId,
                              parentWorkflowId: WorkflowId,
                              subWorkflowId: WorkflowId
  ): Vector[MetadataEvent] = Vector(
    MetadataEvent(MetadataKey(subWorkflowId, None, "rootWorkflowId"),
                  MetadataValue(WomString(rootWorkflowId.toString))
    ),
    MetadataEvent(MetadataKey(subWorkflowId, None, "parentWorkflowId"),
                  MetadataValue(WomString(parentWorkflowId.toString))
    )
  )
  ...
  it should "count rows for one workflow and create alerts every interval" in {
    ...
    (1 to 10) foreach { i =>
      ...
      recorder.processEventsAndGenerateAlerts(1 of uninterestingWriteEvent(workflowId)) should be(
        Vector(HeavyMetadataAlert(workflowId, 10L * i))
      )
      ()
    }
  }
  ...
  it should "be able to reset counters from odd values" in {
    ...
    (1 to 10) foreach { i =>
      ...
      recorder.processEventsAndGenerateAlerts(bigMetadataDump of uninterestingWriteEvent(workflowId)) should be(
        Vector(HeavyMetadataAlert(workflowId, expectedCountAfterProcessing))
      )
  ...
  it should "count rows for multiple workflow and create alerts every interval" in {
    ...
    recorder
      .processEventsAndGenerateAlerts(
        (3 of uninterestingWriteEvent(workflowId1)) ++
          (5 of uninterestingWriteEvent(workflowId2)) ++
          (7 of uninterestingWriteEvent(workflowId3))
      )
    ...
    recorder
      ...
      .toSet should be(
      Set(HeavyMetadataAlert(workflowId1, 12), HeavyMetadataAlert(workflowId2, 20), HeavyMetadataAlert(workflowId3, 28))
    )
  }
  ...
  it should "be able to accumulate counts from subworkflows" in {
    ...
    withClue(recorder.statusString()) {
      recorder.processEventsAndGenerateAlerts(7 of uninterestingWriteEvent(rootWorkflowId)) should be(
        Vector(HeavyMetadataAlert(rootWorkflowId, 11))
      )
    }
    ...
    withClue(recorder.statusString()) {
      recorder.processEventsAndGenerateAlerts(7 of uninterestingWriteEvent(subWorkflow1Id)) should be(
        Vector(HeavyMetadataAlert(subWorkflow1Id, 11))
      )
    }
    ...
    withClue(recorder.statusString()) {
      recorder.processEventsAndGenerateAlerts(7 of uninterestingWriteEvent(subWorkflow2Id)) should be(
        Vector(HeavyMetadataAlert(rootWorkflowId, 25))
      )
    }
    ...
    withClue(recorder.statusString()) {
      recorder.processEventsAndGenerateAlerts(7 of uninterestingWriteEvent(subWorkflow1Id)) should be(
        Vector(HeavyMetadataAlert(subWorkflow1Id, 25))
      )
    }
    ...
    withClue(recorder.statusString()) {
      recorder.processEventsAndGenerateAlerts(7 of uninterestingWriteEvent(subWorkflow2Id)).toSet should be(
        Set(HeavyMetadataAlert(rootWorkflowId, 39), HeavyMetadataAlert(subWorkflow2Id, 16))
      )
    }
