...
class EnhancedCollectionsSpec extends FlatSpec with Matchers {
  ...
  it should "behead an empty queue" in {
    ...
    val (head, newQueue) = q.takeWhileWeighted(10, identity[Int], None)
    head shouldBe empty
  ...
  it should "behead a non empty queue with a max length of 0" in {
    val q = Queue(1)
    val (head, newQueue) = q.takeWhileWeighted(10, identity[Int], Option(0))
    head shouldBe empty
    newQueue shouldBe q
  }
  ...
  it should "use the weight function to compute weight and respect the max weight" in {
    val q = Queue(1, 2, 5)
    val (head, newQueue) = q.takeWhileWeighted(maxWeight = 10, _ * 2, None)
  ...
  it should "respect the max length" in {
    val q = Queue(1, 2, 5)
    val (head, newQueue) = q.takeWhileWeighted(maxWeight = 10, identity[Int], Option(2))
  ...
  it should "not enable the strict flag by default and stop at the last element that goes over maxWeight" in {
    val q = Queue(1, 2, 5)
    val (head, newQueue) = q.takeWhileWeighted(maxWeight = 4, identity[Int], Option(3))
    head shouldBe Vector(1, 2)
    newQueue shouldBe Queue(5)
  }
  ...
  it should "respect when the strict flag is enabled and drop messages over maxWeight" in {
    val q = Queue(1, 5, 2)
    val (head, newQueue) = q.takeWhileWeighted(maxWeight = 4, identity[Int], Option(3), strict = true)
    head shouldBe Vector(1, 2)
    newQueue shouldBe empty
  }
