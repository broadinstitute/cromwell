...
class TesRuntimeAttributesSpec extends WordSpecLike with Matchers with Mockito {
  ...
  private def assertSuccess(runtimeAttributes: Map[String, WdlValue], expectedRuntimeAttributes: TesRuntimeAttributes): Unit = {
    try {
      assert(TesRuntimeAttributes(runtimeAttributes) == expectedRuntimeAttributes)
    } catch {
  ...
  private def assertFailure(runtimeAttributes: Map[String, WdlValue], exMsg: String): Unit = {
    try {
      TesRuntimeAttributes(runtimeAttributes)
