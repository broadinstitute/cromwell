...
class IntervalLimitedAwsJobSubmitActor(configRegion: Option[Region]) extends Actor with ActorLogging {
  ...
  implicit val timer: Timer[IO] =
    cats.effect.IO.timer(ExecutionContext.fromExecutor(Executors.newSingleThreadScheduledExecutor))
...
object IntervalLimitedAwsJobSubmitActor {
  ...
  final case class SubmitAwsJobRequest(job: AwsBatchJob,
                                       attributes: AwsBatchAttributes,
                                       completionPromise: Promise[SubmitJobResponse]
  ) extends IntervalLimitedAwsJobSubmitActorMessage
