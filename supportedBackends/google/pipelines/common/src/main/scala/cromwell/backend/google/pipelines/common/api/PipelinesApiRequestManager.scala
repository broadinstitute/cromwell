...
/**
  ...
  */
class PipelinesApiRequestManager(val qps: Int Refined Positive,
  ...
  private def papiRequestWorkerProps = PipelinesApiRequestWorker
    .props(self, workerBatchInterval, serviceRegistryActor)
  ...
  private case class BeheadedWorkQueue(workToDo: Option[NonEmptyList[PAPIApiRequest]],
                                       newWorkQueue: Queue[PAPIApiRequest]
  )
...
object PipelinesApiRequestManager {
  ...
  def props(qps: Int Refined Positive, requestWorkers: Int Refined Positive, serviceRegistryActor: ActorRef)(implicit
    ...
    Props(new PipelinesApiRequestManager(qps, requestWorkers, serviceRegistryActor)).withDispatcher(BackendDispatcher)
  ...
  case class PAPIStatusPollRequest(workflowId: WorkflowId,
                                   ...
                                   backoff: Backoff = PAPIApiRequest.backoff
  ) extends PAPIApiRequest {
  ...
  final case class PipelinesApiStatusQueryFailed(query: PAPIApiRequest, cause: PAPIApiException)
      extends PAPIApiRequestFailed
  final case class PipelinesApiRunCreationQueryFailed(query: PAPIApiRequest, cause: PAPIApiException)
      extends PAPIApiRequestFailed
  final case class PipelinesApiAbortQueryFailed(query: PAPIApiRequest, cause: PAPIApiException)
      extends PAPIApiRequestFailed

  final private[api] case class PipelinesApiWorkBatch(workBatch: NonEmptyList[PAPIApiRequest])
