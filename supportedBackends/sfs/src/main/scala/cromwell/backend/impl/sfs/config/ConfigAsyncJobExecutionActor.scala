...
sealed trait ConfigAsyncJobExecutionActor extends SharedFileSystemAsyncJobExecutionActor {
  ...
  def writeTaskScript(script: Path, taskName: String, inputs: WorkflowCoercedInputs): List[WomFile] = {
    ...
    // MLC it was like that when I got here
    val InstantiatedCommand(command, createdFiles) = task.instantiateCommand(task.inputsFromMap(inputsWithFqns), NoFunctions).toTry.get
