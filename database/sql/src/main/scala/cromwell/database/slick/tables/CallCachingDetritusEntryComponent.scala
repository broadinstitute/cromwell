...
trait CallCachingDetritusEntryComponent {
  ...
  class CallCachingDetritusEntries(tag: Tag)
    ...
    def fkCallCachingDetritusEntryCallCachingEntryId =
      foreignKey("FK_CALL_CACHING_DETRITUS_ENTRY_CALL_CACHING_ENTRY_ID", callCachingEntryId, callCachingEntries)(
  ...
  val callCachingDetritusEntriesForCallCachingEntryId = Compiled((callCachingEntryId: Rep[Long]) =>
    for {
