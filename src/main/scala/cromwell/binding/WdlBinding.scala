...
class WdlBinding(ast: Ast) {
    ...
    workflow.calls.foreach{_.setParent(workflow)}
    ...
    private def processCall(ast: Ast): Call = {
        ...
        val task = findTask(taskName) getOrElse{throw new UnsupportedOperationException("Cannot find task with name: " + taskName)}
