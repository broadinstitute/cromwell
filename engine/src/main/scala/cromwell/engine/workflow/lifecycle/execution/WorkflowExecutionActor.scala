...
case class WorkflowExecutionActor(params: WorkflowExecutionActorParams)
  ...
  private val backendFactories: Map[String, BackendLifecycleActorFactory] = {
    val factoriesValidation = workflowDescriptor.backendAssignments.values.toList
      .traverse[ErrorOr, (String, BackendLifecycleActorFactory)] { backendName =>
        CromwellBackends.backendLifecycleFactoryActorByName(backendName) map { backendName -> _ }
      }
  ...
  val executionStore: ErrorOr[ActiveExecutionStore] =
    ExecutionStore(workflowDescriptor.callable, params.totalJobsByRootWf, TotalMaxJobsPerRootWf)
    ...
    case Valid(validExecutionStore) =>
      startWith(
        ...
        WorkflowExecutionActorData(workflowDescriptor,
                                   ioEc,
                                   new AsyncIo(params.ioActor, GcsBatchCommandBuilder),
  ...
  when(WorkflowExecutionAbortingState) {
    ...
    case Event(JobFailedNonRetryableResponse(jobKey: BackendJobDescriptorKey,
                                             failure: JobReconnectionNotSupportedException,
                                             _
               ),
               _
        ) if restarting =>
  ...
  def measureTimeBetweenHeartbeats(): Unit = {
    ...
    previousHeartbeatTime foreach { previous =>
      sendGauge(
        NonEmptyList("workflows", List("workflowexecutionactor", "heartbeat", "interval_millis", "set")),
        now.toInstant.toEpochMilli - previous.toInstant.toEpochMilli
      )
    }
  ...
  whenUnhandled {
    ...
    case Event(r: JobSucceededResponse, stateData) =>
      if (r.resultGenerationMode != RunOnBackend) {
        workflowLogger.info(
          s"Job results retrieved (${r.resultGenerationMode}): '${r.jobKey.call.fullyQualifiedName}' (scatter index: ${r.jobKey.index}, attempt ${r.jobKey.attempt})"
        )
      }
      handleCallSuccessful(r.jobKey, r.jobOutputs, r.returnCode, stateData, Map.empty, Set(workflowDescriptor.id))
    // Sub Workflow
    case Event(
          SubWorkflowSucceededResponse(jobKey, descendantJobKeys, rootAndSubworklowIds, callOutputs, cumulativeOutputs),
          currentStateData
        ) =>
      ...
      if (subworkflowOutputs.size == callOutputs.outputs.size) {
        handleCallSuccessful(jobKey,
                             CallOutputs(subworkflowOutputs),
                             None,
                             currentStateData,
                             descendantJobKeys,
                             rootAndSubworklowIds,
                             cumulativeOutputs
        )
      } else {
        handleNonRetryableFailure(
          currentStateData,
          jobKey,
          new Exception(s"Subworkflow produced outputs: [${callOutputs.outputs.keys
              .mkString(", ")}], but we expected all of [${jobKey.node.subworkflowCallOutputPorts.map(_.internalName)}]")
        )
      }
    // Expression
      ...
      expressionKey.node match {
        case _: ExposedExpressionNode | _: ExpressionBasedGraphOutputNode =>
          workflowLogger.debug(
            s"Expression evaluation succeeded: '${expressionKey.node.fullyQualifiedName}' (scatter index: ${expressionKey.index}, attempt: ${expressionKey.attempt})"
          )
  ...
  override protected def onFailure(actorRef: ActorRef, throwable: => Throwable) = {
    ...
    val jobKey = stateData.jobKeyActorMappings.getOrElse(
      actorRef,
      throw new RuntimeException(
        "Programmer Error: A job or sub workflow actor has terminated but was not assigned a jobKey"
      )
    )
    val jobStatus = stateData.executionStore
      .jobStatus(jobKey)
      .getOrElse(
        throw new RuntimeException(
          s"Programmer Error: An actor representing ${jobKey.tag} which this workflow is not running has sent up a terminated message."
        )
      )

    if (!jobStatus.isTerminalOrRetryable) {
      val terminationException =
        new RuntimeException(s"Unexpected failure or termination of the actor monitoring ${jobKey.tag}", throwable)
  ...
  private def handleAbortCommand(data: WorkflowExecutionActorData) = {
    ...
    params.backendSingletonCollection.backendSingletonActors.values.flatten.foreach {
      _ ! BackendSingletonActorAbortWorkflow(workflowIdForLogging)
    }
  ...
  private def handleWorkflowSuccessful(data: WorkflowExecutionActorData) = {
    ...
    def handleSuccessfulWorkflowOutputs(outputs: Map[GraphOutputNode, WomValue]) = {
      ...
      context.parent ! WorkflowExecutionSucceededResponse(data.jobExecutionMap,
                                                          currentRootAndSubworkflowIds,
                                                          localOutputs,
                                                          currentCumulativeOutputs
      )
    ...
    val workflowOutputValuesValidation = workflowOutputNodes
      ...
      .toList
      .traverse[IOChecked, (GraphOutputNode, WomValue)] {
  ...
  private def handleCallAborted(data: WorkflowExecutionActorData,
                                jobKey: JobKey,
                                jobExecutionMap: JobExecutionMap = Map.empty
  ) = {
  ...
  private def pushBackendStatusUnknown(jobKey: BackendJobDescriptorKey): Unit = {
    val unknownBackendStatus =
      MetadataEvent(metadataKeyForCall(jobKey, CallMetadataKeys.BackendStatus), MetadataValue("Unknown"))
  ...
  private def handleRetryableFailure(jobKey: BackendJobDescriptorKey, reason: Throwable, returnCode: Option[Int]) = {
    ...
    val executionDiff = WorkflowExecutionDiff(
      Map(jobKey -> ExecutionStatus.RetryableFailure, newJobKey -> ExecutionStatus.NotStarted)
    )
  ...
  private def handleCallSuccessful(jobKey: JobKey,
    ...
    stay() using data
      .callExecutionSuccess(jobKey, outputs, cumulativeOutputs, rootAndSubworkflowIds)
      .addExecutions(jobExecutionMap)
  }

  private def handleDeclarationEvaluationSuccessful(key: ExpressionKey,
  ...
  override def receive: Receive = { case msg =>
    ...
    sendGauge(
      NonEmptyList(
        "workflows",
        List("workflowexecutionactor", this.stateName.toString, msg.getClass.getSimpleName, "processing_millis", "set")
      ),
      OffsetDateTime.now.toInstant.toEpochMilli - starttime.toInstant.toEpochMilli
    )
  }
  ...
  private def startRunnableNodes(data: WorkflowExecutionActorData): WorkflowExecutionActorData = {
    ...
    def updateExecutionStore(diffs: List[WorkflowExecutionDiff],
                             updatedData: WorkflowExecutionActorData
    ): WorkflowExecutionActorData = {
      val notStartedBackendJobs = diffs.flatMap(d =>
        d.executionStoreChanges.collect {
      ...
      if (data.totalJobsByRootWf.addAndGet(notStartedBackendJobsCt) > TotalMaxJobsPerRootWf) {
        ...
        val updatedDiffs =
          diffs.map(d => d.copy(executionStoreChanges = d.executionStoreChanges -- notStartedBackendJobs))

        notStartedBackendJobs.foreach { key =>
          val errorMsg =
            s"Job $key failed to be created! Error: Root workflow tried creating ${data.totalJobsByRootWf.get} jobs, which is more than $TotalMaxJobsPerRootWf, the max cumulative jobs allowed per root workflow"
          workflowLogger.error(errorMsg)
    ...
    val keyStartDiffs: List[WorkflowExecutionDiff] = runnableKeys map { k =>
      k -> (k match {
        ...
        case key @ ExpressionKey(expr: TaskCallInputExpressionNode, _) =>
          processRunnableTaskCallInputExpression(key, data, expr)
    ...
    } map { case (key: JobKey, value: ErrorOr[WorkflowExecutionDiff]) =>
      value.valueOr { errors =>
        self ! JobFailedNonRetryableResponse(key,
                                             new Exception(errors.toList.mkString(System.lineSeparator))
                                               with NoStackTrace,
                                             None
        )
    ...
    sendGauge(
      NonEmptyList("workflows", List("workflowexecutionactor", "startRunnableNodes", "duration_millis", "set")),
      OffsetDateTime.now.toInstant.toEpochMilli - startRunnableStartTimestamp.toInstant.toEpochMilli
    )
    result
  }
  ...
  private def processRunnableTaskCallInputExpression(key: ExpressionKey,
                                                     data: WorkflowExecutionActorData,
                                                     expressionNode: TaskCallInputExpressionNode
  ): ErrorOr[WorkflowExecutionDiff] = {
    import cats.syntax.either._
    val taskCallNode: CommandCallNode = expressionNode.taskCallNodeReceivingInput.get(())

    (for {
      backendJobDescriptorKey <- data.executionStore.backendJobDescriptorKeyForNode(
        taskCallNode
      ) toChecked s"No BackendJobDescriptorKey found for call node ${taskCallNode.identifier.fullyQualifiedName}"
      factory <- backendFactoryForTaskCallNode(taskCallNode)
      backendInitializationData = params.initializationData.get(factory.name)
      functions = factory.expressionLanguageFunctions(workflowDescriptor.backendDescriptor,
  ...
  private def backendFactoryForTaskCallNode(taskCallNode: CommandCallNode): Checked[BackendLifecycleActorFactory] =
    for {
      name <- workflowDescriptor.backendAssignments
        .get(taskCallNode)
        .toChecked(s"Cannot find an assigned backend for call ${taskCallNode.fullyQualifiedName}")
   ...
   */
  private def processRunnableJob(key: BackendJobDescriptorKey,
                                 data: WorkflowExecutionActorData
  ): ErrorOr[WorkflowExecutionDiff] = {
    import cats.syntax.either._
   ...
   */
  private def processRunnableSubWorkflow(key: SubWorkflowKey,
                                         data: WorkflowExecutionActorData
  ): ErrorOr[WorkflowExecutionDiff] = {
...
object WorkflowExecutionActor {
  ...
  case class WorkflowExecutionAbortedResponse(jobExecutionMap: JobExecutionMap)
      extends WorkflowExecutionActorResponse
      with EngineLifecycleActorAbortedResponse {
    override def toString = "WorkflowExecutionAbortedResponse"
  }

  final case class WorkflowExecutionFailedResponse(jobExecutionMap: JobExecutionMap, reason: Throwable)
      extends WorkflowExecutionActorResponse {
  ...
  case class SubWorkflowSucceededResponse(key: SubWorkflowKey,
                                          ...
                                          outputs: CallOutputs,
                                          cumulativeOutputs: Set[WomValue] = Set.empty
  ) extends SubWorkflowTerminalStateResponse
