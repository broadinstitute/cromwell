...
class ConfigInitializationActorSpec extends AnyFlatSpec with CromwellTimeoutSpec with Matchers
  ...
  private val attributeValidationTests: TableFor4[String, String, Map[String, WomValue], ErrorOr[_]] = Table(
    ...
    ("missing required custom", "Int my_attr", Map(), "Expecting my_attr runtime attribute to be an Integer".invalidNel),
    ...
    ("present required memory", "Float memory_gb", Map("memory" -> WomString("3 GB")), MemorySize(3, MemoryUnit.GB).valid),
    ("missing required memory", "Float memory_gb", Map(), "Expecting memory runtime attribute to be an Integer or String with format '8 GB'. Exception: Not supported WDL type value".invalidNel),
    ("present defaulted memory", "Float memory_gb = 1", Map("memory" -> WomString("3 GB")), MemorySize(3, MemoryUnit.GB).valid),
    ...
    ("present defaulted optional memory", "Float? memory_gb = 1", Map("memory" -> WomString("3 GB")), Option(MemorySize(3, MemoryUnit.GB)).valid),
