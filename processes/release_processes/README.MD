...
users. 
2. Check that all most recent executions of Cromwell's Jenkins tests have succeeded, and that all those executions were 
...
will need to be on the Broad internal network or VPN to open the following links):
    ...
    2. Tests for Cromwell in Terra environment. Log into Jenkins [here](https://fc-jenkins.dsp-techops.broadinstitute.org), check the tests [here](https://fc-jenkins.dsp-techops.broadinstitute.org/view/Batch/).
3. [Run the publish script to create a new version of Cromwell](#how-to-publish-a-new-cromwell-version)  
4. [Run through the "How to Release Cromwell into Firecloud/Terra" process](#how-to-release-cromwell-into-firecloud--terra)  
5. [Run through the "How to Deploy Cromwell in CaaS prod" process](#how-to-deploy-cromwell-in-caas-staging-and-caas-prod)  
