...
object WorkflowDefinitionElementToWomWorkflowDefinition extends Util {
  ...
  def convert(b: WorkflowDefinitionConvertInputs)(implicit
  ...
  ): ErrorOr[WorkflowDefinition] = {

    val a: WorkflowDefinitionConvertInputs = eliminateInputDependencies(b)

    val (meta, parameterMeta) =
      processMetaSections(a.definitionElement.metaSection, a.definitionElement.parameterMetaSection)
    ...
    val innerGraph: ErrorOr[WomGraph] = convertGraphElements(
      GraphLikeConvertInputs(
        graphNodeElements,
        Set.empty,
        Map.empty,
        a.typeAliases,
        a.definitionElement.name,
  ...
  def convertGraphElements(a: GraphLikeConvertInputs)(implicit
    expressionValueConsumer: ExpressionValueConsumer[ExpressionElement],
    fileEvaluator: FileEvaluator[ExpressionElement],
    typeEvaluator: TypeEvaluator[ExpressionElement],
    valueEvaluator: ValueEvaluator[ExpressionElement]
    ...
    for {
      linkedGraph <- LinkedGraphMaker.make(nodes = a.graphElements,
                                           seedGeneratedValueHandles ++ finished,
                                           typeAliases = a.typeAliases,
                                           callables = a.callables
      )
      womGraph <- makeWomGraph(linkedGraph,
                               a.seedNodes,
                               a.externalUpstreamCalls,
                               a.workflowName,
                               a.insideAScatter,
  ...
  private def makeWomGraph(linkedGraph: LinkedGraph,
    ...
    def graphNodeCreationFold(currentValidation: ErrorOr[List[GraphNode]],
                              next: WorkflowGraphElement
    ): ErrorOr[List[GraphNode]] = {
      ...
      currentValidation flatMap { currentList =>
        ...
        val generatedGraphNodesValidation: ErrorOr[Set[GraphNode]] =
          WorkflowGraphElementToGraphNode.convert(
            GraphNodeMakerInputs(
              next,
              upstreamCallNodes,
              linkedGraph.consumedValueLookup,
              availableValues,
              linkedGraph.typeAliases,
              workflowName,
              insideAScatter,
    ...
    val graphNodesValidation = LinkedGraphMaker.getOrdering(linkedGraph) flatMap {
      ordering: List[WorkflowGraphElement] =>
  ...
  private def eliminateInputDependencies(
    a: WorkflowDefinitionConvertInputs
    ...
    val inputElementsWithUpstreams: Seq[NewInputElementsSet] =
      a.definitionElement.inputsSection.map(_.inputDeclarations).getOrElse(Seq.empty) collect {
    ...
    if (inputElementsWithUpstreams.nonEmpty) {
      ...
      val newGraphNodeSet: Set[WorkflowGraphElement] =
        a.definitionElement.graphElements ++ inputElementsWithUpstreams.map(_.newDeclaration)

      val newWorkflowDefinitionElement: WorkflowDefinitionElement = {
        val withNewInputElements =
          a.definitionElement.copy(inputsSection = newInputsSection, graphElements = newGraphNodeSet)
