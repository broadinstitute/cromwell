...
class AstToWorkflowDefinitionElementSpec extends AnyFlatSpec {
  ...
  val mockInputSectionStdout = Option(
    InputsSectionElement(
      Vector(InputDeclarationElement(PrimitiveTypeElement(WomSingleFileType), "i", Some(StdoutElement)))
    )
  )
  val mockInputSectionStderr = Option(
    InputsSectionElement(
      Vector(InputDeclarationElement(PrimitiveTypeElement(WomSingleFileType), "i", Some(StderrElement)))
    )
  )
  val mockInputSectionNonStd = Option(
    InputsSectionElement(
      Vector(InputDeclarationElement(PrimitiveTypeElement(WomStringType), "more", Some(StringLiteral("more"))))
    )
  )

  val mockIntermediatesStdout = Vector(
    IntermediateValueDeclarationElement(PrimitiveTypeElement(WomSingleFileType), "y", StdoutElement)
  )
  val mockIntermediatesStderr = Vector(
    IntermediateValueDeclarationElement(PrimitiveTypeElement(WomSingleFileType), "y", StderrElement)
  )
  val mockIntermediatesNonStd = Vector(
    IntermediateValueDeclarationElement(PrimitiveTypeElement(WomStringType), "here", StringLiteral("here"))
  )

  val mockOutputSectionStdout = Option(
    OutputsSectionElement(Vector(OutputDeclarationElement(PrimitiveTypeElement(WomSingleFileType), "s", StdoutElement)))
  )
  val mockOutputSectionStderr = Option(
    OutputsSectionElement(Vector(OutputDeclarationElement(PrimitiveTypeElement(WomSingleFileType), "s", StderrElement)))
  )
  val mockOutputSectionNonStd = Option(
    OutputsSectionElement(
      Vector(OutputDeclarationElement(PrimitiveTypeElement(WomStringType), "more", StringLiteral("more")))
    )
  )
  ...
  it should "return an error when there is an stdout at intermediate declaration section" in {
    ...
    testIntermediates match {
      ...
      case Invalid(e) =>
        e.head should be(
          "Workflow cannot have stdout expression at intermediate declaration section at workflow-level."
        )
    }
  }
  it should "return an error when there is an stderr at intermediate declaration section" in {
    ...
    testIntermediates match {
      ...
      case Invalid(e) =>
        e.head should be(
          "Workflow cannot have stderr expression at intermediate declaration section at workflow-level."
        )
    }
  }
  ...
  it should "not return an error for non-stdout/stderr in the intermediates section" in {
    ...
    testIntermediates match {
      case Valid(_) => // No action
      case Invalid(_) =>
        fail("Check shouldn't have returned error as intermediate section had non-stdout/stderr intermediates.")
    }
  }
