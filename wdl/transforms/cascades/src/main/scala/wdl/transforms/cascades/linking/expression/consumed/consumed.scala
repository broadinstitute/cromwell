...
import wdl.transforms.cascades.linking.expression.consumed.cascadesExpressionValueConsumers._

package object consumed {

  implicit val expressionElementUnlinkedValueConsumer: ExpressionValueConsumer[ExpressionElement] =
    new ExpressionValueConsumer[ExpressionElement] {
      override def expressionConsumedValueHooks(a: ExpressionElement)(implicit
        expressionValueConsumer: ExpressionValueConsumer[ExpressionElement]
      ): Set[UnlinkedConsumedValueHook] = a match {
        ...
        case other =>
          throw new Exception(s"Cannot generate consumed values for ExpressionElement ${other.getClass.getSimpleName}")
      }
    }
}
