...
class PositiveIntRuntimeAttributesValidation(override val key: String) extends
  ...
  override protected def validateCoercedValue(womValue: WomInteger): ErrorOr[Int Refined Positive] = refineV[Positive](womValue.value.toInt).leftMap(NonEmptyList.one).toValidated
