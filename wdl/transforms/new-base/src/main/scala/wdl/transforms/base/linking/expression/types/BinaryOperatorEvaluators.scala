...
object BinaryOperatorEvaluators {
  ...
  private def forOperation[A <: BinaryOperation](op: (WomType, WomType) => Try[WomType]) = new TypeEvaluator[A] {
    override def evaluateType(a: A, linkedValues: Map[UnlinkedConsumedValueHook, GeneratedValueHandle],typeAliases: Map[String, WomType])(implicit
      expressionTypeEvaluator: TypeEvaluator[ExpressionElement]
    ): ErrorOr[WomType] =
      (a.left.evaluateType(linkedValues, typeAliases), a.right.evaluateType(linkedValues, typeAliases)) flatMapN { (left, right) =>
