...
class GcpBatchDockerCacheMappingOperationsSpec
  ...
  private val batchDockerCacheMappingOperationsMock = new GcpBatchDockerCacheMappingOperations {}
  ...
  it should "successfully parse docker image cache manifest JSON file as instance of Map[String, String]" in {
    ...
    val parsedJsonAsManifestIO =
      batchDockerCacheMappingOperationsMock invokePrivate readFileFromGcsPrivateMethod(mockGcsClient, testJsonGcsPath)
  ...
  it should "not use docker image cache if digest of requested image and cached images don't match" in {
    ...
    val dockerImageCacheDiskOpt =
      batchDockerCacheMappingOperationsMock.getDockerCacheDiskImageForAJob(
