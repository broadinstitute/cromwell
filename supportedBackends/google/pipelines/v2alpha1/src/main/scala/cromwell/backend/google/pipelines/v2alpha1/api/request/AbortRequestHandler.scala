...
trait AbortRequestHandler extends LazyLogging { this: RequestHandler =>
  protected def handleGoogleError(abortQuery: PAPIAbortRequest,
    ...
    if (Option(e.getCode).contains(400) || StringUtils.contains(e.getMessage, "Precondition check failed")) {
      logger.info(
        s"PAPI declined to abort job ${abortQuery.jobId.jobId} in workflow ${abortQuery.workflowId}, most likely because it is no longer running. Marking as finished. Message: ${e.getMessage}"
      )
      abortQuery.requester ! PAPIOperationIsAlreadyTerminal(abortQuery.jobId.jobId)
      Success(())
    } else {
      pollingManager ! PipelinesApiAbortQueryFailed(abortQuery,
  ...
  def handleRequest(abortQuery: PAPIAbortRequest, batch: BatchRequest, pollingManager: ActorRef)(implicit
    ec: ExecutionContext
  ): Future[Try[Unit]] =
