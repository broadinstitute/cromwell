...
trait JobKeyValueEntryComponent {
  ...
  class JobKeyValueEntries(tag: Tag) extends Table[JobKeyValueEntry](tag, "JOB_KEY_VALUE_ENTRY") {
    ...
    override def * = (workflowExecutionUuid,
                      ...
                      storeValue,
                      jobKeyValueEntryId.?
    ) <> (JobKeyValueEntry.tupled, JobKeyValueEntry.unapply)

    def ucJobKeyValueEntryWeuCfqnJiJaSk = index(
      "UC_JOB_KEY_VALUE_ENTRY_WEU_CFQN_JI_JA_SK",
      (workflowExecutionUuid, callFullyQualifiedName, jobIndex, jobAttempt, storeKey),
      unique = true
    )
  }
  ...
  val jobKeyValueEntriesForWorkflowExecutionUuid = Compiled((workflowExecutionUuid: Rep[String]) =>
    for {
