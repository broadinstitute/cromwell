...
object PartialWorkflowSources {
  ...
  val allKeys = List(
    ...
    labelsKey,
    WdlDependenciesKey,
    WorkflowDependenciesKey,
    workflowOnHoldKey,
    RequestedWorkflowIdKey
  )
  ...
  ): Try[Seq[WorkflowSourceFilesCollection]] = {
    import cats.syntax.apply._
    import cats.syntax.traverse._
    import cats.syntax.validated._
    ...
    val partialSources: ErrorOr[PartialWorkflowSources] = {
        ...
        if (actual == out) {
          val warning =
            Array(
              ...
              s"Please switch to using '$in' in future submissions."
            ).mkString(" ")
          log.warn(warning)
          warning
        } else ""
      ...
      val workflowSourceFinal: ErrorOr[Option[String]] = (wdlSource, workflowSource, workflowUrl) match {
        ...
        case (Some(_), Some(_), Some(_)) =>
          s"$WdlSourceKey, $WorkflowSourceKey and $WorkflowUrlKey all 3 can't be supplied".invalidNel
      ...
      val workflowInputsAux: ErrorOr[Map[Int, String]] = formData.toList
        .flatTraverse[ErrorOr, (Int, String)] {
      ...
      val wdlDependenciesDeprecationWarning: String => String =
      ...
      (unrecognized,
       ...
       workflowInputs,
       workflowInputsAux,
       workflowDependenciesFinal,
       onHold
      ) mapN { case (_, source, ids, inputs, aux, dep, onHoldActual) =>
  ...
  private def arrayTypeElementValidation[A](data: String,
                                            interpretElementFunction: _root_.io.circe.Json => ErrorOr[A]
  ) = {
    import cats.syntax.validated._
    ...
    val parseInputsTry = Try {
      YamlUtils.parse(data) match {
        ...
        case Right(json) =>
          json.asArray
            .map(_.traverse(interpretElementFunction))
            .getOrElse(interpretElementFunction(json).map(Vector(_)))
            .validNel
        case Left(error) =>
  ...
  private def partialSourcesToSourceCollections(partialSources: ErrorOr[PartialWorkflowSources],
                                                allowNoInputs: Boolean
  ): ErrorOr[Seq[WorkflowSourceFilesCollection]] = {
    ...
    def validateInputsAndRequestedIds(pws: PartialWorkflowSources): ErrorOr[Seq[RequestedIdAndInputs]] = {
      val workflowInputsValidation = (pws.workflowInputs.isEmpty, allowNoInputs) match {
        ...
        case _ =>
          val sortedInputAuxes = pws.workflowInputsAux.toSeq.sortBy { case (index, _) => index } map {
            case (_, inputJson) => Option(inputJson)
          }
      ...
      workflowInputsValidation flatMap { workflowInputs =>
        if (pws.requestedWorkflowIds.isEmpty) {
          ...
          pws.requestedWorkflowIds
            .zip(workflowInputs)
            .map { case (id, inputs) => RequestedIdAndInputs(Option(id), inputs) }
            .validNel
        } else {
    ...
    def validateOptions(options: Option[WorkflowOptionsJson]): ErrorOr[WorkflowOptions] =
      WorkflowOptions.fromJsonString(options.getOrElse("{}")).toErrorOr leftMap {
        _ map { i => s"Invalid workflow options provided: $i" }
      }

    def validateLabels(labels: WorkflowJson): ErrorOr[WorkflowJson] = {

      def validateKeyValuePair(key: String, value: String): ErrorOr[Unit] =
        (Label.validateLabelKey(key), Label.validateLabelValue(value)).tupled.void

      def validateLabelRestrictions(inputs: Map[String, JsValue]): ErrorOr[Unit] =
        inputs.toList
          .traverse[ErrorOr, Unit] {
            case (key, JsString(s)) => validateKeyValuePair(key, s)
            case (key, other) =>
              s"Invalid label $key: $other : Labels must be strings. ${Label.LabelExpectationsMessage}".invalidNel
          }
          .void
    ...
    partialSources match {
      case Valid(partialSource) =>
        (validateInputsAndRequestedIds(partialSource),
         ...
         partialSource.workflowUrl.traverse(validateWorkflowUrl)
        ) mapN { case (wfInputsAndIds, wfOptions, workflowLabels, wfUrl) =>
          wfInputsAndIds.map { case RequestedIdAndInputs(id, inputsJson) =>
  ...
  def validateWorkflowUrl(workflowUrl: String): ErrorOr[WorkflowUrl] = {
    ...
    if (len > MaxWorkflowUrlLength)
      s"Invalid workflow url: url has length $len, longer than the maximum allowed $MaxWorkflowUrlLength characters".invalidNel
  ...
  private def toMap(someInput: Option[String]): ErrorOr[Map[String, JsValue]] =
    someInput match {
      case Some(input: String) =>
        Try(input.parseJson).toErrorOrWithContext(
          s"parse input: '$input', which is not a valid json. Please check for syntactical errors."
        ) flatMap {
          case JsObject(inputMap) => inputMap.validNel
          case j: JsValue =>
            s"Submitted input '$input' of type ${j.getClass.getSimpleName} is not a valid JSON object.".invalidNel
        }
