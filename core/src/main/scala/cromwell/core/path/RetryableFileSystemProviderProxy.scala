...
class RetryableFileSystemProviderProxy[T <: FileSystemProvider](delegate: T, retryParams: RetryParams = RetryParams.Default)(implicit actorSystem: ActorSystem) extends FileSystemProvider {
  ...
  override def getFileSystem(uri: URI): FileSystem = withRetry { () => delegate.getFileSystem(uri) }
  ...
  override def getPath(uri: URI): Path = withRetry { () => delegate.getPath(uri) }
  override def newFileSystem(uri: URI, env: util.Map[String, _]): FileSystem = withRetry { () => delegate.newFileSystem(uri, env) }
  ...
  override def getScheme: String = withRetry { () => delegate.getScheme }
}
