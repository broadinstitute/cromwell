...
sbt -Dakka.actor.debug.lifecycle=on -Dakka.loglevel="DEBUG" -Dakka.log-config-on-start=on -Dsbt.log.noformat=true -Dakka.test.timefactor=${CROMWELL_AKKA_TEST_TIME_FACTOR} -Dbackend.providers.Local.config.filesystems.local.localization.0=copy "testOnly *CallCachingSlickDatabaseSpec"
