    ...
    ast match {
      ...
      case a: Ast if a.isArrayLiteral =>
        anticipatedType match {
          ...
          case _ => Failure(new WomExpressionException(s"Failed to parse $a for files. Found an unexpected Array literal but anticipated a ${anticipatedType.callCachingName}"))
        }
      case a: Ast if a.isTupleLiteral =>
        ...
        if (unevaluatedElements.size == 1) {
          ...
          anticipatedType match {
            ...
            case _ => Failure(new WomExpressionException(s"Failed to parse $a for files. Found an unexpected Pair literal but anticipated a ${anticipatedType.callCachingName}"))
          }
      ...
      case a: Ast if a.isMapLiteral =>
        anticipatedType match {
          ...
          case _ => Failure(new WomExpressionException(s"Failed to parse $a for files. Found an unexpected Map literal but anticipated a ${anticipatedType.callCachingName}"))
        }
