...
/**
  ...
  */
class PubSubMetadataServiceActor(serviceConfig: Config, globalConfig: Config, serviceRegistryActor: ActorRef)
    extends Actor
    with ActorLogging {
  ...
  protected[this] def createPubSubConnection(): GooglePubSubDAO = {
    ...
    val googleAuth = googleConfig.auth(googleAuthName) match {
      ...
      case Valid(doh) =>
        throw new IllegalArgumentException(
          s"Unable to configure PubSubMetadataServiceActor: ${doh.name} was not a service account auth"
        )
      case Invalid(e) =>
        throw new IllegalArgumentException("Unable to configure PubSubMetadataServiceActor: " + e.toList.mkString(", "))
    }
    val jsonAuth = googleAuth.fileFormat match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          "Unable to configure PubSubMetadataServiceActor: the service account must supply a JSON file"
        )
    }
...
object PubSubMetadataServiceActor {
  def props(serviceConfig: Config, globalConfig: Config, serviceRegistryActor: ActorRef) =
    Props(new PubSubMetadataServiceActor(serviceConfig, globalConfig, serviceRegistryActor))
