...
//import cromwell.backend.google.batch.GcpBatchTestConfig.gcpBatchConfiguration
...
class GcpBatchAsyncBackendJobExecutionActorSpec extends TestKitSuite
  ...
  private def executionActor(jobDescriptor: BackendJobDescriptor,
    ...
    class ExecuteOrRecoverActor extends TestableGcpBatchJobExecutionActor(jobDescriptor, promise, gcpBatchConfiguration, batchSingletonActor = batchSingletonActor, serviceRegistryActor = serviceRegistryActor) {
  ...
  it should "generate correct JesFileInputs from a WdlMap" in {
    ...
    womExecutableChecked match {
      case Right(womExecutable) =>
        ...
        val batchInputs = testActorRef.underlyingActor.generateInputs(jobDescriptor)
        batchInputs should have size 8
        batchInputs should contain(GcpBatchFileInput(
  ...
  it should "generate correct BatchOutputs" in {
    ...
    val batchBackend = makeJesActorRef(SampleWdl.FilePassingWorkflow, workflowInputs, "a", callInputs).underlyingActor
    val jobDescriptor = batchBackend.jobDescriptor
    val workflowId = batchBackend.workflowId
    val batchInputs = batchBackend.generateInputs(jobDescriptor)
    ...
    batchOutputs should contain(GcpBatchFileOutput("out",
  ...
  it should "generate correct JesInputs when a command line contains a write_lines call in it" in {
    ...
    val batchBackend = makeJesActorRef(SampleWdl.ArrayIO, Map.empty, "serialize", inputs, functions).underlyingActor
    val jobDescriptor = batchBackend.jobDescriptor
    val batchInputs = batchBackend.generateInputs(jobDescriptor)
    batchInputs should have size 1
    batchInputs should contain(GcpBatchFileInput(
    ...
    val jesOutputs = batchBackend.generateOutputs(jobDescriptor)
