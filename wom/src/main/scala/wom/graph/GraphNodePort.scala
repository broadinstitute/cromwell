...
object GraphNodePort {
  ...
  final case class ConnectedInputPort(name: String, womType: WomType, upstream: OutputPort, g: Unit => GraphNode)
      extends InputPort
      with DelayedGraphNodePort
  ...
  object GraphNodeOutputPort {
    def apply(name: String, womType: WomType, graphNode: GraphNode): GraphNodeOutputPort =
      GraphNodeOutputPort(WomIdentifier(LocalName(name), graphNode.identifier.fullyQualifiedName.combine(name)),
                          womType,
                          graphNode
      )
  }
  case class GraphNodeOutputPort(override val identifier: WomIdentifier, womType: WomType, graphNode: GraphNode)
      extends OutputPort

  object ExpressionBasedOutputPort
  case class ExpressionBasedOutputPort(override val identifier: WomIdentifier,
                                       womType: WomType,
                                       graphNode: GraphNode,
  ...
  /**
    ...
    */
  final case class ScatterGathererPort(womType: WomArrayType,
                                       outputToGather: PortBasedGraphOutputNode,
                                       g: Unit => GraphNode
  ) extends OutputPort
      with DelayedGraphNodePort {
  ...
  /**
    ...
    */
  final case class ConditionalOutputPort(outputToExpose: PortBasedGraphOutputNode, g: Unit => ConditionalNode)
      extends OutputPort
      with DelayedGraphNodePort {
    // Since this port just wraps a PortBasedGraphOutputNode which itself wraps an output port, we can re-use the same identifier
  ...
  /**
    ...
    */
  final case class SubworkflowCallOutputPort(identifier: WomIdentifier,
                                             outputToExpose: GraphOutputNode,
