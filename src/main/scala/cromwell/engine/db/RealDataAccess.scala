...
// TODO Insert real data access logic, rename.
class RealDataAccess extends DataAccess {
  ...
  override def query(workflowId: Option[Seq[WorkflowId]], wdlUris: Option[Seq[String]], states: Option[Seq[WorkflowState]], beforeStart: Option[Date], afterStart: Option[Date], beforeEnd: Option[Date], afterEnd: Option[Date]): Seq[QueryWorkflowExecutionResult] = ???
}
