...
class Draft2SizeFunctionSpec extends AnyFlatSpec with CromwellTimeoutSpec with Matchers {
  ...
  it should "correctly report a 2048 byte file, in bytes" in {
    val readLike = testFunctions(Success(2048L))
    validate(readLike.size(Seq(Success(WomSingleFile("blah")), Success(WomString("B"))))) { res =>
      assert(res == WomFloat(2048d))
    }
  }

  it should "correctly report a 2048 byte file, in KB" in {
    val readLike = testFunctions(Success(2048L))
    validate(readLike.size(Seq(Success(WomSingleFile("blah")), Success(WomString("KB"))))) { res =>
      assert(res == WomFloat(2d))
    }
  }

  it should "correctly report a 2048 byte file, in KiB" in {
    val readLike = testFunctions(Success(2048L))
    validate(readLike.size(Seq(Success(WomSingleFile("blah")), Success(WomString("Ki"))))) { res =>
      assert(res == WomFloat(2d))
    }
  }
  ...
  it should "correctly report the size of a supplied, optional, 2048 byte file" in {
    val readLike = testFunctions(Success(2048L))
    validate(readLike.size(Seq(Success(WomOptionalValue(WomSingleFileType, Option(WomSingleFile("blah"))))))) { res =>
      assert(res == WomFloat(2048d))
    }
  }
  ...
  it should "correctly report the size of a supplied, optional, 2048 byte file, in MB" in {
    ...
    validate(
      readLike.size(
        Seq(Success(WomOptionalValue(WomSingleFileType, Option(WomSingleFile("blah")))), Success(WomString("MB")))
      )
  ...
  it should "correctly report that an unsupplied optional file is empty" in {
    val readLike = testFunctions(Success(2048L))
    validate(readLike.size(Seq(Success(WomOptionalValue(WomSingleFileType, None))))) { res =>
      assert(res == WomFloat(0d))
    }
  }

  it should "correctly report that an unsupplied File?? is empty" in {
    val readLike = testFunctions(Success(2048L))
    validate(readLike.size(Seq(Success(WomOptionalValue(WomOptionalType(WomSingleFileType), None))))) { res =>
      assert(res == WomFloat(0d))
    }
  }
  ...
  it should "correctly report that an unsupplied optional file is empty, even in MB" in {
    val readLike = testFunctions(Success(2048L))
    validate(readLike.size(Seq(Success(WomOptionalValue(WomSingleFileType, None)), Success(WomString("MB"))))) { res =>
      assert(res == WomFloat(0d))
    }
  }
  ...
  it should "refuse to report file sizes for Ints" in {
    val readLike =
      testFunctions(Failure(new Exception("Bad result: WdlIntegers shouldn't even be tried for getting file size")))
    ...
    oops match {
      ...
      case Failure(e) =>
        e.getMessage should be("The 'size' method expects a 'File' or 'File?' argument but instead got Int.")
    }
  }
  ...
  it should "pass on underlying size reading errors" in {
    ...
    oops match {
      case Success(_) =>
        fail(s"The 'size' engine function didn't return the error generated in the inner 'size' method")
...
object Draft2SizeFunctionSpec {
  def testFunctions(sizeResult: Try[Long]): WdlStandardLibraryFunctions = WdlStandardLibraryFunctions.fromIoFunctionSet(
