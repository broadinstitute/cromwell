...
Also note that the local filesystem (the one on which Cromwell runs on) is implicitly accessible but can be disabled (see below).
...
This workflow is valid WDL and does not involve any backend, or even task. It does involve interacting with a filesystem thought to retrieve the content of `my_file.txt`
...
However if the file is located on a different filesystem (a cloud filesystem for instance), we would need to modify the configuration to tell Cromwell how to interact with this filesystem:
...
Similarly to the engine, you can also configure backend filesystems individually. Some backends might require the use of a filesystems.
