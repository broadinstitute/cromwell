...
class JobTokenDispenserActorSpec
  ...
  it should "dispense indefinitely for an infinite token type" in {
    ...
    senders.foreach(sender =>
      actorRefUnderTest.tell(msg = JobTokenRequest(hogGroupA, TestInfiniteTokenType), sender = sender.ref)
    )
  ...
  it should "limit the dispensing of a limited token type" in {
    ...
    // Ask for 20 tokens
    senders.foreach(sender =>
      actorRefUnderTest.tell(msg = JobTokenRequest(hogGroupA, LimitedTo5Tokens), sender = sender.ref)
    )
    ...
    actorRefUnderTest.underlyingActor.tokenAssignments.keySet should contain theSameElementsAs senders
      .map(_.ref)
      .take(5)
      .toSet
    ...
    actorRefUnderTest.underlyingActor
      .tokenQueues(LimitedTo5Tokens)
      .queues
      .flatMap(_._2)
      .toList should contain theSameElementsInOrderAs senders.drop(5).map(asHogGroupAPlaceholder)
    ...
    actorRefUnderTest.underlyingActor
      ...
      .queues
      .flatMap(_._2)
      .toList should contain theSameElementsInOrderAs senders.slice(8, 20).map(asHogGroupAPlaceholder)
    ...
    actorRefUnderTest.underlyingActor
      ...
      .queues
      .flatMap(_._2)
      .last shouldBe TokenQueuePlaceholder(self, "hogGroupA")
  ...
  AkkaTestUtil.actorDeathMethods(system) foreach { case (name, stopMethod) =>
    it should s"recover tokens lost to actors which are $name before they hand back their token" in {
      ...
      val tokenGrabbingActors = (1 to 6).map { i =>
        TestActorRef[TestTokenGrabbingActor](TestTokenGrabbingActor.props(actorRefUnderTest, LimitedTo5Tokens),
                                             grabberSupervisor,
                                             s"grabber_${name}_" + i
        )
      }
  ...
  it should "skip over dead actors when assigning tokens to the actor queue" in {
    ...
    actorRefUnderTest.underlyingActor
      ...
      .queues
      .flatMap(_._2)
      .toList should contain theSameElementsInOrderAs List(nextInLine1, nextInLine2).map(asHogGroupAPlaceholder)
  ...
  it should "be resilient if the last request for a hog group is removed from the queue before a token is dispensed" in {
    ...
    eventually {
      ...
      actorRefUnderTest.underlyingActor.tokenQueues(tokenType).queues.values.foreach { queue =>
