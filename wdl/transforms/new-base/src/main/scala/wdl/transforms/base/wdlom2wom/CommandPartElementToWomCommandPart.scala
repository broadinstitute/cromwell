...
object CommandPartElementToWomCommandPart {
  ...
  ): ErrorOr[CommandPart] = commandPart match {
  ...
  private def validatePlaceholderType(womExpression: WomExpression,
                                      attributes: PlaceholderAttributeSet
  ): ErrorOr[Unit] = ().validNel
}
...
case class WdlomWomPlaceholderCommandPart(attributes: PlaceholderAttributeSet, expression: WdlomWomExpression)
  ...
  override def instantiate(inputsMap: Map[LocalName, WomValue],
    ...
    expression.evaluateValueForPlaceholder(inputsValues,
                                           functions,
                                           ForCommandInstantiationOptions(valueMapper)
    ) flatMap { evaluatedExpression =>
      instantiateFromValue(evaluatedExpression, valueMapper).map(List(_))
    }
  }

  private def instantiateFromValue(value: EvaluatedValue[_],
                                   valueMapper: WomValue => WomValue
  ): ErrorOr[InstantiatedCommand] = value.value match {
    case WomBoolean(b) if attributes.trueAttribute.isDefined && attributes.falseAttribute.isDefined =>
      InstantiatedCommand(
        commandString = if (b) { attributes.trueAttribute.get }
        else { attributes.falseAttribute.get },
        createdFiles = value.sideEffectFiles.toList
      ).validNel
    case p: WomPrimitive =>
      InstantiatedCommand(valueMapper(p).valueString, createdFiles = value.sideEffectFiles.toList).validNel
    case WomOptionalValue(_, Some(v)) => instantiateFromValue(value.copy(value = v), valueMapper)
    case WomOptionalValue(_, None) =>
      attributes.defaultAttribute match {
        case Some(default) =>
          InstantiatedCommand(commandString = default, createdFiles = value.sideEffectFiles.toList).validNel
        case None => InstantiatedCommand(commandString = "", createdFiles = value.sideEffectFiles.toList).validNel
      }
    case WomArray(WomArrayType(_: WomPrimitiveType), arrayValue) =>
