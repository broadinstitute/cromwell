...
final case class WorkflowStoreEngineActor private (store: WorkflowStore,
...
) extends LoggingFSM[WorkflowStoreActorState, WorkflowStoreActorData]
    with ActorLogging
  ...
  when(Idle) { case Event(cmd: WorkflowStoreActorEngineCommand, _) =>
  ...
  private def startNewWork(command: WorkflowStoreActorEngineCommand,
                           sndr: ActorRef,
                           nextData: WorkflowStoreActorData
  ) = {
    val work: Future[Any] = command match {
          ...
          response match {
            case NewWorkflowsToStart(workflows) =>
              ...
              workflowsIds foreach { w =>
                WorkflowProcessingEventPublishing.publish(w,
                                                          workflowHeartbeatConfig.cromwellId,
                                                          PickedUp,
      ...
      case FindWorkflowsWithAbortRequested(cromwellId) =>
        store.findWorkflowsWithAbortRequested(cromwellId) map { ids =>
          sndr ! FindWorkflowsWithAbortRequestedSuccess(ids)
        } recover { case t =>
          sndr ! FindWorkflowsWithAbortRequestedFailure(t)
        }
      case AbortWorkflowCommand(id) =>
        workflowStoreAccess.abort(id) map { workflowStoreAbortResponse =>
          ...
          case WorkflowStoreAbortResponse.NotFound =>
            sndr ! WorkflowAbortFailureResponse(
              id,
  ...
  private def addWorkCompletionHooks[A](command: WorkflowStoreActorEngineCommand, work: Future[A]) =
    work.onComplete {
      case Success(_) =>
        self ! WorkDone
      case Failure(t) =>
        log.error("Error occurred during {}: {} because {}",
  ...
  /**
    ...
    */
  private def newWorkflowMessage(maxWorkflows: Int,
                                 excludedGroups: Set[String]
  ): Future[WorkflowStoreEngineActorResponse] = {
    def fetchStartableWorkflowsIfNeeded =
      if (maxWorkflows > 0) {
        workflowStoreAccess.fetchStartableWorkflows(maxWorkflows,
                                                    workflowHeartbeatConfig.cromwellId,
                                                    workflowHeartbeatConfig.ttl,
                                                    excludedGroups
        )
      } else {
...
object WorkflowStoreEngineActor {
  def props(
    ...
    Props(
      WorkflowStoreEngineActor(workflowStore,
                               workflowStoreAccess,
                               serviceRegistryActor,
                               abortAllJobsOnTerminate,
  ...
  final case class NewWorkflowsToStart(workflows: NonEmptyList[WorkflowToStart])
      extends WorkflowStoreEngineActorResponse

  final case class WorkflowStoreActorCommandWithSender(command: WorkflowStoreActorEngineCommand, sender: ActorRef)

  final case class WorkflowStoreActorData(currentOperation: Option[WorkflowStoreActorCommandWithSender],
                                          pendingOperations: List[WorkflowStoreActorCommandWithSender]
  ) {
    def withCurrentCommand(command: WorkflowStoreActorEngineCommand, sender: ActorRef) =
      this.copy(currentOperation = Option(WorkflowStoreActorCommandWithSender(command, sender)))
    def withPendingCommand(newCommand: WorkflowStoreActorEngineCommand, sender: ActorRef) =
      this.copy(pendingOperations = this.pendingOperations :+ WorkflowStoreActorCommandWithSender(newCommand, sender))
    def pop =
  ...
  case class WorkflowOnHoldToSubmittedFailure(workflowId: WorkflowId, failure: Throwable)
      extends WorkflowOnHoldToSubmittedResponse
}
