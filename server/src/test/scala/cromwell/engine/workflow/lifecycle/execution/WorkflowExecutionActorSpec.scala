...
class WorkflowExecutionActorSpec extends CromwellTestKitSpec with AnyFlatSpecLike with Matchers with BeforeAndAfter with WorkflowDescriptorBuilderForSpecs {
  ...
  it should "allow a backend to tell it to retry... up to a point" in {
    ...
    val jobExecutionTokenDispenserActor = system.actorOf(JobTokenDispenserActor.props(serviceRegistry, Rate(100, 1.second), None, "Execution", "Running"), "JobExecutionTokenDispenser")
