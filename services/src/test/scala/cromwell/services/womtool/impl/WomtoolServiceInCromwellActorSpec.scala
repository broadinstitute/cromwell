...
class WomtoolServiceInCromwellActorSpec extends ServicesSpec {
  ...
  object TestData {
    ...
    val wdlValidDraft2NoInputs: String =
      s"""
         |workflow wf_hello {
         |  call hello
         |}
    ...
    val successfulDescription: WorkflowDescription = WorkflowDescription(
      valid = true,
      errors = List.empty,
      validWorkflow = true,
      name = "wf_hello",
      inputs = List(InputDescription("hello.addressee", WomStringType, "String", optional = false, default = None)),
      outputs = List.empty,
      images = List.empty,
  ...
  "WomtoolServiceInCromwellActor" should {
    ...
    "return valid for a valid workflow" in {
      ...
      check(
        DescribeRequest(wsfc, List.empty),
        DescribeSuccess(
    ...
    "return an error for an empty describe request" in {
      ...
      check(DescribeRequest(wsfc, List.empty), DescribeFailure("Either workflow source or url has to be supplied"))
    }
    ...
    "return an error when both workflow URL and workflow source specified" in {
      ...
      check(DescribeRequest(wsfc, List.empty), DescribeFailure("Both workflow source and url can't be supplied"))
    }
    ...
    "return an error when the workflow URL's host can't be resolved" in {
      ...
      (for {
        result <- (womtoolActor ? DescribeRequest(wsfc, List.empty)).mapTo[DescribeResult]
        _ = result should (
          be(
            DescribeFailure(
