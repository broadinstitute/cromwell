...
class EjeaBackendIsCopyingCachedOutputsSpec
  ...
  "An EJEA in BackendIsCopyingCachedOutputs state" should {
    ...
    case class InitialHashDataAndEjhaResponseCombination(name: String,
      ...
      def hashingCompletedSuccessfully =
        ejhaResponse == hashResultsEjhaResponse || initialHashData == hashResultsDataValue
    }
    ...
    val initialHashDataAndEjhaResponseCombinations = List(
      ...
      InitialHashDataAndEjhaResponseCombination("unsuccessful hashes in initial data",
                                                hashErrorDataValue,
                                                None,
                                                hashErrorDataValue,
                                                mode => !mode.writeToCache
      ),
      // The mode will ALWAYS have "writeToCache" in these scenarios:
      InitialHashDataAndEjhaResponseCombination("unsuccessful hashes from EJHA",
                                                None,
                                                hashErrorEjhaResponse,
                                                hashErrorDataValue,
                                                mode => mode.writeToCache
      ),
      InitialHashDataAndEjhaResponseCombination("successful hashes from EJHA",
                                                None,
                                                hashResultsEjhaResponse,
                                                hashResultsDataValue,
                                                mode => mode.writeToCache
      ),
      InitialHashDataAndEjhaResponseCombination("successful hashes in initial data",
                                                hashResultsDataValue,
                                                None,
                                                hashResultsDataValue,
                                                mode => mode.writeToCache
    ...
    CallCachingModes.filter(_.readFromCache) foreach { mode =>
      initialHashDataAndEjhaResponseCombinations filter { _.validForMode(mode) } foreach {
        case combo @ InitialHashDataAndEjhaResponseCombination(hashComboName,
                                                               initialHashData,
                                                               ejhaResponse,
                                                               finalHashData,
                                                               _
          ...
          if (mode.readFromCache) {
            ...
            s"not invalidate a call for caching if backend copying failed when invalidation is disabled, when it was going to receive $hashComboName, if call caching is $mode" in {
              val invalidationDisabledOptions =
                CallCachingOptions(invalidateBadCacheResults = false, workflowOptionCallCachePrefixes = None)
            ...
            def checkInvalidateOnCopyFailure(expectMetric: Boolean) = {
              ...
              s"invalidate a call for caching $metricsExpectationString if backend copying failed (preserving any received hashes) when call caching is $mode, the EJEA has $hashComboName and then gets a success result" in {
                ...
                // Send the response from the copying actor
                val copyFailureMessage =
                  if (expectMetric) cacheHitBlacklistedResponse(cacheHitNumber)
                  else copyAttemptFailedResponse(cacheHitNumber)
                    ...
                    case InstrumentationServiceMessage(CromwellIncrement(CromwellBucket(prefix, path))) =>
                      ...
                      path should be(
                        NonEmptyList.of(
                          ...
                          "bucketblacklisted",
                          helper.taskName,
                          helper.backendWorkflowDescriptor.hogGroup.value
                        )
                      )
                  }
                }
  ...
  def standardResponsePendingData(hashes: Option[Try[CallCacheHashes]]) =
    ResponsePendingData(helper.backendJobDescriptor, helper.bjeaProps, hashes, Option(helper.ejhaProbe.ref), cacheHit)
  def ejeaInBackendIsCopyingCachedOutputsState(initialHashes: Option[Try[CallCacheHashes]],
