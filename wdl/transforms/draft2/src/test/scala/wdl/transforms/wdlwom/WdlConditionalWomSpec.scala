  ...
  it should "respect expression inputs and convert conditional task outputs into workflow outputs" in {
    ...
    def validateGraph(workflowGraph: Graph) = {
      ...
      def validateOuterGraph: OuterGraphValidations = {
        val conditionalNode = workflowGraph.nodes
          .firstByType[ConditionalNode]
          .getOrElse(fail("Resulting graph did not contain a ConditionalNode"))

        val inputNodes: Set[GraphInputNode] = workflowGraph.nodes.filterByType[GraphInputNode]

        val b_inputNode =
          inputNodes.find(_.localName == "b").getOrElse(fail("Resulting graph did not contain the 'b' GraphInputNode"))
        b_inputNode.womType should be(WomBooleanType)
        val foo_i_inputNode = inputNodes
          .find(_.localName == "foo.i")
          .getOrElse(fail("Resulting graph did not contain the 'foo.i' GraphInputNode"))
        ...
        val expressionNode = workflowGraph.nodes
          .collectFirst {
        ...
        workflowGraph.nodes should be(
          Set(conditionalNode, foo_i_inputNode, b_inputNode, foo_out_output, expressionNode)
        )
      ...
      def validateInnerGraph(validatedOuterGraph: OuterGraphValidations): InnerGraphValidations = {
        val foo_i_innerInput = validatedOuterGraph.conditionalNode.innerGraph.nodes
          .collectFirst {
            case gin: ExternalGraphInputNode if gin.identifier.fullyQualifiedName.value == "conditional_test.foo.i" =>
              gin
          }
          .getOrElse(fail("Conditional inner graph did not contain a GraphInputNode 'foo.i'"))

        val foo_callNode = validatedOuterGraph.conditionalNode.innerGraph.nodes
          .collectFirst {
        ...
        val foo_out_innerOutput = validatedOuterGraph.conditionalNode.innerGraph.nodes
          .collectFirst {
            case gon: GraphOutputNode if gon.localName == "foo.out" => gon
          }
          .getOrElse(fail("Conditional inner graph did not contain a GraphOutputNode 'foo.out'"))

        validatedOuterGraph.conditionalNode.innerGraph.nodes should be(
          Set(foo_i_innerInput, foo_callNode, foo_out_innerOutput)
        )
      ...
      def validateConnections(validatedOuterGraph: OuterGraphValidations, validatedInnerGraph: InnerGraphValidations) =
        ...
        validatedOuterGraph.conditionalNode.conditionalOutputPorts.toList match {
          ...
          case other =>
            fail(
              "Expected exactly one output to be gathered in this conditional but got:" + other.mkString("\n",
                                                                                                         "\n",
                                                                                                         "\n"
              )
            )
