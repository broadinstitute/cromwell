...
/**
  * FIXME: Anything in here should be triaged - a) is it used and b) shared stuff moves to lenthall.
  */
object TerminalUtil {
  ...
  def mdTable(rows: Seq[Seq[String]], header: Seq[String]): String = {
    ...
    val maxWidths = widths.head.indices.map { column => maxWidth(widths, column) }.toSeq
