...
object WdlDraft2WomConditionalNodeMaker extends WomConditionalNodeMaker[If] {
  ...
  /**
    ...
    */
  override def toWomConditionalNode(ifBlock: If,
                                    localLookup: Map[String, OutputPort],
     ...
     */
    val possiblyNeededNestedOgins: Map[String, OuterGraphInputNode] = outerLookup filterNot { case (name, _) =>
      localLookup.contains(name)
    } map { case (name, outerPort) =>
      /*
    ...
    val possiblyNeededNestedOginPorts: Map[String, OutputPort] = possiblyNeededNestedOgins map {
      case (name: String, ogin: OuterGraphInputNode) => name -> ogin.singleOutputPort
    }
    ...
    val ifConditionGraphInputExpressionValidation = WdlWomExpression.toAnonymousExpressionNode(
      WomIdentifier("conditional"),
      ifConditionExpression,
      localLookup ++ possiblyNeededNestedOginPorts,
      Map.empty,
      preserveIndexForOuterLookups,
      ifBlock,
      PlainAnonymousExpressionNode.apply
    )
    val ifConditionTypeValidation = ifConditionExpression.evaluateType((localLookup ++ outerLookup).map { case (k, v) =>
      k -> v.womType
    }) flatMap {
      case coerceable if WomBooleanType.isCoerceableFrom(coerceable) => Valid(())
      case other =>
        s"An if block must be given a boolean expression but instead got '${ifBlock.condition.toWomString}' (a ${other.stableName})".invalidNel
    }
    ...
    (ifConditionGraphInputExpressionValidation, ifConditionTypeValidation, innerGraphValidation) mapN {
      (ifConditionGraphInputExpression, _, innerGraph) =>
