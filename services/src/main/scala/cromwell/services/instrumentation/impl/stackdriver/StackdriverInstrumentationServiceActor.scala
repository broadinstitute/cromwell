...
class StackdriverInstrumentationServiceActor(serviceConfig: Config,
  ...
  val metricServiceSettings =
    MetricServiceSettings.newBuilder.setCredentialsProvider(FixedCredentialsProvider.create(credentials)).build
  ...
  private def sendMetricData(): Unit = {
    metricsMap.foreach { case (key, value: Vector[Double]) =>
      ...
      writeMetrics(key, dataPoint) recover { case e =>
        logger.error(s"Failed to send metrics to Stackdriver API for metric ${key.name} with value $dataPoint.", e)
      }
    }
  ...
  private def writeMetrics(metricObj: StackdriverMetric, value: Double): Try[Unit] = {
    def timeInterval(metricKind: StackdriverMetricKind): TimeInterval =
      metricKind match {
        case StackdriverGauge =>
          TimeInterval.newBuilder.setEndTime(Timestamps.fromMillis(System.currentTimeMillis)).build
        case StackdriverCumulative =>
          TimeInterval.newBuilder
            .setStartTime(ActorCreationTime)
            .setEndTime(Timestamps.fromMillis(System.currentTimeMillis))
            .build
      }

    def createTimeSeries(metricKind: StackdriverMetricKind,
                         metric: Metric,
                         resource: MonitoredResource,
                         dataPointList: util.List[Point]
    ): TimeSeries =
      metricKind match {
        case StackdriverGauge =>
          TimeSeries.newBuilder.setMetric(metric).setResource(resource).addAllPoints(dataPointList).build
        case StackdriverCumulative =>
          TimeSeries.newBuilder
            .setMetric(metric)
            .setResource(resource)
            .setMetricKind(MetricDescriptor.MetricKind.CUMULATIVE)
            .addAllPoints(dataPointList)
            .build
      }
    ...
    val metric: Metric =
      Metric.newBuilder.setType(s"$CustomMetricDomain/${metricObj.name}").putAllLabels(metricLabelsMap.asJava).build
    ...
    val timeSeriesRequest =
      CreateTimeSeriesRequest.newBuilder.setName(projectName.toString).addAllTimeSeries(timeSeriesList.asJava).build
...
object StackdriverInstrumentationServiceActor {
  ...
  def props(serviceConfig: Config, globalConfig: Config, serviceRegistryActor: ActorRef) = Props(
    new StackdriverInstrumentationServiceActor(serviceConfig, globalConfig, serviceRegistryActor)
  )
