...
object WomMap {
  def coerceMap(m: Map[_, _], womMapType: WomMapType): WomMap = {
    val coerced: Map[Try[WomValue], Try[WomValue]] = m map { case (k, v) =>
      womMapType.keyType.coerceRawValue(k) -> womMapType.valueType.coerceRawValue(v)
    }
  ...
  def fromTsv(tsv: String, womMapType: WomMapType = WomMapType(WomAnyType, WomAnyType)): Try[WomMap] =
    FileUtil.parseTsv(tsv) match {
      ...
      case Success(table) if table.head.length != 2 =>
        Failure(new UnsupportedOperationException("TSV must be 2 columns to convert to a Map"))
...
final case class WomMap private (womType: WomMapType, value: Map[WomValue, WomValue])
    extends WomValue
    with WomArrayLike
  ...
  if (typesUsedInKey.size == 1 && typesUsedInKey.head != womType.keyType)
    throw new UnsupportedOperationException(
      s"Could not construct a $womType with map keys of unexpected type: [${value.keys.mkString(", ")}]"
    )

  if (typesUsedInKey.size > 1)
    throw new UnsupportedOperationException(
      s"Cannot construct $womType with mixed types in map keys: [${value.keys.mkString(", ")}]"
    )
  ...
  if (typesUsedInValue.size == 1 && typesUsedInValue.head != womType.valueType)
    throw new UnsupportedOperationException(
      s"Could not construct a $womType with map values of unexpected type: [${value.values.mkString(", ")}]"
    )

  if (typesUsedInValue.size > 1)
    throw new UnsupportedOperationException(
      s"Cannot construct $womType with mixed types in map values: [${value.values.mkString(", ")}]"
    )
  ...
  def tsvSerialize: Try[String] =
    (womType.keyType, womType.valueType) match {
      ...
      case (_: WomPrimitiveType, _: WomPrimitiveType) =>
        ...
        Success(
          value
            .map { case (k, v) => s"${k.valueString}\t${v.valueString}" }
            .mkString(start = "", sep = "\n", end = "\n")
        )
      case _ =>
  ...
  def traverseValues[R <: WomValue, G[_]](f: WomValue => G[R])(implicit applicative: Applicative[G]): G[WomMap] =
    if (value.isEmpty) applicative.pure(this)
      ...
      val traverseFunction: (WomValue, WomValue) => G[(WomValue, R)] = { case (key, v) =>
        applicative.map(f(v))(key -> _)
      }
  ...
  override def collectAsSeq[T <: WomValue](filterFn: PartialFunction[WomValue, T]): Seq[T] = {
    val collected = value flatMap { case (k, v) =>
      Seq(k.collectAsSeq(filterFn), v.collectAsSeq(filterFn))
    }
    collected.flatten.toSeq
  }

  override def initialize(ioFunctionSet: IoFunctionSet): IOChecked[WomValue] = traverseValues(
