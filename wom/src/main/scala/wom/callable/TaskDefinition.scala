...
sealed trait TaskDefinition extends Callable {
  ...
  def instantiateCommand(taskInputs: WomEvaluatedCallInputs,
    ...
    // Just the raw command parts, no separators.
    val rawParts = commandTemplate.toList.map(_.instantiate(mappedInputs, functions, valueMapper, runtimeEnvironment))
    // Add separators command parts and monoid smash down to one instantiated command.
    val instantiatedCommand = rawParts.intercalate(InstantiatedCommand(commandPartSeparator).validNel)
