...
class StatsDInstrumentationServiceActorSpec
  ...
  override def beforeAll(): Unit = {
    ...
    udpListenerActor =
      system.actorOf(Props(new UDPListenerActor(new InetSocketAddress("localhost", 8125), udpProbe.ref)))
  ...
  List(
    ...
    StatsDTestBit("set gauges",
                  CromwellGauge(testGaugeBucket, 89),
    ...
    StatsDTestBit(
      ...
      Set(
        "prefix_value.cromwell.test_prefix.test.metric.bucket.timing.stddev:0.00|g",
    ...
    StatsDTestBit(
      "increment counters with cromwell id prefix",
      CromwellIncrement(testBucket),
    ...
    StatsDTestBit(
      "add count with cromwell id prefix",
      CromwellCount(testBucket, 80),
    ...
    StatsDTestBit(
      "set gauges with cromwell id prefix",
      CromwellGauge(testGaugeBucket, 89),
      Set("prefix_value.cromwell.cromwell-101.test_prefix.test.gauge.metric.bucket:89|g"),
      prependCromwellInstancePrefix = true
    ),
    StatsDTestBit(
      "set timings with cromwell id prefix",
      CromwellTiming(testBucket.expand("timing"), 5.seconds),
      Set(
        "prefix_value.cromwell.cromwell-101.test_prefix.test.metric.bucket.timing.stddev:0.00|g",
    ...
    case StatsDTestBit(description,
                       metric,
                       expectedExactPackets,
                       expectedFuzzyPackets,
                       prependCromwellInstancePrefix
        ) =>
      it should description in {

        val globalConfig = if (prependCromwellInstancePrefix) cromwellInstanceConfig else ConfigFactory.empty()

        val instrumentationActor =
          TestActorRef(new StatsDInstrumentationServiceActor(config, globalConfig, registryProbe))
        instrumentationActor ! InstrumentationServiceMessage(metric)
        val received = udpProbe.receiveWhile(patience) { case Udp.Received(data, _) =>
          data.utf8String
        }

        expectedExactPackets foreach { packet =>
          if (!received.contains(packet)) {
            val prefix = packet.split(":").head
            received.find(_.startsWith(prefix)) match {
              case Some(sharedPrefix) =>
                fail(s"Missing packet: $packet, but found: $sharedPrefix. Should this be a fuzzy packet?")
              case None => fail(s"Missing packet: $packet, and no packets received with prefix $prefix")
            }
          }
        }
        expectedFuzzyPackets foreach { packet =>
          if (!received.exists(_.contains(packet))) fail(s"Missing fuzzy packet: $packet")
        }
      }
  }
