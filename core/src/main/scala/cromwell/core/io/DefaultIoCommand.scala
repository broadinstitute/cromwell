...
object DefaultIoCommand {
  case class DefaultIoCopyCommand(override val source: Path,
    ...
    override def commandDescription: String = s"DefaultIoCopyCommand.init source '$source' destination '$destination' overwrite '$overwrite'"
  }
  ...
  case class DefaultIoContentAsStringCommand(override val file: Path, override val options: IoReadOptions) extends IoContentAsStringCommand(file, options) {
    override def commandDescription: String = s"DefaultIoContentAsStringCommand.init file '$file' options '$options'"
  }

  case class DefaultIoSizeCommand(override val file: Path) extends IoSizeCommand(file) {
    override def commandDescription: String = s"DefaultIoSizeCommand.init file '$file'"
  }
  ...
  case class DefaultIoWriteCommand(override val file: Path,
    ...
    override def commandDescription: String = s"DefaultIoWriteCommand.init file '$file' content length " +
  ...
  case class DefaultIoDeleteCommand(override val file: Path,
    ...
    override def commandDescription: String = s"DefaultIoDeleteCommand.init file '$file' swallowIOExceptions '$swallowIOExceptions'"
  }

  case class DefaultIoHashCommand(override val file: Path) extends IoHashCommand(file) {
    override def commandDescription: String = s"DefaultIoHashCommand.init file '$file'"
  }

  case class DefaultIoTouchCommand(override val file: Path) extends IoTouchCommand(file) {
    override def commandDescription: String = s"DefaultIoTouchCommand.init file '$file'"
  }

  case class DefaultIoExistsCommand(override val file: Path) extends IoExistsCommand(file) {
    override def commandDescription: String = s"DefaultIoExistsCommand.init file '$file'"
  }

  case class DefaultIoReadLinesCommand(override val file: Path) extends IoReadLinesCommand(file) {
    override def commandDescription: String = s"DefaultIoReadLinesCommand.init file '$file'"
  }

  case class DefaultIoIsDirectoryCommand(override val file: Path) extends IoIsDirectoryCommand(file) {
    override def commandDescription: String = s"DefaultIoIsDirectoryCommand.init file '$file'"
  }
}
